// The dark.sss parity gate (ui-theme-migration.md P1): the embedded stylesheet, parsed, must
// declare the SAME styling as the legacy C++ rule builder - rule for rule, value for value -
// BEFORE the C++ body may be deleted. Comparison is by MERGED selector signature (several
// source rules may target one selector; the cascade merges them identically in both sheets),
// with drawables compared structurally (type + fields; state lists recursed per state).
// This test retires when the consistency pass deliberately supersedes the legacy look.
...
namespace
{
    ...
    SelKey KeyOf(const StyleSelector& sel)
    {
        SelKey k;
        k.type = sel.ViewType;
    ...
    void MergeSheet(const StyleSheet& sheet, Array<MergedRule>& out)
    {
    ...
    bool DrawablesEquivalent(Drawable* a, Drawable* b); // fwd

    bool StateListsEquivalent(StateListDrawable* a, StateListDrawable* b)
    {
    ...
    bool DrawablesEquivalent(Drawable* a, Drawable* b)
    {
        if (a == b)
        {
        ...
        if (auto* ca = core::Cast<ColorDrawable>(a))
        {
            auto* cb = core::Cast<ColorDrawable>(b);
            return cb != nullptr && ColorsEqual(ca->Color, cb->Color);
        }
        if (auto* ra = core::Cast<RoundedRectDrawable>(a))
        {
            auto* rb = core::Cast<RoundedRectDrawable>(b);
        ...
        if (auto* sa = core::Cast<StateListDrawable>(a))
        {
            auto* sb = core::Cast<StateListDrawable>(b);
            return sb != nullptr && StateListsEquivalent(sa, sb);
        }
        if (core::Cast<SVGDrawable>(a) != nullptr)
        {
            return core::Cast<SVGDrawable>(b) != nullptr; // same glyph family suffices
        }
        // Unknown drawable type: require exact type match at least.
        return a->GetType() == b->GetType();
    }

    bool ValuesEquivalent(const StyleValue& a, const StyleValue& b)
    {
        ...
        return true; // string/unhandled kinds unused by this theme
    }
}
...
TEST_CASE("theme-parity: dark.sss (parsed) matches the legacy C++ DarkTheme rule-for-rule")
{
    ...
    // Every legacy selector must exist in the parsed sheet with equivalent properties, and
    // vice versa (two loops so a failure names the missing side).
    for (const MergedRule& lm : b)
    {
        ...
        if (pm == nullptr)
        {
            std::printf("PARITY: legacy selector missing from dark.sss: type=%s classes='%s' "
                        "state=%u pseudo='%s'\n",
    ...
    for (const MergedRule& pm : a)
    {
        ...
        INFO("dark.sss declares a selector the legacy theme does not");
        CHECK(found);
    }
}
