...
namespace foundation::render
{
                          ...
                          u32 viewIndex, u32 frameIndex)
    {
        ...
        // Hot reload: rebuild both pipelines when the shaders changed.
        const u64 shaderVersion =
            m_shaders->Version(u8"ssgi") + m_shaders->Version(u8"ssgi_resolve");
        ...
        graph.AddRenderPass(
            u8"ssgi.trace",
            [this, &graph, hdr, depth, normal, gi, pc](rendergraph::PassBuilder& b)
            {
                ...
                b.NeverCull();
                b.SetExecute(
                    [this, &graph, hdr, depth, normal, pc](rhi::RenderPassEncoder& rp)
                    {
                        rhi::BindGroup* bg = EnsureBindGroup(
                            graph.GetTextureView(hdr), graph.GetTextureView(depth),
                            graph.GetTextureView(normal), Combine(graph, hdr, depth, normal));
                        if (bg == nullptr)
                        {
                            return;
                        }
        ...
        graph.AddRenderPass(
            u8"ssgi.resolve",
            [this, &graph, gi, histPrev, velocity, hdr, out, histCur, histPrevView,
             rpc2](rendergraph::PassBuilder& b)
            {
                ...
                b.NeverCull();
                b.SetExecute(
                    [this, &graph, gi, velocity, hdr, histPrevView,
                     rpc2](rhi::RenderPassEncoder& rp)
                    {
                        rhi::BindGroup* bg = EnsureResolveBindGroup(
                            graph.GetTextureView(gi), histPrevView,
