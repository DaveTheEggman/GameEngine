...
// UI.Gamekit - :toast partition
//
// Transient notification overlay ("toasts"): a full-viewport pass-through layer that stacks
// notification cards in its bottom-right corner, newest nearest the corner. COPIED from the toolkit
// ToastHost (foundation.ui.toolkit:toast_host) into the game kit per the "gamekit uses CORE, not
// toolkit" rule - identical behaviour, only the namespace/module differ.
//
// Behavior contract:
...
//    hittable), so it can overlay the whole window/screen root.
...
//  - The owning app calls Update(dt) once per frame - there is no self-ticking.

module;
#include "Core/Prelude.h"
#include "Core/Reflection/Reflect.h"

export module foundation.ui.gamekit:toast;
...
export namespace foundation::ui::gamekit
{
    ...
    class ToastCard : public foundation::ui::FlexLayout
    {
        RTTI_OBJECT(ToastCard, foundation::ui::FlexLayout)
    public:
        ...
        {
            const Color bg = ResolveStyleColor(foundation::ui::StyleProperty::Background,
                                               Color{0.13f, 0.14f, 0.17f, 0.97f});
            const f32 r = ResolveStyleFloat(foundation::ui::StyleProperty::CornerRadius, 0.0f);
            const Rectangle bounds{0, 0, Width(), Height()};
            if (r > 0.0f)
            {
                ctx.VG().FillRoundedRect(bounds, r, bg);
                // Accent trim over the card's left edge; its width equals the corner radius so its
                // rounded left corners coincide exactly with the card's.
    ...
    /// Bottom-right toast stack. Attach to a screen/window RootView (it fills the viewport and passes
    /// input through outside the cards); call Update(dt) once per frame.
    class ToastHost : public foundation::ui::ViewGroup
    {
        RTTI_OBJECT(ToastHost, foundation::ui::ViewGroup)
    public:
    ...
    protected:
        ...
        {
            const foundation::ui::BoxConstraints cardConstraints{ToastWidth, ToastWidth, 0.0f, 200.0f};
            for (const Entry& e : m_entries)
            {
        ...
        void OnLayout(f32, f32, f32 width, f32 height) override
        {
            ...
            for (usize i = m_entries.Size(); i-- > 0;)
            {
                foundation::ui::View* card = m_entries[i].card;
    ...
    private:
        struct Entry
        {
            ...
            f32 duration = 0.0f;
            bool closing = false;
            foundation::ui::View* card = nullptr; // borrowed; the child tree owns it
        ...
        [[nodiscard]] static foundation::ui::StyleProperty SeverityProperty(ToastSeverity severity)
        {
            switch (severity)
            {
            case ToastSeverity::Success:
                return foundation::ui::StyleProperty::SuccessColor;
            case ToastSeverity::Warning:
                return foundation::ui::StyleProperty::WarningColor;
            case ToastSeverity::Error:
                return foundation::ui::StyleProperty::ErrorColor;
            case ToastSeverity::Info:
            default:
                return foundation::ui::StyleProperty::AccentColor;
            }
        }
    ...
    RTTI_DEFINE_OBJECT(ToastCard, "rtti::ui.gamekit")
    RTTI_DEFINE_OBJECT(ToastHost, "rtti::ui.gamekit")
}
