...
{
    const StringView file = u8"raptor_vfs_test.tmp";
...
TEST_CASE("vfs: writable + enumerable round-trip")
{
    ...
    REQUIRE(w->Save(u8"raptor_vfs_dir/sub/blob.bin", Span<const byte>{ payload, ArrayCount(payload) }).IsOk());
    CHECK(native.Exists(u8"raptor_vfs_dir/sub/blob.bin"));

    // Enumerate the subfolder; the blob is listed and is not a directory.
    Array<DirEntry> entries;
    REQUIRE(e->Enumerate(u8"raptor_vfs_dir/sub", entries).IsOk());
    ...
    REQUIRE(e->Enumerate(u8"raptor_vfs_dir", parent).IsOk());
    bool foundSub = false;
    ...
    CHECK_FALSE(e->Enumerate(u8"raptor_vfs_dir/nope", missing).IsOk());

    // Cleanup.
    CHECK(w->Delete(u8"raptor_vfs_dir/sub/blob.bin").IsOk());
    CHECK(RemoveDirectory(u8"raptor_vfs_dir/sub"));
    CHECK(RemoveDirectory(u8"raptor_vfs_dir"));
}
