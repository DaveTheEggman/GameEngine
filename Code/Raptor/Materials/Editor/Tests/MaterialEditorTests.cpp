...
namespace
{
    void RemoveTree()
    {
        FileDelete(u8"raptor_mat_edit_db/lit.rasset");
        RemoveDirectory(u8"raptor_mat_edit_db");
    }
}
...
TEST_CASE("material editor: cooks a MaterialAsset -> MaterialSource")
{
    ...
    NativeFileSystem outMount(u8"raptor_mat_edit_db");
    Guid id;
    ...
    // --- cook: author a material, import into an asset, build into the output DB ---
    {
        ...
        RefPtr<Material> authored = MaterialBuilder(u8"litMat")
            .Shader(u8"lit")
            .Transparent()
            .Color(u8"tint", Vec4{ 0.1f, 0.2f, 0.3f, 1.0f })
