/// Raptor::Materials — the `:instance` partition.
///
...
export module raptor.materials:instance;
...
public:
    ...
    void SetFloat2(StringView n, Vec2 v){ WriteUniform(n, &v, sizeof(v)); }
    void SetFloat3(StringView n, Vec3 v){ WriteUniform(n, &v, sizeof(v)); }
    void SetFloat4(StringView n, Vec4 v){ WriteUniform(n, &v, sizeof(v)); }
    void SetColor(StringView n, Vec4 c) { SetFloat4(n, c); }
    ...
    [[nodiscard]] Span<const u8> UniformData() const noexcept {
        return m_uniformData.Size() > 0 ? Span<const u8>{ m_uniformData.Data(), m_uniformData.Size() }
    ...
    void ResetProperty(StringView n) {
        ...
        if (d.IsUniform() && m_uniformData.Size() >= d.offset + d.size) {
            ...
            if (defaults.Size() >= d.offset + d.size) { MemCopy(m_uniformData.Data() + d.offset, defaults.Data() + d.offset, d.size); }
            SetUniformDirty();
        } else if (d.IsTexture()) { m_textures.Remove(i); SetBindGroupDirty(); }
        else if (d.IsSampler())   { m_samplers.Remove(i); SetBindGroupDirty(); }
        m_overrides.Clear(i);
    }
