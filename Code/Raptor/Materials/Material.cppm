/// Raptor::Materials — the `:material` partition.
///
...
export module raptor.materials:material;
...
public:
    ...
    void SetDefaultFloat2(StringView n, Vec2 v)      { WriteUniform(n, &v, sizeof(v)); }
    void SetDefaultFloat3(StringView n, Vec3 v)      { WriteUniform(n, &v, sizeof(v)); }
    void SetDefaultFloat4(StringView n, Vec4 v)      { WriteUniform(n, &v, sizeof(v)); }
    void SetDefaultColor(StringView n, Vec4 c)       { SetDefaultFloat4(n, c); }

    void SetDefaultTexture(StringView n, rhi::TextureView* tex) {
        const isize i = GetPropertyIndex(n);
        if (i >= 0 && m_properties[static_cast<usize>(i)].IsTexture()) { m_defaultTextures.InsertOrAssign(static_cast<usize>(i), tex); }
    }
    void SetDefaultSampler(StringView n, rhi::Sampler* s) {
        const isize i = GetPropertyIndex(n);
        if (i >= 0 && m_properties[static_cast<usize>(i)].IsSampler()) { m_defaultSamplers.InsertOrAssign(static_cast<usize>(i), s); }
    }
    ...
    void SetRawDefaultUniformData(Span<const u8> data) {
        ...
        if (n > 0) { MemCopy(m_defaultUniformData.Data(), data.Data(), n); }
    }
...
RAPTOR_DEFINE_OBJECT(Material, "raptor::materials")
