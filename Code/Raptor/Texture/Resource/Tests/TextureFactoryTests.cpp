...
namespace
{
    void RemoveTree()
    {
        FileDelete(u8"raptor_texfac_db/tex.rasset");
        FileDelete(u8"raptor_texfac_db/tex.data.bin");
        RemoveDirectory(u8"raptor_texfac_db");
    }
}
...
TEST_CASE("texture.factory: cooked TextureResource -> live GPU Texture")
{
    ...
    NativeFileSystem mount(u8"raptor_texfac_db");
    Guid id;
