...
{
    usize len = 0;
    CHECK(XmlLexer::ReadName(u8"element", len) == XmlResult::Ok); CHECK(len == 7u);
    CHECK(XmlLexer::ReadName(u8"ns:element", len) == XmlResult::Ok); CHECK(len == 10u);
    CHECK(XmlLexer::ReadName(u8"_underscore", len) == XmlResult::Ok); CHECK(len == 11u);
    CHECK(XmlLexer::ReadName(u8"name123", len) == XmlResult::Ok); CHECK(len == 7u);
    CHECK(XmlLexer::ReadName(u8"name-with-dash", len) == XmlResult::Ok); CHECK(len == 14u);
    CHECK(XmlLexer::ReadName(u8"name.with.dot", len) == XmlResult::Ok); CHECK(len == 13u);
}
...
TEST_CASE("xml.lexer: ReadTextContent")
{
    ...
    CHECK(output == StringView(u8"hello")); CHECK(len == 5u);
    output.Clear();
...
TEST_CASE("xml.lexer: ReadCDataContent")
{
    ...
    CHECK(output == StringView(u8"content")); CHECK(len == 10u);
    output.Clear();
    CHECK(XmlLexer::ReadCDataContent(u8"text with <special> chars]]>", len, output) == XmlResult::Ok);
    CHECK(output == StringView(u8"text with <special> chars"));
    output.Clear();
    CHECK(XmlLexer::ReadCDataContent(u8"unclosed content", len, output) == XmlResult::CDataUnclosed);
}
...
TEST_CASE("xml.lexer: ReadCommentContent")
{
    ...
    CHECK(output == StringView(u8"comment")); CHECK(len == 10u);
    output.Clear();
    ...
    output.Clear();
    CHECK(XmlLexer::ReadCommentContent(u8"illegal -- sequence-->", len, output) == XmlResult::CommentIllegalSequence);
}

TEST_CASE("xml.lexer: ReadProcessingInstruction")
{
    String target, data; usize len = 0;
    CHECK(XmlLexer::ReadProcessingInstruction(u8"target?>", len, target, data) == XmlResult::Ok);
    CHECK(target == StringView(u8"target")); CHECK(data.IsEmpty());
    target.Clear(); data.Clear();
    CHECK(XmlLexer::ReadProcessingInstruction(u8"target data?>", len, target, data) == XmlResult::Ok);
    CHECK(target == StringView(u8"target")); CHECK(data == StringView(u8"data"));
    target.Clear(); data.Clear();
    CHECK(XmlLexer::ReadProcessingInstruction(u8"php echo 'hello'; ?>", len, target, data) == XmlResult::Ok);
    CHECK(target == StringView(u8"php")); CHECK(data == StringView(u8"echo 'hello'; "));
}
...
TEST_CASE("xml.lexer: SplitQualifiedName")
{
    ...
    CHECK(prefix == StringView(u8"a")); CHECK(localName == StringView(u8"b:c"));
}
