// Raptor::Xml — :namespace partition
//
...
export namespace raptor::xml
{
    ...
    public:
        ...
        [[nodiscard]] static XmlResult ValidateNamespaceDeclaration(StringView prefix, StringView uri)
        {
            // "xml" may only be bound to the XML namespace.
            if (prefix == StringView(u8"xml") && uri != XmlNamespaces::Xml) { return XmlResult::PrefixReserved; }
            // "xmlns" cannot be declared.
            if (prefix == StringView(u8"xmlns")) { return XmlResult::PrefixReserved; }
            // The XML namespace can only bind to "xml".
            if (uri == XmlNamespaces::Xml && prefix != StringView(u8"xml")) { return XmlResult::NamespaceInvalid; }
            // The XMLNS namespace cannot bind to any prefix.
            if (uri == XmlNamespaces::Xmlns) { return XmlResult::NamespaceInvalid; }
            return XmlResult::Ok;
        }
        ...
        [[nodiscard]] static bool StartsWithXml(StringView name)
        {
            ...
            return (c0 == u8'x' || c0 == u8'X') && (c1 == u8'm' || c1 == u8'M') && (c2 == u8'l' || c2 == u8'L');
        }
    };
}
