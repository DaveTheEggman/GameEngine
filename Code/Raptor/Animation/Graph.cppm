/// Raptor::Animation — the `:graph` partition.
///
...
public:
    ...
    virtual void Evaluate(const Skeleton& skeleton, f32 normalizedTime, Span<BoneTransform> outPoses) const = 0;
...
public:
    ...
    void FireEvents(f32 prevNorm, f32 currentNorm, bool looping, const AnimationEventHandler& handler) const override {
        if (m_clip == nullptr || !handler || m_clip->Events().IsEmpty() || m_clip->duration <= 0.0f) { return; }
        ...
        if (looping && currentNorm < prevNorm) {
            ...
            for (const AnimationEvent& e : m_clip->Events()) { if (e.time > prevAbs && e.time <= m_clip->duration) { handler(StringView{ e.name }, e.time); } }
            for (const AnimationEvent& e : m_clip->Events()) { if (e.time <= curAbs) { handler(StringView{ e.name }, e.time); } }
        } else {
...
public:
    ...
    void SetWeight(i32 boneIndex, f32 weight) {
        if (boneIndex >= 0 && static_cast<usize>(boneIndex) < m_weights.Size()) { m_weights[static_cast<usize>(boneIndex)] = Clamp(weight, 0.0f, 1.0f); }
    }
    void SetAll(f32 weight) { const f32 c = Clamp(weight, 0.0f, 1.0f); for (usize i = 0; i < m_weights.Size(); ++i) { m_weights[i] = c; } }
    ...
    [[nodiscard]] Span<const f32> Weights() const noexcept { return { m_weights.Data(), m_weights.Size() }; }
private:
    Array<f32> m_weights;
};
...
public:
    ...
    void Evaluate(const Skeleton& skeleton, f32 normalizedTime, Span<BoneTransform> outPoses) const override {
        ...
        Array<BoneTransform> a; a.Resize(n);
        Array<BoneTransform> b; b.Resize(n);
        SampleClip(*clipA, skeleton, normalizedTime * clipA->duration, Span<BoneTransform>{ a.Data(), n });
        SampleClip(*clipB, skeleton, normalizedTime * clipB->duration, Span<BoneTransform>{ b.Data(), n });
        BlendPoses(Span<const BoneTransform>{ a.Data(), n }, Span<const BoneTransform>{ b.Data(), n }, blend, outPoses);
    }
...
private:
    void SampleEntry(usize idx, const Skeleton& skeleton, f32 normalizedTime, Span<BoneTransform> outPoses) const {
...
struct BlendTree2DEntry { Vec2 position = Vec2{ 0, 0 }; AnimationClip* clip = nullptr; };
...
public:
    ...
    void AddEntry(Vec2 position, AnimationClip* clip) { m_entries.PushBack(BlendTree2DEntry{ position, clip }); }
    void AddEntry(f32 x, f32 y, AnimationClip* clip) { m_entries.PushBack(BlendTree2DEntry{ Vec2{ x, y }, clip }); }

    void Evaluate(const Skeleton& skeleton, f32 normalizedTime, Span<BoneTransform> outPoses) const override {
        if (m_entries.IsEmpty()) { return; }
        if (m_entries.Size() == 1) { SampleEntry(0, skeleton, normalizedTime, outPoses); return; }

        const Vec2 paramPos{ parameterX, parameterY };
        ...
        const usize n = static_cast<usize>(skeleton.BoneCount());
        Array<BoneTransform> temp; temp.Resize(n);
        bool firstSample = true;
        for (usize i = 0; i < m_entries.Size(); ++i) {
            if (weights[i] < 0.001f || m_entries[i].clip == nullptr) { continue; }
            AnimationClip* clip = m_entries[i].clip;
            SampleClip(*clip, skeleton, normalizedTime * clip->duration, Span<BoneTransform>{ temp.Data(), n });
            if (firstSample) {
            ...
            BlendPoses(Span<const BoneTransform>{ outPoses.Data(), outPoses.Size() }, Span<const BoneTransform>{ temp.Data(), n }, relative, outPoses);
        }
    }
...
private:
    [[nodiscard]] bool CompareFloat(f32 value) const {
        switch (op) {
        case ComparisonOp::Equal:        return Abs(value - threshold) < 0.0001f;
        case ComparisonOp::NotEqual:     return Abs(value - threshold) >= 0.0001f;
...
public:
    ...
    [[nodiscard]] const Array<AnimationGraphCondition>& Conditions() const noexcept { return m_conditions; }

    void AddBoolCondition(i32 parameterIndex, bool expected = true) { m_conditions.PushBack(AnimationGraphCondition{ parameterIndex, ComparisonOp::Equal, expected ? 1.0f : 0.0f }); }
    void AddFloatCondition(i32 parameterIndex, ComparisonOp op, f32 threshold) { m_conditions.PushBack(AnimationGraphCondition{ parameterIndex, op, threshold }); }
    void AddIntCondition(i32 parameterIndex, ComparisonOp op, i32 threshold) { m_conditions.PushBack(AnimationGraphCondition{ parameterIndex, op, static_cast<f32>(threshold) }); }
...
public:
    ...
    AnimationGraphState(StringView name, UniquePtr<IAnimationStateNode> node)
        : m_name(name), m_node(node.Get()), m_owned(static_cast<UniquePtr<IAnimationStateNode>&&>(node)), m_ownsNode(true) {}
...
public:
    ...
    [[nodiscard]] Array<UniquePtr<AnimationGraphTransition>>& Transitions() noexcept { return m_transitions; }
    [[nodiscard]] BoneMask* Mask() const noexcept { return m_mask.Get(); }
    void SetMask(UniquePtr<BoneMask> mask) { m_mask = static_cast<UniquePtr<BoneMask>&&>(mask); }

    i32 AddState(UniquePtr<AnimationGraphState> state) { const i32 idx = static_cast<i32>(m_states.Size()); m_states.PushBack(static_cast<UniquePtr<AnimationGraphState>&&>(state)); return idx; }
    void AddTransition(UniquePtr<AnimationGraphTransition> t) { m_transitions.PushBack(static_cast<UniquePtr<AnimationGraphTransition>&&>(t)); }
...
public:
    ...
    i32 AddLayer(UniquePtr<AnimationLayer> layer) { const i32 idx = static_cast<i32>(m_layers.Size()); m_layers.PushBack(static_cast<UniquePtr<AnimationLayer>&&>(layer)); return idx; }
private:
...
public:
    AnimationGraphPlayer(AnimationGraph& graph, Skeleton& skeleton) : m_graph(&graph), m_skeleton(&skeleton) {
        ...
        for (usize i = 0; i < boneCount; ++i) { m_skinningMatrices[i] = Mat4::Identity(); m_prevSkinningMatrices[i] = Mat4::Identity(); }
        ...
        for (auto& layer : graph.Layers()) {
            for (auto& state : layer->States()) {
                ...
                if (node->Type() == NodeType::BlendTree1D) {
                    ...
                    if (t->parameterIndex >= 0) { m_links1D.PushBack(Link1D{ t, t->parameterIndex }); }
                } else if (node->Type() == NodeType::BlendTree2D) {
                    auto* t = static_cast<BlendTree2D*>(node);
                    if (t->parameterIndexX >= 0 || t->parameterIndexY >= 0) { m_links2D.PushBack(Link2D{ t, t->parameterIndexX, t->parameterIndexY }); }
                }
            }
        }
        ResetToBind();
    ...
    [[nodiscard]] Span<const Mat4> GetPrevSkinningMatrices() const { return { m_prevSkinningMatrices.Data(), m_prevSkinningMatrices.Size() }; }
...
private:
    ...
    [[nodiscard]] Span<const AnimationGraphParameter> Params() const noexcept { return { m_parameters.Data(), m_parameters.Size() }; }
    ...
    void SampleLayerPoses(AnimationLayer& layer, AnimationGraphLayerRuntime& rt) {
        ...
        if (rt.isTransitioning && rt.previousStateIndex >= 0) {
            ...
            if (prevState != nullptr && prevState->Node() != nullptr) {
                prevState->Node()->Evaluate(*m_skeleton, rt.previousTime, Span<BoneTransform>{ rt.prevPoses.Data(), rt.prevPoses.Size() });
                currentState->Node()->Evaluate(*m_skeleton, rt.currentTime, Span<BoneTransform>{ rt.poses.Data(), rt.poses.Size() });
                const f32 blend = Clamp(rt.transitionElapsed / rt.transitionDuration, 0.0f, 1.0f);
    ...
    void CombineLayers() {
        if (m_layerRuntimes.IsEmpty()) { return; }
        ...
        for (usize layerIdx = 1; layerIdx < m_layerRuntimes.Size() && layerIdx < m_graph->Layers().Size(); ++layerIdx) {
            ...
            if (layer.blendMode == LayerBlendMode::Override) {
                ...
                for (usize b = 0; b < m_finalPoses.Size() && b < rt.poses.Size(); ++b) {
                    ...
                    const Vec3 deltaPos   = rt.poses[b].position - bind.position;
                    const Quat deltaRot   = rt.poses[b].rotation * Inverse(bind.rotation);
                    const Vec3 deltaScale = rt.poses[b].scale / bind.scale;   // component-wise
                    m_finalPoses[b].position = m_finalPoses[b].position + deltaPos * w;
                    m_finalPoses[b].rotation = Slerp(Quat::Identity, deltaRot, w) * m_finalPoses[b].rotation;
                    m_finalPoses[b].scale    = m_finalPoses[b].scale * Lerp(Vec3::One, deltaScale, w);
                }
            }
        }
    }
