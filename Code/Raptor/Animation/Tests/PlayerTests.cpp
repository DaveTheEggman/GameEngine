...
TEST_CASE("player: events fire in order across updates")
{
    ...
    clip.SortEvents();
    player.Play(&clip);
    player.Update(0.5f); CHECK(fired.Size() == 1); CHECK(fired[0] == StringView{ u8"A" });
    player.Update(0.5f); CHECK(fired.Size() == 2); CHECK(fired[1] == StringView{ u8"B" });
    player.Update(0.5f); CHECK(fired.Size() == 3); CHECK(fired[2] == StringView{ u8"C" });
}
