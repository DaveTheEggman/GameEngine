/// Raptor::AnimationSubsystem — the `:components` partition.
///
...
/// scene tick.
///
/// It sits at the animation<->render seam (depends on both raptor.animation and the render
/// components); neither of those depends back on it.

module;
#include "Core/Prelude.h"

export module raptor.animation.subsystem:components;

import raptor.core;
import raptor.scene;
import raptor.animation;          // Skeleton, AnimationClip, AnimationPlayer, AnimationGraph(+Player)
import raptor.render.subsystem;   // MeshComponentManager / MeshComponent (the feed target)
