/// Raptor::Animation — the `:clip` partition.
///
...
public:
    ...
    void AddKeyframe(f32 time, const T& value) { m_keyframes.PushBack(Keyframe<T>{ time, value }); }
...
public:
    ...
    [[nodiscard]] const Array<UniquePtr<Vec3Track>>& PositionTracks() const noexcept { return m_positionTracks; }
    [[nodiscard]] const Array<UniquePtr<QuatTrack>>& RotationTracks() const noexcept { return m_rotationTracks; }
    [[nodiscard]] const Array<UniquePtr<Vec3Track>>& ScaleTracks()    const noexcept { return m_scaleTracks; }
    ...
    void FireEvents(f32 prevTime, f32 currentTime, Handler&& handler) const {
        if (m_events.IsEmpty() || duration <= 0.0f) { return; }

        if (currentTime > prevTime && currentTime <= duration) {
            for (const AnimationEvent& e : m_events) {
                if (e.time > prevTime && e.time <= currentTime) { handler(StringView{ e.name }, e.time); }
            }
        } else if (currentTime > duration) {
            if (isLooping) {
                for (const AnimationEvent& e : m_events) {
                    if (e.time > prevTime && e.time <= duration) { handler(StringView{ e.name }, e.time); }
                }
                f32 wrapped = currentTime;
                while (wrapped >= duration) { wrapped -= duration; }
                for (const AnimationEvent& e : m_events) {
                    if (e.time <= wrapped) { handler(StringView{ e.name }, e.time); }
                }
            } else {
                for (const AnimationEvent& e : m_events) {
                    if (e.time > prevTime && e.time <= duration) { handler(StringView{ e.name }, e.time); }
    ...
    void ComputeDuration() {
        duration = 0.0f;
        for (auto& t : m_positionTracks) { if (!t->Keyframes().IsEmpty()) { duration = Max(duration, t->Keyframes()[t->Keyframes().Size() - 1].time); } }
        for (auto& t : m_rotationTracks) { if (!t->Keyframes().IsEmpty()) { duration = Max(duration, t->Keyframes()[t->Keyframes().Size() - 1].time); } }
        for (auto& t : m_scaleTracks)    { if (!t->Keyframes().IsEmpty()) { duration = Max(duration, t->Keyframes()[t->Keyframes().Size() - 1].time); } }
    }
