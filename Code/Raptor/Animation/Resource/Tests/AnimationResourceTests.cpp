...
namespace
{
    void RemoveTree()
    {
        FileDelete(u8"raptor_anim_res_db/skel.rasset");
        FileDelete(u8"raptor_anim_res_db/clip.rasset");
        RemoveDirectory(u8"raptor_anim_res_db");
    }
...
TEST_CASE("skeleton resource: round-trips through the resource manager")
{
    ...
    RemoveTree();
    NativeFileSystem mount(u8"raptor_anim_res_db");
...
TEST_CASE("animation graph resource: composite — resolves clip refs through the manager")
{
    ...
    FileDelete(u8"raptor_anim_res_db/walk.rasset");
    FileDelete(u8"raptor_anim_res_db/graph.rasset");
    RemoveDirectory(u8"raptor_anim_res_db");
    NativeFileSystem mount(u8"raptor_anim_res_db");
