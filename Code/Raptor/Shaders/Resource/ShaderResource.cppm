/// Raptor::ShaderResource — the `raptor.shaders.resource` module.
///
...
export module raptor.shaders.resource;
...
{
    RAPTOR_OBJECT(ShaderSource, ISerializable)
public:
    String name;
    String vertexSource;
    ...
    void Serialize(ISerializer& ar) override
    {
        ...
        raptor::core::Serialize(ar, "vertexSource", vertexSource);
        raptor::core::Serialize(ar, "fragmentSource", fragmentSource);
    }
};
...
{
    RAPTOR_OBJECT(ShaderResource, Object)
public:
    void Init(ShaderSystem* system, StringView name) { m_system = system; m_name = String(name); }

    [[nodiscard]] StringView Name() const noexcept { return m_name.AsView(); }
    [[nodiscard]] u64 Version() { return (m_system != nullptr) ? m_system->Version(m_name.AsView()) : 0ull; }
    [[nodiscard]] rhi::ShaderModule* GetVariant(ShaderStage stage, ShaderFlags flags)
    {
        return (m_system != nullptr) ? m_system->GetVariant(m_name.AsView(), stage, flags) : nullptr;
    }
...
public:
    ...
    [[nodiscard]] const TypeInfo* ProductType() const override { return &ShaderResource::StaticType(); }
    ...
    [[nodiscard]] RefPtr<Object> Create(ResourceManager& manager, raptor::content::Instance& instance) override
    {
        ...
        m_system->RegisterSource(source->name.AsView(), ShaderStage::Vertex,   source->vertexSource.AsView());
        m_system->RegisterSource(source->name.AsView(), ShaderStage::Fragment, source->fragmentSource.AsView());
...
RAPTOR_DEFINE_OBJECT(ShaderSource, "raptor::shaders")
RAPTOR_DEFINE_OBJECT(ShaderResource, "raptor::shaders")
