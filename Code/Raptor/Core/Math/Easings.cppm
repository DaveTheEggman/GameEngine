...
export module raptor.core:easings;
...
export namespace raptor::core
{
    ...
    [[nodiscard]] inline f32 EaseInOutExponential(f32 t) noexcept
    {
        ...
        if (t == 1.0f) { return 1.0f; }
        t *= 2.0f;
        if (t < 1.0f) { return 0.5f * Pow(2.0f, 10.0f * (t - 1.0f)); }
        t -= 1.0f;
        return 0.5f * (-Pow(2.0f, -10.0f * t) + 2.0f);
    }

    // --- circular ---
    [[nodiscard]] inline f32 EaseInCircular(f32 t) noexcept { return -1.0f * (Sqrt(1.0f - t * t) - 1.0f); }
    [[nodiscard]] inline f32 EaseOutCircular(f32 t) noexcept { t -= 1.0f; return Sqrt(1.0f - t * t); }
    [[nodiscard]] inline f32 EaseInOutCircular(f32 t) noexcept
    {
        t *= 2.0f;
        if (t < 1.0f) { return -0.5f * (Sqrt(1.0f - t * t) - 1.0f); }
        t -= 2.0f;
