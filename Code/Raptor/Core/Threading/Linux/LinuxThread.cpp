// Raptor Core — Threading backend, Linux (pthreads) implementation.
...
namespace raptor::core::sys
{
    ...
    namespace
    {
        ...
        pthread_mutex_t* AsMutex(void* storage) noexcept { return static_cast<pthread_mutex_t*>(storage); }
        pthread_cond_t* AsCond(void* storage) noexcept { return static_cast<pthread_cond_t*>(storage); }
    }
    ...
    bool MutexTryLock(void* storage) noexcept { return pthread_mutex_trylock(AsMutex(storage)) == 0; }
    ...
    void CondWait(void* cond, void* mutex) noexcept { pthread_cond_wait(AsCond(cond), AsMutex(mutex)); }
