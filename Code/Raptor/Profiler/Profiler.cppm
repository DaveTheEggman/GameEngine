/// Raptor::Profiler — `raptor.profiler`.
///
...
/// Instrument with the RAPTOR_PROFILE_SCOPE macro (Profiler.h) — it compiles to nothing when
/// RAPTOR_PROFILING is off. Thread-safe: scopes touch only thread-local state; the registry +
/// frame swap are mutex-guarded, and the merge runs at frame end when workers are idle.

module;
#include "Core/Prelude.h"

export module raptor.profiler;

import raptor.core;

using namespace raptor::core;

export namespace raptor::profiler {
...
public:
    ...
    void EndFrame() noexcept {
        ...
        for (ThreadData* td : m_threads) {
            for (const ProfileSample& s : td->samples) { m_completed.samples.PushBack(s); }
            td->samples.Clear();
    ...
    [[nodiscard]] String BuildReport() const {
        ...
        Array<u32> order;
        for (u32 i = 0; i < static_cast<u32>(f.samples.Size()); ++i) { order.PushBack(i); }
...
private:
    Profiler() = default;
    ~Profiler() { for (ThreadData* td : m_threads) { DefaultAllocator().Delete(td); } }
...
// RAII scope: brackets a profiled region. Use via RAPTOR_PROFILE_SCOPE (Profiler.h).
class ScopedProfile {
public:
