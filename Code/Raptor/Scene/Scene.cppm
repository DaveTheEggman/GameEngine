...
public:
    ...
    void SetLocalPosition(EntityHandle entity, Vec3 position) {
        if (!IsValid(entity)) { return; }
    ...
    [[nodiscard]] Mat4 GetWorldMatrix(EntityHandle entity) const {
        return IsValid(entity) ? m_transforms[entity.index].worldMatrix : Mat4::Identity();
    }
    [[nodiscard]] Mat4 GetPrevWorldMatrix(EntityHandle entity) const {
        return IsValid(entity) ? m_transforms[entity.index].prevWorldMatrix : Mat4::Identity();
    }
    // Translation row of the world matrix (row-vector convention).
    [[nodiscard]] Vec3 GetWorldPosition(EntityHandle entity) const {
        const Mat4 w = GetWorldMatrix(entity);
        return Vec3{ w.m[3][0], w.m[3][1], w.m[3][2] };
    }
    ...
    void UpdateTransforms() {
        ...
        for (u32 i = 0; i < count; ++i) {
            if (m_transforms[i].dirty && m_entities[i].alive && !m_transforms[i].parent.IsAssigned()) {
                UpdateTransformRecursive(i, Mat4::Identity());
            }
        }
    }
...
protected:
    ...
    struct TransformData {
        ...
        Mat4         prevWorldMatrix = Mat4::Identity();
    ...
    void UpdateTransformRecursive(u32 index, const Mat4& parentWorld) {
        {
        ...
        const Mat4 myWorld = m_transforms[index].worldMatrix;
        EntityHandle child = m_transforms[index].firstChild;
        while (child.IsAssigned() && IsValid(child)) {
