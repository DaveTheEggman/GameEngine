...
import raptor.scene;
import raptor.scene.resource;
import raptor.scene.editor;
...
namespace
{
    ...
    void Serialize(ISerializer& ar, Tag& t) { raptor::core::Serialize(ar, "team", t.team); }
    ...
    void RemoveTree()
    {
        FileDelete(u8"raptor_scene_db/level.rasset");
        FileDelete(u8"raptor_scene_db/level.scene.bin");
        RemoveDirectory(u8"raptor_scene_db");
    }
}
...
TEST_CASE("SaveScene -> content DB -> LoadScene round-trips a scene")
{
    ...
    RemoveTree();
    NativeFileSystem mount(u8"raptor_scene_db");
