// Raptor::RenderGraph — :validator partition
//
...
export module raptor.rendergraph:validator;
    ...
    private:
        ...
        static void CheckRedundantWrites(RenderGraph& graph, Array<ValidationMessage>& out)
        {
            ...
            for (RenderGraphPass* pass : graph.Passes())
            {
                for (const RGResourceAccess& access : pass->accesses)
                {
                    if (access.IsRead() && access.handle.IsValid()) { lastWriter.Remove(access.handle.index); }
                }
                for (const RGResourceAccess& access : pass->accesses)
                {
