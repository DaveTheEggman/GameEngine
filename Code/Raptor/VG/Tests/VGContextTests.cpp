...
TEST_CASE("vg.context: transform is baked into emitted vertices")
{
    ...
    ctx.FillRect(Rect{ 0, 0, 10, 10 }, Color32::Green);
...
TEST_CASE("vg.context: opacity scales vertex alpha")
{
    ...
    ctx.FillRect(Rect{ 0, 0, 10, 10 }, Color32{ 255, 255, 255, 255 });
    ctx.PopOpacity();
...
TEST_CASE("vg.context: immediate-mode path fill")
{
    ...
    ctx.LineTo(10, 10);
    ctx.ClosePath();
    ctx.Fill(Color32::Blue, FillRule::NonZero, /*antiAlias*/ false);
...
TEST_CASE("vg.context: DrawImage registers the texture and switches command")
{
    ...
    ctx.FillRect(Rect{ 0, 0, 5, 5 }, Color32::Red); // solid command (tex 0)
...
{
    VGContext ctx;
    ctx.FillRect(Rect{ 0, 0, 5, 5 }, Color32::Red);
    ctx.Clear();
    VGBatch& batch = ctx.GetBatch();
    CHECK(batch.VertexCount() == 0u);
