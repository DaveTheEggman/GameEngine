...
export namespace raptor::vg
{
    ...
    public:
        ...
        [[nodiscard]] virtual Color GetColorAt(Vec2 position, Rect bounds) const = 0;
    ...
    public:
        ...
        [[nodiscard]] Color GetColorAt(Vec2 /*position*/, Rect /*bounds*/) const override { return m_color; }
    ...
    {
    public:
        Vec2 startPoint; ///< Start point of the gradient line.
        Vec2 endPoint;   ///< End point of the gradient line.
        Array<GradientStop> stops; ///< Color stops defining the gradient.

        VGLinearGradientFill() = default;
        VGLinearGradientFill(Vec2 inStart, Vec2 inEnd) : startPoint(inStart), endPoint(inEnd) {}

        /// Add a color stop.
        void AddStop(f32 offset, Color color) { stops.PushBack(GradientStop(offset, color)); }

        [[nodiscard]] Color GetColorAt(Vec2 position, Rect /*bounds*/) const override
        {
            const Vec2 gradientDir = endPoint - startPoint;
            const f32 gradientLenSq = gradientDir.x * gradientDir.x + gradientDir.y * gradientDir.y;
            if (gradientLenSq < 0.0001f)
                return BaseColor();

            const Vec2 toPoint = position - startPoint;
    ...
    {
    public:
        Vec2 center;  ///< Center of the gradient.
        f32 radius = 0.0f; ///< Radius of the gradient.
        Array<GradientStop> stops; ///< Color stops defining the gradient.

        VGRadialGradientFill() = default;
        VGRadialGradientFill(Vec2 inCenter, f32 inRadius) : center(inCenter), radius(inRadius) {}
    ...
    public:
        ...
        explicit VGConicGradientFill(Vec2 inCenter, f32 inStartAngle = 0.0f) : center(inCenter), startAngle(inStartAngle) {}
