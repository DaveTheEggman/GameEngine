...
export module raptor.vg:curves;
...
export namespace raptor::vg
{
    ...
    public:
        ...
        [[nodiscard]] static Vec2 QuadraticPointAt(Vec2 p0, Vec2 p1, Vec2 p2, f32 t)
        {
            const f32 mt = 1.0f - t;
        ...
        [[nodiscard]] static Vec2 CubicPointAt(Vec2 p0, Vec2 p1, Vec2 p2, Vec2 p3, f32 t)
        {
            const f32 mt = 1.0f - t;
            const f32 mt2 = mt * mt;
            const f32 t2 = t * t;
        ...
        [[nodiscard]] static Vec2 QuadraticTangentAt(Vec2 p0, Vec2 p1, Vec2 p2, f32 t)
        {
            const f32 mt = 1.0f - t;
            const Vec2 tangent = (p1 - p0) * (2.0f * mt) + (p2 - p1) * (2.0f * t);
        ...
        [[nodiscard]] static Vec2 CubicTangentAt(Vec2 p0, Vec2 p1, Vec2 p2, Vec2 p3, f32 t)
        {
            const f32 mt = 1.0f - t;
            const f32 mt2 = mt * mt;
            const f32 t2 = t * t;
            const Vec2 tangent = (p1 - p0) * (3.0f * mt2) + (p2 - p1) * (6.0f * mt * t) + (p3 - p2) * (3.0f * t2);
            const f32 len = Length(tangent);
            if (len > 0.0001f)
                return tangent / len;
            return Vec2{ 1.0f, 0.0f };
        ...
        [[nodiscard]] static f32 QuadraticLength(Vec2 p0, Vec2 p1, Vec2 p2, i32 steps = 16)
        {
            f32 length = 0.0f;
            Vec2 prev = p0;
            for (i32 i = 1; i <= steps; ++i)
            {
                ...
                const Vec2 next = QuadraticPointAt(p0, p1, p2, t);
        ...
        [[nodiscard]] static f32 CubicLength(Vec2 p0, Vec2 p1, Vec2 p2, Vec2 p3, i32 steps = 16)
        {
            f32 length = 0.0f;
            Vec2 prev = p0;
            for (i32 i = 1; i <= steps; ++i)
            {
                const f32 t = static_cast<f32>(i) / static_cast<f32>(steps);
                const Vec2 next = CubicPointAt(p0, p1, p2, p3, t);
                length += Distance(prev, next);
                prev = next;
            }
            return length;
        ...
        static void FlattenQuadratic(Vec2 p0, Vec2 p1, Vec2 p2, f32 tolerance, Array<Vec2>& output)
        {
        ...
        static void FlattenCubic(Vec2 p0, Vec2 p1, Vec2 p2, Vec2 p3, f32 tolerance, Array<Vec2>& output)
        {
        ...
        static void ArcToCubics(Vec2 from, f32 rx, f32 ry, f32 xAxisRotation, bool largeArc, bool sweep, Vec2 to, Array<Vec2>& controlPoints)
        {
    ...
    private:
        static void FlattenQuadraticRecursive(Vec2 p0, Vec2 p1, Vec2 p2, f32 toleranceSq, i32 depth, Array<Vec2>& output)
        {
            if (depth > 16)
            {
                output.PushBack(p0);
            ...
            const Vec2 mid = (p0 + p2) * 0.5f;
            const Vec2 deviation = p1 - mid;
        ...
        static void FlattenCubicRecursive(Vec2 p0, Vec2 p1, Vec2 p2, Vec2 p3, f32 toleranceSq, i32 depth, Array<Vec2>& output)
        {
            if (depth > 16)
            {
                output.PushBack(p0);
        ...
        [[nodiscard]] static f32 PointToLineDistanceSq(Vec2 point, Vec2 lineStart, Vec2 lineEnd)
        {
        ...
        static void ArcSegmentToCubic(f32 cx, f32 cy, f32 rx, f32 ry, f32 phi, f32 a1, f32 a2, Array<Vec2>& controlPoints)
        {
            ...
            controlPoints.PushBack(Vec2{ x1 + alpha * dx1, y1 + alpha * dy1 }); // CP1
            controlPoints.PushBack(Vec2{ x4 - alpha * dx2, y4 - alpha * dy2 }); // CP2
            controlPoints.PushBack(Vec2{ x4, y4 });                            // End point
        }
    };
}
