...
export module raptor.model:model_mesh;
...
public:
    ...
    void setVertexData(const T* data, usize count) {
        ...
        if (m_vertexData.IsEmpty() || bytes > m_vertexData.Size()) return;
    ...
    void setIndexData(const u16* indices, usize count) {
        usize bytes = count * 2;
        if (m_indexData.IsEmpty() || m_use32BitIndices || bytes > m_indexData.Size()) return;
    ...
    void addUniformSkinning(i32 jointIndex) {
        ...
        if (m_vertexData.IsEmpty() || m_vertexCount == 0) return;
        ...
        m_vertexElements.PushBack(VertexElement(VertexSemantic::Joints, VertexElementFormat::UShort4, oldStride));
        m_vertexElements.PushBack(VertexElement(VertexSemantic::Weights, VertexElementFormat::Float4, oldStride + 8));
    }
    ...
    void scalePositions(Vec3 scale) {
        ...
        if (posOffset < 0 || m_vertexData.IsEmpty()) return;
    ...
    void remapJointIndices(const i32* remap, usize remapCount) {
        ...
        if (jointsOffset < 0 || m_vertexData.IsEmpty()) return;
