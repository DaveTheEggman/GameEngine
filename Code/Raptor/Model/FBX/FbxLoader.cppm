...
export module raptor.model.fbx;
...
export namespace raptor::model::fbx {
...
private:
    ...
    static TexturePixelFormat convertPixelFormat(raptor::image::PixelFormat fmt) {
        switch (fmt) {
        case raptor::image::PixelFormat::R8:    return TexturePixelFormat::R8;
        case raptor::image::PixelFormat::RG8:   return TexturePixelFormat::RG8;
        case raptor::image::PixelFormat::RGB8:  return TexturePixelFormat::RGB8;
        case raptor::image::PixelFormat::RGBA8: return TexturePixelFormat::RGBA8;
        case raptor::image::PixelFormat::BGR8:  return TexturePixelFormat::BGR8;
        case raptor::image::PixelFormat::BGRA8: return TexturePixelFormat::BGRA8;
        default: return TexturePixelFormat::Unknown;
        }
    }

    static void storeImageData(const raptor::image::Image& image, ModelTexture* texture) {
    ...
    void loadMeshes(Model& model) {
        for (size_t i = 0; i < m_scene->meshes.count; ++i) {
            ...
            stride += static_cast<i32>(sizeof(Vec3));
            ...
            stride += static_cast<i32>(sizeof(Vec2));
            ...
            if (isSkinned) {
                ...
                stride += static_cast<i32>(sizeof(Vec4));
        ...
        Array<u8>& vertexBytes, HashMap<size_t, i32>& vtxMap)
    {
        ...
        // Position.
        *reinterpret_cast<Vec3*>(vertex + positionOffset) =
            Vec3(static_cast<f32>(pos.x), static_cast<f32>(pos.y), static_cast<f32>(pos.z));

        // Normal.
        *reinterpret_cast<Vec3*>(vertex + normalOffset) =
            Vec3(static_cast<f32>(normal.x), static_cast<f32>(normal.y), static_cast<f32>(normal.z));

        // TexCoord.
        *reinterpret_cast<Vec2*>(vertex + texCoordOffset) =
            Vec2(static_cast<f32>(uv.x), static_cast<f32>(uv.y));
        ...
        *reinterpret_cast<u32*>(vertex + colorOffset) =
        ...
        // Tangent.
        *reinterpret_cast<Vec3*>(vertex + tangentOffset) =
            Vec3(static_cast<f32>(tangent.x), static_cast<f32>(tangent.y), static_cast<f32>(tangent.z));
        ...
        if (isSkinned) {
            ...
            *reinterpret_cast<Vec4*>(vertex + weightsOffset) =
                Vec4(weights[0], weights[1], weights[2], weights[3]);
        }
    ...
    void loadSkins(Model& model) {
        for (size_t i = 0; i < m_scene->skin_deformers.count; ++i) {
            ...
            for (size_t j = 0; j < skinDef->clusters.count; ++j) {
                ...
                Mat4 ibm = convertMatrix(cluster->geometry_to_bone);
                ...
                if (bakedNode->translation_keys.count > 0 && !bakedNode->constant_translation) {
                    ...
                    for (size_t ki = 0; ki < bakedNode->translation_keys.count; ++ki) {
                        ...
                        channel->addKeyframe(static_cast<f32>(key->time),
                            Vec4(static_cast<f32>(key->value.x),
    ...
    /// Converts a ufbx 3x4 matrix to our 4x4 Mat4.
    /// ufbx column -> Mat4 row (same pattern as GltfLoader's column-major transpose).
    static Mat4 convertMatrix(ufbx_matrix m) {
        return Mat4{{
