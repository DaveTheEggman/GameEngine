...
export module raptor.model.gltf;
...
export namespace raptor::model::gltf {
...
private:
    ...
    bool loadImageFromDataUri(const char* dataUri, raptor::image::Image& outImage) {
        namespace iio = raptor::image::io;
    ...
    void loadMeshPrimitives(cgltf_mesh* meshData, ModelMesh* mesh) {
        ...
        for (cgltf_size p = 0; p < meshData->primitives_count; ++p) {
            ...
            for (i32 v = 0; v < primVertexCount; ++v) {
                ...
                *reinterpret_cast<Vec3*>(vertex + positionOffset) = Vec3(pos[0], pos[1], pos[2]);

                // Normal.
                if (normalAccessor) {
                    float normal[3] = {};
                    cgltf_accessor_read_float(normalAccessor, static_cast<cgltf_size>(v), normal, 3);
                    *reinterpret_cast<Vec3*>(vertex + normalOffset) = Vec3(normal[0], normal[1], normal[2]);
                } else {
                    *reinterpret_cast<Vec3*>(vertex + normalOffset) = Vec3(0, 1, 0);
                }
                ...
                if (texCoordAccessor) {
                    ...
                    *reinterpret_cast<Vec2*>(vertex + texCoordOffset) = Vec2(uv[0], uv[1]);
                }
                ...
                if (tangentAccessor) {
                    ...
                    *reinterpret_cast<Vec3*>(vertex + tangentOffset) = Vec3(tangent[0], tangent[1], tangent[2]);
                } else {
                    *reinterpret_cast<Vec3*>(vertex + tangentOffset) = Vec3(1, 0, 0);
                }
                ...
                if (isSkinned && jointsAccessor && weightsAccessor) {
                    ...
                    *reinterpret_cast<Vec4*>(vertex + weightsOffset) = Vec4(weights[0], weights[1], weights[2], weights[3]);
                }
            }
    ...
    void loadNodes(Model& model) {
        ...
        for (cgltf_size i = 0; i < m_data->nodes_count; ++i) {
            ...
            if (node->has_translation)
                bone->translation = Vec3(node->translation[0], node->translation[1], node->translation[2]);
            ...
            // Scale.
            if (node->has_scale)
                bone->scale = Vec3(node->scale[0], node->scale[1], node->scale[2]);
