/// Raptor::MeshResource — the `raptor.geometry.resource` module.
///
...
export module raptor.geometry.resource;
...
export namespace raptor::geometry {
...
class StaticMeshSource : public ISerializable {
    RAPTOR_OBJECT(StaticMeshSource, ISerializable)
public:
    String     name;
    ...
    static void FromMesh(const StaticMesh& mesh, StaticMeshSource& out) {
        ...
        if (mesh.VertexDataSize() > 0) { MemCopy(out.vertexBlob.Data(), mesh.VertexData(), mesh.VertexDataSize()); }
        out.indexData.Clear();
        out.indexData.Reserve(mesh.IndexCount());
        for (u32 i = 0; i < mesh.IndexCount(); ++i) { out.indexData.PushBack(mesh.indices.Get(i)); }
    ...
    void FillStatic(StaticMesh& mesh) const {
        ...
        if (vcount > 0) { MemCopy(mesh.vertices.Data(), vertexBlob.Data(), vcount * sizeof(StaticMeshVertex)); }
        mesh.indices.Resize(static_cast<u32>(indexData.Size()));
        for (usize i = 0; i < indexData.Size(); ++i) { mesh.indices.Set(static_cast<u32>(i), indexData[i]); }
        mesh.subMeshes.Clear();
        for (usize i = 0; i < subStart.Size(); ++i) {
            mesh.subMeshes.PushBack(SubMesh{ subStart[i], subCount[i],
                                             (i < subMaterial.Size() ? subMaterial[i] : 0),
...
protected:
    ...
    void SerializeStatic(ISerializer& ar) {
        ...
        raptor::core::Serialize(ar, "vertexBlob", vertexBlob);
        raptor::core::Serialize(ar, "indexData", indexData);
        raptor::core::Serialize(ar, "subStart", subStart);
        raptor::core::Serialize(ar, "subCount", subCount);
        raptor::core::Serialize(ar, "subMaterial", subMaterial);
        raptor::core::Serialize(ar, "subPrim", subPrim);
    }
};
...
class SkinnedMeshSource final : public StaticMeshSource {
    RAPTOR_OBJECT(SkinnedMeshSource, StaticMeshSource)
public:
    ...
    void Serialize(ISerializer& ar) override {
        SerializeStatic(ar);
        raptor::core::Serialize(ar, "skinningBlob", skinningBlob);
        raptor::core::Serialize(ar, "skeletonIndex", skeletonIndex);
    }
    ...
    static void FromMesh(const SkinnedMesh& mesh, SkinnedMeshSource& out) {
        ...
        if (mesh.SkinningDataSize() > 0) { MemCopy(out.skinningBlob.Data(), mesh.SkinningData(), mesh.SkinningDataSize()); }
    ...
    void FillSkinned(SkinnedMesh& mesh) const {
        ...
        if (scount > 0) { MemCopy(mesh.skinning.Data(), skinningBlob.Data(), scount * sizeof(VertexSkinning)); }
...
public:
    [[nodiscard]] const TypeInfo* ProductType() const override { return &StaticMesh::StaticType(); }
...
public:
    [[nodiscard]] const TypeInfo* ProductType() const override { return &SkinnedMesh::StaticType(); }
    [[nodiscard]] RefPtr<Object> Create(ResourceManager& manager, raptor::content::Instance& instance) override {
        (void)manager;
        RefPtr<ISerializable> object = instance.ReadObject();
...
RAPTOR_DEFINE_OBJECT(StaticMeshSource, "raptor::geometry")
RAPTOR_DEFINE_OBJECT(SkinnedMeshSource, "raptor::geometry")

} // namespace raptor::geometry
