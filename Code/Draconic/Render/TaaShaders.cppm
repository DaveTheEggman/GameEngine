/// Draconic::Render - the `:taa_shaders` partition.
///
/// HLSL source for the TAA resolve pass, split out from the pass logic (TaaPass.cppm) so the shader text
/// is isolated and trivial to lift into standalone .hlsl files later. Pass-internal: imported by `:taa`,
/// which registers these with the ShaderSystem. Mirrors the draconic.vg.renderer:shaders pattern.

module;
#include "Core/Prelude.h"

export module draconic.render:taa_shaders;
