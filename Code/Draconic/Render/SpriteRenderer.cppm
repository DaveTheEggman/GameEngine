...
struct SpriteInstance {
    Vec4 positionSize;      // xyz world center, w width
    Vec4 sizeOrientation;   // x height, y orientation mode
    Vec4 tint;
...
public:
    ...
    void Resolve(const RenderRecordContext& ctx, Span<const DrawItem> items, Array<ResolvedDraw>& out) override {
        ...
        if (!vr.ok) { return; }
        struct ViewUBO { Mat4 viewProj; Mat4 view; } ubo{ ctx.viewProj, ctx.viewMatrix };
        ...
        while (i < items.Size()) {
            ...
            if (ir.ok) {
                ...
                for (usize k = i; k < j; ++k) {
                    ...
                    si.positionSize    = Vec4{ s->worldCenter.x, s->worldCenter.y, s->worldCenter.z, s->size.x };
                    si.sizeOrientation = Vec4{ s->size.y, static_cast<f32>(s->orientation), 0.0f, 0.0f };
                    si.tint            = Vec4{ s->tint.r, s->tint.g, s->tint.b, s->tint.a };
                    si.uvRect          = s->uvRect;
                }
