...
TEST_CASE("SelectPose: Hashed is deterministic, in range, and NOT the identity/linear map")
{
    constexpr u32 M = 32;
    bool bucketHit[M] = {};
    u32  matchesModulo = 0;   // how often Hash(i)%M coincides with i%M (a linear map would be ALL of them)
    for (u32 i = 0; i < 4096; ++i) {
        const u32 p = SelectPose(PoseAssignment::Hashed, i, M, nullptr);
        CHECK(p < M);                                                     // in range
        CHECK(p == SelectPose(PoseAssignment::Hashed, i, M, nullptr));    // deterministic (stable per i -> motion blur safe)
        CHECK(p == static_cast<u32>(HashInteger(i) % M));                 // it IS the splitmix finalizer
        bucketHit[p] = true;
        if (p == i % M) { ++matchesModulo; }
    }
    for (u32 b = 0; b < M; ++b) { CHECK(bucketHit[b]); }   // every pose bucket used (good spread)
...
TEST_CASE("ColumnPose: constant down a column, steps across columns")
{
    ...
    for (u32 col = 0; col < 50; ++col) {
        ...
        CHECK(p == col % M);
        for (u32 row = 0; row < 20; ++row) { CHECK(ColumnPose(col, M) == p); }
    }
    // Adjacent columns differ while col < M (no aliasing until the modulo wraps).
    for (u32 col = 0; col + 1 < M; ++col) { CHECK(ColumnPose(col, M) != ColumnPose(col + 1, M)); }
}
