/// Draconic::Render - the `:tonemap_shaders` partition.
///
/// HLSL source for the tonemap pass, split out from the pass logic (TonemapPass.cppm) so the shader text
/// is isolated and trivial to lift into standalone .hlsl files later. Pass-internal: imported by
/// `:tonemap`, which registers these with the ShaderSystem. Mirrors the draconic.vg.renderer:shaders pattern.

module;
#include "Core/Prelude.h"

export module draconic.render:tonemap_shaders;
