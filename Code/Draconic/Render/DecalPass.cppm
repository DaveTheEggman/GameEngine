...
struct DecalUniforms {
    Mat4 world;         // decal box world transform (scale = box size); projects along local +Z
    Mat4 invWorld;      // world -> decal-local (the box clip)
    Mat4 invViewProj;   // (ndc, depth) -> world, for depth reconstruction (this view, jittered)
    Vec4 color;         // rgba tint
    Vec4 params;        // x,y = 1/full-target-size ; z,w = cos(angleFadeStart), cos(angleFadeEnd)
};
...
public:
    ...
    void DeclareDecals(rendergraph::RenderGraph& graph, rendergraph::RGHandle hdr, rendergraph::RGHandle depth,
                       Span<const DecalInstance> decals, const Mat4& viewProj, u32 w, u32 h,
        ...
        const Vec2 invSize{ 1.0f / static_cast<f32>(w), 1.0f / static_cast<f32>(h) };
        ...
        for (const DecalInstance& d : decals) {
            ...
            u.color       = Vec4{ d.color.r, d.color.g, d.color.b, d.color.a };
            u.params      = Vec4{ invSize.x, invSize.y, Cos(d.fadeStart), Cos(d.fadeEnd) };
