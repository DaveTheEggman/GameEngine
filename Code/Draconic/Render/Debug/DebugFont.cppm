...
inline Array<u8> GenerateTextureData() {
    ...
    for (i32 charIdx = 0; charIdx < kCharCount; ++charIdx) {
        ...
        for (i32 row = 0; row < kCharHeight; ++row) {
            ...
            for (i32 col = 0; col < kCharWidth; ++col) {
                ...
                data[static_cast<usize>((baseY + row) * kTextureWidth + (baseX + col))] = bit ? 255u : 0u;
            }
        }
    }
    ...
    for (i32 row = 0; row < kCharHeight; ++row) {
        for (i32 col = 0; col < kCharWidth; ++col) {
            ...
            if (px < kTextureWidth && py < kTextureHeight) { data[static_cast<usize>(py * kTextureWidth + px)] = 255u; }
        }
    }
    return data;
}
...
inline bool GetCharUV(u32 c, f32& u0, f32& v0, f32& u1, f32& v1) noexcept {
    u0 = v0 = u1 = v1 = 0.0f;
    if (static_cast<i32>(c) < kFirstChar || static_cast<i32>(c) > kLastChar) { return false; }
