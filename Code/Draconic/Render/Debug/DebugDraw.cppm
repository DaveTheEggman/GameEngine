...
struct Debug2DCommand {
    ...
    Float2 position{};     // pixels, top-left origin (negative x = right-aligned, see DrawScreenTextRight)
...
public:
    ...
    void DrawLineOverlay(Float3 from, Float3 to, Color color) { DrawLine(from, to, color, true); }
    void DrawRay(Float3 origin, Float3 direction, Color color, bool overlay = false) { DrawLine(origin, origin + direction, color, overlay); }
    ...
    void DrawQuad(Float3 v0, Float3 v1, Float3 v2, Float3 v3, Color color, bool overlay = false) {
    ...
    void DrawFilledBox(Float3 mn, Float3 mx, Color color, bool overlay = false) {
        const Float3 v0{ mn.x, mn.y, mn.z }, v1{ mx.x, mn.y, mn.z }, v2{ mx.x, mn.y, mx.z }, v3{ mn.x, mn.y, mx.z };
        const Float3 v4{ mn.x, mx.y, mn.z }, v5{ mx.x, mx.y, mn.z }, v6{ mx.x, mx.y, mx.z }, v7{ mn.x, mx.y, mx.z };
    ...
    void DrawFilledBoxCenter(Float3 center, Float3 halfExtents, Color color, bool overlay = false) {
    ...
    void DrawWireBox(Float3 mn, Float3 mx, Color color, bool overlay = false) {
        const Float3 c000{ mn.x, mn.y, mn.z }, c100{ mx.x, mn.y, mn.z }, c010{ mn.x, mx.y, mn.z }, c110{ mx.x, mx.y, mn.z };
        const Float3 c001{ mn.x, mn.y, mx.z }, c101{ mx.x, mn.y, mx.z }, c011{ mn.x, mx.y, mx.z }, c111{ mx.x, mx.y, mx.z };
    ...
    void DrawWireBoxCenter(Float3 center, Float3 halfExtents, Color color, bool overlay = false) {
        DrawWireBox(center - halfExtents, center + halfExtents, color, overlay);
    }
    // Local AABB transformed by a world matrix (OBB).
    void DrawTransformedBox(Float3 mn, Float3 mx, const Float4x4& world, Color color, bool overlay = false) {
        Float3 c[8];
        c[0] = TransformPoint(world, Float3{ mn.x, mn.y, mn.z }); c[1] = TransformPoint(world, Float3{ mx.x, mn.y, mn.z });
        c[2] = TransformPoint(world, Float3{ mn.x, mx.y, mn.z }); c[3] = TransformPoint(world, Float3{ mx.x, mx.y, mn.z });
        c[4] = TransformPoint(world, Float3{ mn.x, mn.y, mx.z }); c[5] = TransformPoint(world, Float3{ mx.x, mn.y, mx.z });
        c[6] = TransformPoint(world, Float3{ mn.x, mx.y, mx.z }); c[7] = TransformPoint(world, Float3{ mx.x, mx.y, mx.z });
    ...
    void DrawCircle(Float3 center, Float3 u, Float3 v, f32 radius, Color color, i32 segments = 32, bool overlay = false) {
    ...
    void DrawCircleNormal(Float3 center, f32 radius, Float3 normal, Color color, i32 segments = 32, bool overlay = false) {
    ...
    void DrawWireSphere(Float3 center, f32 radius, Color color, i32 segments = 24, bool overlay = false) {
    ...
    void DrawWireSphereOverlay(Float3 center, f32 radius, Color color, i32 segments = 24) { DrawWireSphere(center, radius, color, segments, true); }
    void DrawCircleOverlay(Float3 center, Float3 u, Float3 v, f32 radius, Color color, i32 segments = 32) { DrawCircle(center, u, v, radius, color, segments, true); }
    // Wireframe capsule (cylinder body + two hemisphere caps).
    void DrawCapsule(Float3 center, f32 radius, f32 height, Color color, i32 segments = 16, bool overlay = false) {
        const f32 halfHeight = height * 0.5f - radius;
        const Float3 top = center + Float3{ 0, halfHeight, 0 }, bottom = center - Float3{ 0, halfHeight, 0 };
        ...
        for (i32 i = 0; i < segments; ++i) {   // vertical lines + the two end circles
            ...
            const Float3 o0{ Cos(a0) * radius, 0, Sin(a0) * radius }, o1{ Cos(a1) * radius, 0, Sin(a1) * radius };
        ...
        for (i32 i = 0; i < halfSeg; ++i) {     // hemisphere arcs (XY + ZY planes, top + bottom)
            ...
            DrawLine(top + Float3{ Sin(a0) * radius, Cos(a0) * radius, 0 }, top + Float3{ Sin(a1) * radius, Cos(a1) * radius, 0 }, color, overlay);
            DrawLine(top + Float3{ 0, Cos(a0) * radius, Sin(a0) * radius }, top + Float3{ 0, Cos(a1) * radius, Sin(a1) * radius }, color, overlay);
            DrawLine(bottom + Float3{ Sin(a0) * radius, -Cos(a0) * radius, 0 }, bottom + Float3{ Sin(a1) * radius, -Cos(a1) * radius, 0 }, color, overlay);
            DrawLine(bottom + Float3{ 0, -Cos(a0) * radius, Sin(a0) * radius }, bottom + Float3{ 0, -Cos(a1) * radius, Sin(a1) * radius }, color, overlay);
        }
    }
    ...
    void DrawArrow(Float3 start, Float3 end, Color color, f32 headSize = 0.1f, bool overlay = false) {
        DrawLine(start, end, color, overlay);
        const Float3 dir = Normalized(end - start);
        const Float3 perp1 = Normalized(Cross(dir, (Abs(dir.y) < 0.99f) ? Float3{ 0, 1, 0 } : Float3{ 1, 0, 0 }));
    ...
    void DrawCylinder(Float3 center, f32 radius, f32 height, Color color, i32 segments = 16, bool overlay = false) {
    ...
    void DrawCone(Float3 apex, Float3 direction, f32 length, f32 angle, Color color, i32 segments = 16, bool overlay = false) {
    ...
    void DrawFrustum(const Float4x4& invViewProj, Color color, bool overlay = false) {
        Float3 corners[8]; i32 idx = 0;
        for (i32 z = 0; z < 2; ++z) for (i32 y = 0; y < 2; ++y) for (i32 x = 0; x < 2; ++x) {
            corners[idx++] = TransformPoint(invViewProj, Float3{ x == 0 ? -1.0f : 1.0f, y == 0 ? -1.0f : 1.0f, static_cast<f32>(z) });
        }
    ...
    void DrawScreenText(f32 x, f32 y, StringView text, Color color, f32 scale = 1.0f) {
        ...
        const i32 start = static_cast<i32>(m_textChars.Size());
        AppendChars(text);
        m_2d.PushBack(Debug2DCommand{ Debug2DKind::Text, Float2{ x, y }, Float2{ 0, 0 }, color, start, static_cast<i32>(text.Size()), scale });
    }
    void DrawScreenTextRight(f32 rightMargin, f32 y, StringView text, Color color, f32 scale = 1.0f) {
        ...
        m_2d.PushBack(Debug2DCommand{ Debug2DKind::Text, Float2{ -(rightMargin + 1.0f), y }, Float2{ 0, 0 }, color, start, static_cast<i32>(text.Size()), scale });
    }
    void DrawScreenRect(f32 x, f32 y, f32 width, f32 height, Color color) {
        m_2d.PushBack(Debug2DCommand{ Debug2DKind::Rectangle, Float2{ x, y }, Float2{ width, height }, color, 0, 0, 1.0f });
    }
