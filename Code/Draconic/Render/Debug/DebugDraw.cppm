    ...
    public:
                         ...
                         bool overlay = false)
        {
            ...
            { // hemisphere arcs (XY + ZY planes, top + bottom)
                const f32 a0 = static_cast<f32>(i) * halfStep,
                          a1 = static_cast<f32>(i + 1) * halfStep;
        ...
        void DrawFrustum(const Float4x4& invViewProj, Color color, bool overlay = false)
        {
            ...
            for (i32 z = 0; z < 2; ++z)
                for (i32 y = 0; y < 2; ++y)
                    for (i32 x = 0; x < 2; ++x)
                    {
                        corners[idx++] = TransformPoint(invViewProj, Float3{x == 0 ? -1.0f : 1.0f,
                                                                            y == 0 ? -1.0f : 1.0f,
                                                                            static_cast<f32>(z)});
                    }
        ...
        void DrawScreenText(f32 x, f32 y, StringView text, Color color, f32 scale = 1.0f)
        {
            ...
            const i32 start = static_cast<i32>(m_textChars.Size());
            AppendChars(text);
            m_2d.PushBack(Debug2DCommand{Debug2DKind::Text, Float2{x, y}, Float2{0, 0}, color,
                                         start, static_cast<i32>(text.Size()), scale});
        }
        ...
        {
            m_2d.PushBack(Debug2DCommand{Debug2DKind::Rectangle, Float2{x, y},
                                         Float2{width, height}, color, 0, 0, 1.0f});
        }
