...
export namespace draconic::render
{
    ...
    public:
        ...
        Status Initialize()
        {
            ...
            m_shaders->RegisterSource(u8"debug_geom", shaders::ShaderStage::Fragment,
                                      DebugGeomPS());
            m_shaders->RegisterSource(u8"debug_screen", shaders::ShaderStage::Vertex,
                                      DebugScreenVS());
                             ...
                             i32 vpY, u32 vpW, u32 vpH, u32 frameIndex, u32 viewIndex)
        {
            ...
            graph.AddRenderPass(
                 ...
                 otN](rendergraph::PassBuilder& b)
                {
                    ...
                    b.NeverCull();
                    b.SetExecute(
                        [vb, p, vpMat, dlN, ol0, olN, dt0, dtN, ot0,
                         otN](rhi::RenderPassEncoder& rp)
                        {
    ...
    private:
                              ...
                              const Float4x4& viewProj, u32 vpW, u32 vpH)
        {
            ...
            for (const debug::Debug2DCommand& cmd : d->Commands2D())
            {
                ...
                if (cmd.kind == debug::Debug2DKind::Rectangle)
                {
                    ...
                    EmitQuad(out, cmd.position.x, cmd.position.y, cmd.size.x, cmd.size.y, u0, v0,
                             u1, v1, col);
                }
                else
                {
                    ...
                    {
                        f32 u0, v0, u1, v1;
                        if (debug::GetCharUV(chars[static_cast<usize>(cmd.textStart + i)], u0, v0,
                                             u1, v1))
                        {
        ...
        Pipelines* EnsurePipelines(rhi::TextureFormat colorFmt, rhi::TextureFormat depthFmt)
        {
            ...
            m_geom.lineDepth =
                MakeGeomPipeline(colorFmt, depthFmt, rhi::PrimitiveTopology::LineList,
                                 rhi::CompareFunction::LessEqual);
            m_geom.lineOverlay = MakeGeomPipeline(
                colorFmt, depthFmt, rhi::PrimitiveTopology::LineList, rhi::CompareFunction::Always);
            m_geom.triDepth =
                MakeGeomPipeline(colorFmt, depthFmt, rhi::PrimitiveTopology::TriangleList,
                                 rhi::CompareFunction::LessEqual);
            m_geom.triOverlay =
                MakeGeomPipeline(colorFmt, depthFmt, rhi::PrimitiveTopology::TriangleList,
        ...
        rhi::RenderPipeline* MakeGeomPipeline(rhi::TextureFormat colorFmt,
                                              ...
                                              rhi::PrimitiveTopology topo, rhi::CompareFunction cmp)
        {
            rhi::ShaderModule* vs = m_shaders->GetVariant(
                u8"debug_geom", shaders::ShaderStage::Vertex, shaders::ShaderFlags::None);
        ...
        Status CreateFontAtlas()
        {
            ...
            if (rhi::Queue* q = m_device->GetQueue(rhi::QueueType::Graphics, 0))
            {
                ...
                if (q->CreateTransferBatch(batch).IsOk() && batch != nullptr)
                {
                    ...
                    batch->WriteTexture(m_fontTex, Span<const u8>{pixels.Data(), pixels.Size()},
