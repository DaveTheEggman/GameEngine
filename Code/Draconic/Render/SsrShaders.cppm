/// Draconic::Render - the `:ssr_shaders` partition.
///
/// HLSL source for the SSR pass, split out from the pass logic (SsrPass.cppm) so the shader text is
/// isolated and trivial to lift into standalone .hlsl files later. Pass-internal: imported by `:ssr`,
/// which registers these with the ShaderSystem. Mirrors the draconic.vg.renderer:shaders pattern.

module;
#include "Core/Prelude.h"

export module draconic.render:ssr_shaders;
