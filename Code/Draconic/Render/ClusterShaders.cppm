/// Draconic::Render - the `:cluster_shaders` partition.
///
/// HLSL source for the clustered-light-culling build compute kernel, split out from the pass logic
/// (ClusterSystem.cppm) so the shader text is isolated and trivial to lift into a standalone .hlsl
/// file later. Pass-internal: imported by `:cluster_system`, which registers it with the ShaderSystem.
/// Mirrors the FxaaShaders.cppm pattern.

module;
#include "Core/Prelude.h"

export module draconic.render:cluster_shaders;
