...
export namespace draconic::render
{
    ...
    public:
        ...
        void BeginRendering(rhi::CommandEncoder& encoder, u32 frameIndex) override
        {
            ...
            m_frame->SetBloom(m_bloomEnabled ? m_bloomIntensity : 0.0f, m_bloomThreshold,
                              m_bloomKnee);
                         ...
                         const ViewPostOverride* postOverride = nullptr) override
        {
            ...
            if (firstSight)
            {
                ...
                ExtractDecalsInto(
                    scene, *snapshot); // screen-space decals (DecalPass, not the Renderer path)
                ExtractLightsInto(scene, *snapshot); // lights are shading inputs, not draws
                ExtractReflectionProbesInto(
                    scene, *snapshot); // reflection probes (capture/prefilter inputs)
            ...
            ViewCamera camera;
            Color clearColor{0.392f, 0.584f, 0.929f,
                             1.0f}; // cornflower fallback (no primary camera)
            ...
            settings.clear =
                rhi::ClearColor{clearColor.r, clearColor.g, clearColor.b, clearColor.a};
                ...
                settings.post.taaEnabled || (settings.post.ssrEnabled && m_ssrParams.temporal);
            {
                DRACONIC_PROFILE_SCOPE(
                    "Render.AddView"); // binds the view + builds/sorts its draw list
    ...
    protected:
        void OnInit() override
        {
            ...
            m_psoCache = MakeUnique<materials::PipelineStateCache>(DefaultAllocator(), *m_shaders,
                                                                   *m_device);
            ...
            m_meshRenderer =
                MakeUnique<MeshRenderer>(DefaultAllocator(), *m_device, *m_shaders, *m_psoCache,
                                         *m_materialSystem, m_framesInFlight);
            ...
            m_spriteRenderer = MakeUnique<SpriteRenderer>(DefaultAllocator(), *m_device, *m_shaders,
                                                          m_framesInFlight);
            ...
            if (kEnableClusters)
            {
                m_clusterSystem = MakeUnique<ClusterSystem>(DefaultAllocator(), *m_device,
                                                            *m_shaders, m_framesInFlight);
            ...
            m_tonemapPass = MakeUnique<TonemapPass>(DefaultAllocator(), *m_device, *m_shaders,
                                                    m_framesInFlight);
            ...
            m_debugPass = MakeUnique<DebugDrawPass>(DefaultAllocator(), *m_device, *m_shaders,
                                                    m_framesInFlight);
            ...
            m_frame
                ->EnableGpuProfiling(); // per-pass GPU timestamps (cheap; read on the P-key dump)
        }
        ...
        void OnShutdown() override
        {
            ...
            m_meshRenderer
                .Reset(); // before the systems it borrows (releases material instances first)
            m_materialSystem.Reset();
            m_psoCache.Reset();
            m_shaders.Reset();
