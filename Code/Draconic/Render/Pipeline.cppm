                         ...
                         u32 probeBase = 0, u32 probeCount = 0)
        {
            ...
            graph.AddRenderPass(
                ...
                [this, &view, &registry, depth, colorH, normalH, velocityH, materialH, colorLoad,
                 colorFormat, frameIndex, viewIndex, prevViewProj, jitter, prevJitter, cluster,
                 shadow, ibl, depthLoad, colorSub, probeHandle, probeValid, probeBase,
                 probeCount](rendergraph::PassBuilder& b)
                {
                    ...
                    b.SetColorTarget(0, colorH, colorLoad, rhi::StoreOp::Store,
                                     view.Settings().clear, colorSub);
                    ...
                    b.SetBundleExecute(
                         ...
                         probeCount](rhi::CommandEncoder& enc, Array<rhi::RenderBundle*>& out)
                        {
                            ResolveAndEmit(view, registry, enc, frameIndex, viewIndex, colorFormat,
                                           ...
                                           prevJitter, PassAffinity::Opaque, cluster, shadow, ibl,
                                           out, /*sceneDepth*/ nullptr,
                    ...
                    b.SetBundleExecute(
                         ...
                         probeCount](rhi::CommandEncoder& enc, Array<rhi::RenderBundle*>& out)
                        {
                            ...
                            ResolveAndEmit(view, registry, enc, frameIndex, viewIndex, colorFormat,
                                           ...
                                           PassAffinity::Blended, cluster, shadow, ibl, out,
                                           sceneDepth, /*probesEnabled*/ true, probeBase,
                                           probeCount);
                        });
                });
        }
    ...
    public:
                                 ...
                                 bool frustumCull = false, Span<const Float4> cullBounds = {})
        {
            ...
            if (frustumCull)
            {
                ...
                items =
                    Span<const DrawItem>{m_shadowCullScratch.Data(), m_shadowCullScratch.Size()};
            }
        ...
        void BuildShadowCasterList(const ExtractedScene& scene, SceneShadowCtx& ctx)
        {
            ...
            {
                const auto* md = static_cast<const MeshRenderData*>(it.data);
                ctx.casterBounds.PushBack(Float4{md->worldCenter.x, md->worldCenter.y,
                                                 md->worldCenter.z, md->worldRadius});
            }
        }
        ...
        void End()
        {
            ...
            if (atlasView != nullptr)
            {
                ...
                for (usize k = 0; k < sceneCount; ++k)
                {
                    ...
                    for (const LocalShadowCaster& c : ctx.scene->LocalShadowCasters())
                    {
                        ...
                        {
                            for (u32 f = 0;
                                 f < need && m_localShadows.Size() < kMaxLocalShadowEntries; ++f)
                            {
                                GpuLocalShadow dead;
                                dead.atlasScaleBias =
                                    Float4{0, 0, 0, 0}; // shader guard -> unshadowed
                        ...
                        for (u32 f = 0; f < need; ++f)
                        {
                            ...
                            entry.atlasSelect =
                                c.isStatic ? 1.0f : 0.0f; // sampled atlas array layer
                            ...
                            dst.PushBack(
                                AtlasDraw{LocalShadowTile{entry.viewProj, t.x, t.y, t.w, t.h,
...

            {
                DRACONIC_PROFILE_SCOPE(
                    "Compose.Prepare"); // per-frame GPU buffer sizing + pool resets
                ...
                if (m_probeSystem != nullptr && m_probeSystem->ActiveCount() > 0)
                {
                    ...
                    for (Renderer* r : m_registry->Unique())
                    {
                        r->SetProbes(m_probeSystem->PrefilterArrayView(),
                                     m_probeSystem->ProbeBuffer(), m_probeSystem->ActiveCount());
                    }
                }
                else
                {
            ...
            // (barriers + transient depth allocation/aliasing) + execute. (§9: one graph, all views.)
            {
                                ...
                                b.SetExecute(
                                    [this, reg, draws](rhi::RenderPassEncoder& rp)
                                    {
                                        for (const AtlasDraw& d : *draws)
                                        {
                                            ...
                                            const Span<const DrawItem> casters{
                                                d.ctx->casters.Data(), d.ctx->casters.Size()};
                                            rp.SetViewport(
                                                static_cast<f32>(t.x), static_cast<f32>(t.y),
                                                static_cast<f32>(t.w), static_cast<f32>(t.h));
                                            rp.SetScissor(static_cast<i32>(t.x),
                                                          static_cast<i32>(t.y), t.w, t.h);
                    ...
                    if (captureScene != nullptr && capCtx != nullptr)
                    {
                        ...
                        for (u32 face = 0; face < 6; ++face)
                        {
                            ...
                            const rendergraph::RGHandle capNormal = m_graph.CreateTransient(
                                u8"probe.normal",
                                rendergraph::RGTextureDesc(kGNormalFormat, res, res));
                            ...
                            m_sky->DeclareSky(
                                ...
                                capCtx->SunAngularSize(), Float3{1.0f, 0.98f, 0.92f}, sunInt, 0, 0,
                                res, res, m_frameIndex, /*viewIndex*/ 10u + face, capCtx->Uid(),
                                sub);
                        }
                        ...
                        m_probeSystem->DeclareBlit(m_graph, capturedH, probePrefilteredH,
                                                   task.slot);
                ...
                for (usize i = 0; i < m_views.ActiveCount(); ++i)
                {
                    ...
                    IblBinding ibl;
                    IBLSystem::Context* viewIblCtx =
                        (viewScene != nullptr) ? viewScene->ibl : nullptr;
                    ...
                    const Float4x4 curViewProj =
                        v->Camera().ViewProjection(); // jittered when TAA on
                    const Float4x4 prevViewProj = (viewIndex < m_prevViewProj.Size())
                                                      ? m_prevViewProj[viewIndex]
                                                      : curViewProj;
                                                ...
                                                rhi::TextureFormat colorFmt)
                    {
                        ...
                        const f32 sunInt =
                            viewIblCtx->HasSunDisc() ? viewIblCtx->SunIntensity() : 0.0f;
                        m_sky->DeclareSky(
                            m_graph, colorTarget, velocityTarget, depth, viewIblCtx->EnvHandle(),
                            viewIblCtx->EnvView(), colorFmt, m_pass.DepthFormat(), invVP,
                            prevViewProj, jitter, prevJitter, v->Camera().position,
                            viewIblCtx->SkyIntensity(), viewIblCtx->SunDir(),
                            viewIblCtx->SunAngularSize(), Float3{1.0f, 0.98f, 0.92f}, sunInt,
                    ...
                    if (m_tonemap != nullptr)
                    {
                        ...
                        {
                            aoH = m_ao->DeclareAo(
                                m_graph, depth, normalT, v->Width(), v->Height(),
                        ...
                        {
                            litHdr = m_ao->DeclareApply(m_graph, sceneHdr, aoH, v->Width(),
                                                        v->Height(), post.aoStrength);
                        }
                        ...
                        {
                            const f32 taaFar =
                                (v->Camera().farZ > 0.0f) ? v->Camera().farZ : 1000.0f;
                            sceneColor = m_taa->DeclareTaa(m_graph, litHdr, velocityT, depth,
                                                           viewIndex, v->Width(), v->Height(),
                                                           post.taaBlend, post.taaGamma,
                                                           m_taaMotionScale, /*near*/ 0.1f, taaFar);
                        }
                        ...
                        {
                            bloomH =
                                m_bloom->DeclareBloom(m_graph, sceneColor, v->Width(), v->Height(),
                        ...
                        m_tonemap->DeclareTonemap(
                            ...
                            /*clearColor*/ fxaa || clearColor, v->Settings().clear,
                            v->TargetFormat(), v->ViewportX(), v->ViewportY(), v->ViewportWidth(),
                            v->ViewportHeight(), m_frameIndex, viewIndex, post.exposure,
                            bloomStrength, uvScale, uvOffset, aoStrength, showAo, post.agxTonemap);
                        ...
                        m_pass.DeclarePostTonemapUI(*v, *m_registry, m_graph, m_frameIndex,
                                                    viewIndex, fxaa ? tonemapOut : colorH, depth,
                        ...
                        if (fxaa)
                        {
                            ...
                            m_fxaa->DeclareFxaa(m_graph, tonemapOut, colorH, clearColor,
                                                ...
                                                v->ViewportX(), v->ViewportY(), v->ViewportWidth(),
                                                v->ViewportHeight(), m_frameIndex, viewIndex, texel,
                    ...
                    else
                    {
                        ...
                        m_pass.DeclarePass(*v, *m_registry, m_graph, m_frameIndex, viewIndex,
                                           colorH, depth, clearColor, v->TargetFormat(), normalT,
                                           velocityT, materialT, prevViewProj, jitter, prevJitter,
                                           cluster, shadow, ibl, rhi::LoadOp::Load,
                                           rendergraph::RGSubresourceRange{}, probePrefilteredH,
                                           probeActive, probeRange.base, probeRange.count);
                        ...
                        m_pass.DeclarePostTonemapUI(*v, *m_registry, m_graph, m_frameIndex,
                                                    viewIndex, colorH, depth, v->TargetFormat(),
                    ...
                    if (m_debugPass != nullptr)
                    {
                        ...
                        if (m_debugGlobal != nullptr || sceneDbg != nullptr)
                        {
                            m_debugPass->DeclareGeometry(
                                ...
                                v->TargetFormat(), m_pass.DepthFormat(), v->ViewportX(),
                                v->ViewportY(), v->ViewportWidth(), v->ViewportHeight(),
                                m_frameIndex, viewIndex);
                            m_debugPass->DeclareScreen(
                                m_graph, colorH, unjitteredVP, m_debugGlobal, sceneDbg,
                                v->TargetFormat(), v->ViewportX(), v->ViewportY(),
            ...
            m_prevViewProj =
                m_curViewProj;          // this frame's view-projs become next frame's "previous"
