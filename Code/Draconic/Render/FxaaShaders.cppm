/// Draconic::Render - the `:fxaa_shaders` partition.
///
/// HLSL source for the FXAA pass, split out from the pass logic (FxaaPass.cppm) so the shader text is
/// isolated and trivial to lift into standalone .hlsl files later. Pass-internal: imported by `:fxaa`,
/// which registers these with the ShaderSystem. Mirrors the draconic.vg.renderer:shaders pattern.

module;
#include "Core/Prelude.h"

export module draconic.render:fxaa_shaders;
