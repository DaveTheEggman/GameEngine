...
public:
    ...
    [[nodiscard]] rendergraph::RGHandle DeclareBloom(rendergraph::RenderGraph& graph, rendergraph::RGHandle hdr,
        ...
        for (u32 i = 0; i < levels; ++i) {
            ...
            BloomPush push{};
            push.srcTexel = Vec2{ 1.0f / static_cast<f32>(srcW), 1.0f / static_cast<f32>(srcH) };
        ...
        for (i32 i = static_cast<i32>(levels) - 2; i >= 0; --i) {
            ...
            const rendergraph::RGHandle dst = chain[i];
            BloomPush push{};
            push.srcTexel = Vec2{ 1.0f / static_cast<f32>(lw[i + 1]), 1.0f / static_cast<f32>(lh[i + 1]) };
            const u32 dw = lw[i], dh = lh[i];
...
private:
    struct BloomPush { Vec2 srcTexel{}; f32 threshold = 1.0f; f32 knee = 0.5f; i32 firstPass = 0; f32 pad0 = 0, pad1 = 0, pad2 = 0; };
