...
export namespace draconic::render
{
    ...
    public:
                                                         ...
                                                         u32 vpH, f32 threshold, f32 knee)
        {
            ...
            u32 levels = 1;
            for (u32 w = vpW / 2, h = vpH / 2; levels < kMaxLevels && w > 8 && h > 8;
                 w /= 2, h /= 2)
            {
                ++levels;
            }
            ...
            for (u32 i = 0; i < levels; ++i)
            {
                ...
                BloomPush push{};
                push.srcTexel =
                    Float2{1.0f / static_cast<f32>(srcW), 1.0f / static_cast<f32>(srcH)};
                ...
                graph.AddRenderPass(
                    ...
                    {
                        b.SetColorTarget(0, dst, rhi::LoadOp::Clear, rhi::StoreOp::Store,
                        ...
                        b.SetExecute(
                            [this, &graph, src, push](rhi::RenderPassEncoder& rp)
                            {
                                rhi::BindGroup* bg = EnsureBindGroup(
                                    graph.GetTextureView(src), graph.GetTextureGeneration(src));
                                if (bg == nullptr)
                                {
                                    return;
                                }
