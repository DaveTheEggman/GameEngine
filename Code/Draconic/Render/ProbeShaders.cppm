/// Draconic::Render - the `:probe_shaders` partition.
///
/// HLSL source for the reflection-probe capture->prefilter passes, split out from the system logic
/// (ReflectionProbeSystem.cppm) so the shader text is isolated and trivial to lift into standalone .hlsl
/// files later. Pass-internal: imported by `:probes`, which registers these with the ShaderSystem.
/// Mirrors the draconic.vg.renderer:shaders pattern.

module;
#include "Core/Prelude.h"

export module draconic.render:probe_shaders;
