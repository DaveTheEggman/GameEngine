...
export namespace draconic::render
{
    ...
    public:
        ...
        Status Initialize()
        {
            ...
            if (!MakePipelineLayout(m_shadowViewLayout, m_objectLayout,
                                    m_shadowPipelineLayoutSingle))
            {
                return Status{ErrorCode::Unknown};
            }
        ...
        void PrepareFrame(u32 maxDraws, u32 frameIndex) override
        {
            ...
            if (!m_viewRing.Reserve(maxDraws) || !m_shadowViewRing.Reserve(kMaxShadowPasses) ||
                ...
                !m_localShadowRing.Reserve(kMaxLocalShadows) ||
                !m_boneRing.Reserve(kMaxBoneMatrices))
            {
                return;
            }
            if (!EnsureBoneDevice())
        ...
        void EnsureMultiMeshSet(const MultiMeshRenderData& mm)
        {
            ...
            if (set->dirtyFrames > 0 && set->instanceBuf != nullptr && mm.transforms != nullptr)
            {
                if (auto* dst = static_cast<InstanceData*>(set->instanceBuf->Map()))
                {
                    ...
                    {
                        const Color tint = (mm.tints != nullptr)
                                               ? mm.tints[i]
                                               : mm.color; // per-instance or shared
            ...
            if (set->skinned &&
                (set->offsetsBuf == nullptr || mm.instanceCount > set->offsetsCapacity))
            {
                m_device->WaitIdle();
        ...
        void FillSkinnedMultiMeshOffsets(const ExtractedScene& scene)
        {
            ...
            for (RenderData* data : scene.Items())
            {
                ...
                for (u32 i = 0; i < mm->instanceCount; ++i)
                {
                    ...
                    const u32 prvBase =
                        pool->prevBase + bucket; // last frame's pose (motion vectors)
                    od[base + i] =
                        DataOffsets{i, curBase, prvBase,
                                    0}; // .x = Instances[] idx, .y = cur bone base, .z = prev
                }
                     ...
                     Array<ResolvedDraw>& out) override
        {
            ...
            *static_cast<ViewData*>(view.ptr) = vd;
            ...
            while (i < items.Size())
            {
                ...
                const bool headSkinned = head->mesh != nullptr && head->mesh->IsSkinned() &&
                                         head->boneMatrices != nullptr;
                ...
                if (allowInstancing)
                {
                    while (j < items.Size())
                    {
                        ...
                        if (nd->multiMesh || nd->mesh != head->mesh ||
                            nd->material != head->material)
                        {
                            break;
                        }
                        ++j;
                ...
                if (head->multiMesh)
                {
                    ...
                    if (mmMesh != nullptr)
                    {
                        ResolveMultiMeshDepth(ctx, shadowViewOffset,
                                              *static_cast<const MultiMeshRenderData*>(head),
                                              *mmMesh, out);
                    }
                    ++i;
                    continue;
    ...
    private:
                           ...
                           Array<ResolvedDraw>& out)
        {
            materials::Material* mat =
                (md.material != nullptr) ? md.material : m_defaultMaterial.Get();
            ...
            *static_cast<ObjectData*>(obj.ptr) = od;
            ...
            const auto emit = [&](materials::Material* m, u64 indexOffset, u32 indexCount)
            {
                ...
                rhi::PipelineLayout* plLayout =
                    GetOrCreatePipelineLayout(set2, /*instanced*/ false);
                if (plLayout == nullptr)
                {
                    return;
                }
                rhi::RenderPipeline* pso =
                    m_psoCache->GetPipeline(config, plLayout, ctx.colorFormat);
                if (pso == nullptr)
                {
                    return;
                }
                              ...
                              Array<ResolvedDraw>& out)
        {
            ...
            else
            {
                ...
                for (u32 k = 0; k < count; ++k)
                {
                    ...
                    id[k] = InstanceData{md->world,
                                         ctx.needsMotion ? PrevWorldFor(md->entityId, md->world)
                                                         : md->world,
                                         md->color};
                                ...
                                Array<ResolvedDraw>& out)
        {
            ...
            *static_cast<ObjectData*>(obj.ptr) = od;
            ...
            d.materialSet =
                matSet; // set 2: material (masked casters only - for the alpha-test sample)
                              ...
                              const GpuMesh& mesh, Array<ResolvedDraw>& out)
        {
            ...
            materials::Material* mat =
                (mm.material != nullptr) ? mm.material : m_defaultMaterial.Get();
        ...
        bool EnsureViewBindGroup(const RenderRecordContext& ctx)
        {
            ...
            if (viewBuf == nullptr || lightBuf == nullptr || localBuf == nullptr ||
                boneBuf == nullptr || m_activeShadowView == nullptr ||
                m_activeAtlasView == nullptr || m_shadowSampler == nullptr || sh == nullptr ||
                prefilter == nullptr || brdf == nullptr || m_envSampler == nullptr)
            {
                return false;
            }
