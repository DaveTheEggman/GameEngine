...
TEST_CASE("ui.scripthost: Ui.* ops drive real screen-tier controls by id + click -> delegate")
{
    ...
    REQUIRE(h != 0);
    CHECK(host.OverlayCount() == 1u);              // handle registered synchronously
    CHECK(ui->ScreenOverlayCount() == 0);          // ATTACH is deferred (mutation-queue rule)
    ui->Context().MutationQueueRef().Drain();
    CHECK(ui->ScreenOverlayCount() == 1);          // attached after the queue drains
    ...
    host.PopOverlay(h);
    CHECK(host.OverlayCount() == 0u);      // handle dies synchronously
    CHECK(ui->ScreenOverlayCount() == 1);  // DETACH is deferred
    ui->Context().MutationQueueRef().Drain();
    CHECK(ui->ScreenOverlayCount() == 0);  // detached after the queue drains
...
TEST_CASE("ui.scripthost: push/pop from INSIDE a click handler defers the tree mutation")
{
    // Fable's finding: a script's "close" button pops its own overlay from the onClick handler.
    // The detach must be DEFERRED (mutation-queue rule) - destroying the view tree mid-dispatch is
    // the exact bug the rule guards against. Same for a push from a handler.
    runtime::Context ctx;
    auto* ui = ctx.AddSubsystem<UISubsystem>();
    ctx.Startup();

    RefPtr<UIDocument> menu = MakeDocument(u8"<FlexLayout><Button id=\"close\" text=\"Close\"/></FlexLayout>");
    UiScriptHost host;
    host.Attach(*ui);
    host.SetDocumentResolver(
        Function<RefPtr<UIDocument>(const Guid&)>{[&](const Guid&) { return menu; }});

    const i32 h = host.PushOverlay(Guid{7, 7});
    REQUIRE(h != 0);
    ui->Context().MutationQueueRef().Drain();
    CHECK(ui->ScreenOverlayCount() == 1);

    // Bind a handler that pops THIS overlay, then fire it as a real click (inside dispatch).
    RefPtr<PopDelegate> popFn = MakeRef<PopDelegate>(DefaultAllocator());
    popFn->host = &host;
    popFn->handle = h;
    host.OnClick(h, u8"close", popFn);

    Button* close = Cast<ViewGroup>(ui->ScreenRoot())->FindByName<Button>(u8"close");
    REQUIRE(close != nullptr);
    close->OnClick.Invoke(close); // fire the click -> handler pops the overlay

    // Mid-dispatch: the handle is gone but the tree is STILL attached (no mid-dispatch destroy).
    CHECK(host.OverlayCount() == 0u);
    CHECK(ui->ScreenOverlayCount() == 1);
    ui->Context().MutationQueueRef().Drain(); // the deferred detach runs at the safe point
    CHECK(ui->ScreenOverlayCount() == 0);
