...
export namespace draconic::editor
{
    ...
    public:
            ...
            : m_context(&context), m_host(&host), m_uiHost(&uiHost), m_title(instance.Name())
        {
            ...
            m_gameUI = host.Ctx().GetSubsystem<draconic::engine::ui::UISubsystem>();
    ...
    private:
        ...
        draconic::engine::scene::SceneSubsystem* m_scenes = nullptr; // borrowed (context subsystem: registry + tick)
        scene::SceneManager
        ...
        draconic::engine::ui::UISubsystem* m_gameUI = nullptr; // RT-canvas host seam (borrowed)
    ...
    CreateSceneInstance(EditorContext& context, draconic::content::Group* target = nullptr)
    {
        ...
        // saves with the scene, shows in the hierarchy, and is free to edit or delete.
        {
            ...
            seeded.AddSystem<draconic::engine::render::LightComponentManager>();
            ...
            draconic::engine::render::LightComponent& light =
                seeded.GetSystem<draconic::engine::render::LightComponentManager>()->Add(sun);
    ...
    inline void RegisterSceneEditor(EditorContext& context, runtime::IApplicationHost& host,
                                    ...
                                    draconic::engine::runtime::DefaultApplication* embeddedApp = nullptr)
    {
                                              ...
                                              Array<byte>& out) -> bool
        {
            ...
            auto* scenes = appHost->Ctx().GetSubsystem<draconic::engine::scene::SceneSubsystem>();
            if (scenes == nullptr)
            {
                return false;
            }
                                     ...
                                     const ImportOptions* options)
            {
                ...
                if (generated.regenerated)
                {
                    ...
                    if (payload.Get() != nullptr)
                    {
                        ...
                        if (auto* scenes = appHost->Ctx().GetSubsystem<draconic::engine::scene::SceneSubsystem>())
                        {
                            scenes->ForEachScene(
                                [&](scene::Scene& scene)
                                {
