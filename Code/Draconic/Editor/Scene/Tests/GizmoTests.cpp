...
namespace
{
    ...
    GizmoRay RayThrough(Float3 point)
    {
        return GizmoRay{ kCamPos, Normalized(point - kCamPos) };
    }
...
TEST_CASE("gizmo: ray-axis / ray-ring / ray-point distances")
{
    ...
    Float3 hit;
    CHECK(TransformGizmo::RayRingDistance(RayThrough(Float3{ 0.8f, 0.0f, 0.0f }),
                                          Float3{}, Float3{ 0, 0, 1 }, 0.8f, &hit)
          == doctest::Approx(0.0f).epsilon(0.01f));
    ...
    CHECK(TransformGizmo::RayRingDistance(away, Float3{}, Float3{ 0, 0, 1 }, 0.8f, nullptr) == kFloatMax);
...
TEST_CASE("gizmo: hover picks with priorities and grazing disables")
{
    ...
    CHECK(g.UpdateHover(RayThrough(Float3{ 0.7f, 0.0f, 0.0f }), GizmoMode::Translate) == GizmoAxis::X);
    CHECK(g.UpdateHover(RayThrough(Float3{ 0.0f, 0.7f, 0.0f }), GizmoMode::Translate) == GizmoAxis::Y);
    ...
    CHECK(g.UpdateHover(RayThrough(Float3{ 0.4f, 0.4f, 0.0f }), GizmoMode::Translate) == GizmoAxis::PlaneZ);
    ...
    CHECK(g.UpdateHover(RayThrough(Float3{ 3.0f, 3.0f, 0.0f }), GizmoMode::Translate) == GizmoAxis::None);
    ...
    CHECK(g.UpdateHover(RayThrough(Float3{ 0.566f, 0.566f, 0.0f }), GizmoMode::Rotate) == GizmoAxis::Z);
    // The outer screen-space ring (radius 1.0, camera-facing).
    CHECK(g.UpdateHover(RayThrough(Float3{ 0.0f, 1.0f, 0.0f }), GizmoMode::Rotate) == GizmoAxis::View);
}
...
TEST_CASE("gizmo: translate drag along an axis, on a plane, and snapped")
{
    ...
    REQUIRE(g.UpdateHover(RayThrough(Float3{ 0.7f, 0.0f, 0.0f }), GizmoMode::Translate) == GizmoAxis::X);
    ...
    REQUIRE(g.UpdateHover(RayThrough(Float3{ 0.4f, 0.4f, 0.0f }), GizmoMode::Translate) == GizmoAxis::PlaneZ);
...
TEST_CASE("gizmo-controller: a drag session is exactly one undo entry restoring the start")
{
    ...
    CHECK_FALSE(commands.CanUndo());
    commands.Redo();
    CHECK(scene.GetLocalTransform(edit.Resolve(id)).position.x == doctest::Approx(2.0f).epsilon(0.05f));
    ...
    CHECK(scene.GetLocalTransform(edit.Resolve(id)).position.x == doctest::Approx(3.0f).epsilon(0.05f));
    commands.Undo();
    CHECK(scene.GetLocalTransform(edit.Resolve(id)).position.x == doctest::Approx(2.0f).epsilon(0.05f));
    commands.Undo();
    CHECK(scene.GetLocalTransform(edit.Resolve(id)).position.x == doctest::Approx(0.0f));
...
TEST_CASE("gizmo-controller: pose tracks selection even while the pointer is off the viewport")
{
    ...
    CHECK_FALSE(ctl.Gizmo().IsDragging());
    CHECK(scene.GetLocalTransform(edit.Resolve(b)).position.x == doctest::Approx(6.0f).epsilon(0.05f));
    REQUIRE(commands.CanUndo());
    commands.Undo();
