...
export namespace draconic::particles
{
    ...
    {
        Position = 0, Velocity = 1, StartVelocity = 2, Color = 3, Size = 4,
        Age = 5, Lifetime = 6, Rotation = 7, RotationSpeed = 8, Axis = 9,
        Custom = 32, MaxStreams = 64,
    };

    enum class StreamElementType : u8 { Float, Float2, Float3, Float4, Int32 };
    ...
    public:
        ...
        [[nodiscard]] T&       operator[](i32 index) noexcept       { return m_data[static_cast<usize>(index)]; }
        [[nodiscard]] const T& operator[](i32 index) const noexcept { return m_data[static_cast<usize>(index)]; }
        [[nodiscard]] T*       Data() noexcept                      { return m_data.Data(); }
        [[nodiscard]] Span<T>  Slice(i32 count) noexcept            { return Span<T>{ m_data.Data(), static_cast<usize>(count) }; }

        void SwapRemoveElement(i32 index, i32 aliveCount) noexcept override
        {
            const i32 last = aliveCount - 1;
            if (index < last) { m_data[static_cast<usize>(index)] = m_data[static_cast<usize>(last)]; }
        }
    ...
    public:
        ...
        ~ParticleStreamContainer()
        {
            ...
            for (ParticleStream*& s : m_streams) { if (s != nullptr) { alloc.Delete(s); s = nullptr; } }
        }
        ...
        [[nodiscard]] CPUStream<T>* GetCPUStream(ParticleStreamId id) const noexcept
        {
            ...
            if (s == nullptr || !s->IsCPU() || s->ElementType() != StreamElementOf<T>::value) { return nullptr; }
        ...
        void EnsureStream(ParticleStreamId id, StreamElementType elementType)
        {
            ...
            if (m_streams[slot] != nullptr) { return; }
            IAllocator& alloc = DefaultAllocator();
            switch (elementType)
            {
        ...
        [[nodiscard]] f32 GetLifeRatio(i32 index) const noexcept
        {
            ...
            if (ages == nullptr || lifetimes == nullptr) { return 1.0f; }
        ...
        {
            for (ParticleStream* s : m_streams) { if (s != nullptr) { s->SwapRemoveElement(index, aliveCount); } }
            --aliveCount;
        }
        ...
        i32 CompactDead() noexcept
        {
            const CPUStream<f32>* ages = Ages();
            const CPUStream<f32>* lifetimes = Lifetimes();
            if (ages == nullptr || lifetimes == nullptr) { return 0; }
            i32 removed = 0;
            for (i32 i = aliveCount - 1; i >= 0; --i)
            {
                if ((*ages)[i] >= (*lifetimes)[i]) { SwapRemove(i); ++removed; }
            }
            return removed;
        }
