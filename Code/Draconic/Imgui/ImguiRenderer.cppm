...
class ImguiRenderer {
public:
    ImguiRenderer(rhi::Device& device, shaders::ShaderSystem& shaders, u32 framesInFlight) noexcept
        : m_device(&device), m_shaders(&shaders), m_fif(framesInFlight < 1 ? 1 : (framesInFlight > kMaxFIF ? kMaxFIF : framesInFlight)) {}
    ...
    Status Initialize() {
        ...
        rhi::BindGroupLayoutEntry projE = rhi::BindGroupLayoutEntry::UniformBuffer(0, rhi::ShaderStage::Vertex);
        rhi::BindGroupLayoutEntry texE  = rhi::BindGroupLayoutEntry::SampledTexture(0, rhi::ShaderStage::Fragment);
        ...
        if (!CreateFontAtlas()) { return Status{ ErrorCode::Unknown }; }
        ...
        for (u32 i = 0; i < m_fif; ++i) {
            rhi::BufferDesc ud{}; ud.size = sizeof(Float4x4); ud.usage = rhi::BufferUsage::Uniform; ud.memory = rhi::MemoryLocation::CpuToGpu; ud.label = u8"imgui.proj";
            ...
            if (!m_device->CreateBindGroup(bgd, m_frames[i].bindGroup).IsOk()) { return Status{ ErrorCode::Unknown }; }
        }
        return Status{};
    }
    ...
    void Render(rhi::CommandEncoder& enc, rhi::TextureView* target, rhi::TextureFormat targetFormat,
                u32 width, u32 height, const ImDrawData* drawData, u32 frameIndex) {
        if (target == nullptr || drawData == nullptr) { return; }
        EnsureFontUploaded(enc);
        rhi::RenderPipeline* pipeline = EnsurePipeline(targetFormat);
        if (pipeline == nullptr) { return; }
        if (drawData->TotalVtxCount <= 0 || drawData->CmdListsCount <= 0) { return; }

        FrameSlot& slot = m_frames[frameIndex % m_fif];
        if (!UploadGeometry(slot, drawData)) { return; }
        ...
        if (void* p = slot.ubo->Map()) { MemCopy(p, &proj, sizeof(Float4x4)); slot.ubo->Unmap(); }
        ...
        for (i32 n = 0; n < drawData->CmdListsCount; ++n) {
            ...
            for (i32 c = 0; c < cmdList->CmdBuffer.Size; ++c) {
                ...
                if (cmd.ElemCount == 0 || cmd.UserCallback != nullptr) { continue; }
                ...
                if (w <= 0 || h <= 0) { continue; }
                pass->SetScissor(x < 0 ? 0 : x, y < 0 ? 0 : y, static_cast<u32>(w), static_cast<u32>(h));
                pass->SetBindGroup(0, slot.bindGroup, Span<const u32>{});
                pass->DrawIndexed(cmd.ElemCount, 1, cmd.IdxOffset + globalIdx, static_cast<i32>(cmd.VtxOffset + globalVtx), 0);
            }
...
private:
    ...
    struct FrameSlot {
        rhi::Buffer*    vtx = nullptr; u64 vtxCap = 0;
        rhi::Buffer*    idx = nullptr; u64 idxCap = 0;
    ...
    bool CreateFontAtlas() {
        ...
        if (pixels == nullptr || w <= 0 || h <= 0) { return false; }
        const u64 bytes = static_cast<u64>(w) * static_cast<u64>(h) * 4u;

        rhi::TextureDesc td{};
        td.format = rhi::TextureFormat::RGBA8Unorm; td.width = static_cast<u32>(w); td.height = static_cast<u32>(h);
        td.usage = rhi::TextureUsage::Sampled | rhi::TextureUsage::CopyDst; td.label = u8"imgui.font";
        if (!m_device->CreateTexture(td, m_fontTex).IsOk()) { return false; }
        rhi::TextureViewDesc vd{}; vd.format = rhi::TextureFormat::RGBA8Unorm; vd.dimension = rhi::TextureViewDimension::Texture2D;
        if (!m_device->CreateTextureView(m_fontTex, vd, m_fontView).IsOk()) { return false; }

        rhi::BufferDesc sd{}; sd.size = bytes; sd.usage = rhi::BufferUsage::CopySrc; sd.memory = rhi::MemoryLocation::CpuToGpu; sd.label = u8"imgui.fontStaging";
        if (!m_device->CreateBuffer(sd, m_fontStaging).IsOk()) { return false; }
        if (void* p = m_fontStaging->Map()) { MemCopy(p, pixels, bytes); m_fontStaging->Unmap(); }
        m_fontW = static_cast<u32>(w); m_fontH = static_cast<u32>(h);
        ...
        io.Fonts->SetTexID(static_cast<ImTextureID>(1));   // single font texture; we always bind it
        return true;
    }

    void EnsureFontUploaded(rhi::CommandEncoder& enc) {
        if (m_fontUploaded || m_fontTex == nullptr || m_fontStaging == nullptr) { return; }
        enc.TransitionTexture(m_fontTex, rhi::ResourceState::Undefined, rhi::ResourceState::CopyDst);
        rhi::BufferTextureCopyRegion region{};
        region.bufferOffset = 0; region.bytesPerRow = m_fontW * 4u; region.rowsPerImage = m_fontH;
        region.textureExtent = rhi::Extent3D{ m_fontW, m_fontH, 1 };
        enc.CopyBufferToTexture(m_fontStaging, m_fontTex, region);
        enc.TransitionTexture(m_fontTex, rhi::ResourceState::CopyDst, rhi::ResourceState::ShaderRead);
        m_fontUploaded = true;
    }
    ...
    bool UploadGeometry(FrameSlot& slot, const ImDrawData* drawData) {
        ...
        if (vtxBytes == 0 || idxBytes == 0) { return false; }
        if (!EnsureBuffer(slot.vtx, slot.vtxCap, vtxBytes, rhi::BufferUsage::Vertex)) { return false; }
        if (!EnsureBuffer(slot.idx, slot.idxCap, idxBytes, rhi::BufferUsage::Index))  { return false; }

        auto* vdst = static_cast<u8*>(slot.vtx->Map());
        auto* idst = static_cast<u8*>(slot.idx->Map());
        if (vdst == nullptr || idst == nullptr) { if (vdst) slot.vtx->Unmap(); if (idst) slot.idx->Unmap(); return false; }
        u64 vo = 0, io_ = 0;
        for (i32 n = 0; n < drawData->CmdListsCount; ++n) {
            ...
            MemCopy(vdst + vo, cl->VtxBuffer.Data, vb); vo += vb;
            MemCopy(idst + io_, cl->IdxBuffer.Data, ib); io_ += ib;
        }
        slot.vtx->Unmap(); slot.idx->Unmap();
        return true;
    }
    ...
    bool EnsureBuffer(rhi::Buffer*& buf, u64& cap, u64 bytes, rhi::BufferUsage usage) {
        ...
        if (buf != nullptr) { m_device->DestroyBuffer(buf); buf = nullptr; }
        u64 want = (bytes + bytes / 2u + 0xFFFFu) & ~static_cast<u64>(0xFFFFu);
        rhi::BufferDesc bd{}; bd.size = want; bd.usage = usage; bd.memory = rhi::MemoryLocation::CpuToGpu; bd.label = u8"imgui.geom";
    ...
    rhi::RenderPipeline* EnsurePipeline(rhi::TextureFormat fmt) {
        ...
        rhi::ShaderModule* vs = m_shaders->GetVariant(u8"imgui", shaders::ShaderStage::Vertex,   shaders::ShaderFlags::None);
        rhi::ShaderModule* ps = m_shaders->GetVariant(u8"imgui", shaders::ShaderStage::Fragment, shaders::ShaderFlags::None);
        ...
        rhi::VertexBufferLayout vbl{}; vbl.stride = sizeof(ImDrawVert); vbl.stepMode = rhi::VertexStepMode::Vertex;
        vbl.attributes = Span<const rhi::VertexAttribute>{ attrs, 3 };

        rhi::ColorTargetState color{}; color.format = fmt; color.blend = rhi::BlendState::AlphaBlend();
    ...
    void Shutdown() {
        for (u32 i = 0; i < kMaxFIF; ++i) {
            if (m_frames[i].bindGroup) { m_device->DestroyBindGroup(m_frames[i].bindGroup); m_frames[i].bindGroup = nullptr; }
            if (m_frames[i].ubo) { m_device->DestroyBuffer(m_frames[i].ubo); m_frames[i].ubo = nullptr; }
            if (m_frames[i].vtx) { m_device->DestroyBuffer(m_frames[i].vtx); m_frames[i].vtx = nullptr; }
            if (m_frames[i].idx) { m_device->DestroyBuffer(m_frames[i].idx); m_frames[i].idx = nullptr; }
        }
        ...
        if (m_fontStaging) { m_device->DestroyBuffer(m_fontStaging); m_fontStaging = nullptr; }
        if (m_fontView) { m_device->DestroyTextureView(m_fontView); m_fontView = nullptr; }
        if (m_fontTex) { m_device->DestroyTexture(m_fontTex); m_fontTex = nullptr; }
    ...
    rhi::Texture*     m_fontTex = nullptr; rhi::TextureView* m_fontView = nullptr;
    rhi::Buffer*      m_fontStaging = nullptr; rhi::Sampler* m_sampler = nullptr;
