...
struct ModelVertex {
    ...
    Float3 tangent{ 1, 0, 0 };    // 12 bytes
    // Total: 48 bytes

    constexpr ModelVertex() = default;
    constexpr ModelVertex(Float3 pos, Float3 nrm, Float2 uv, u32 col = 0xFFFFFFFF, Float3 tan = { 1, 0, 0 })
...
struct SkinnedModelVertex {
    ...
    constexpr SkinnedModelVertex(Float3 pos, Float3 nrm, Float2 uv, u32 col, Float3 tan,
                                 u16 j0, u16 j1, u16 j2, u16 j3, Float4 wt)
        : position(pos), normal(nrm), texCoord(uv), color(col), tangent(tan),
