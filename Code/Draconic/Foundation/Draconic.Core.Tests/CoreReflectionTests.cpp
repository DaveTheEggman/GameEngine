...
#include "Draconic.Core/Prelude.h" // brings <new> into reach for container instantiation (GCC)
...
TEST_CASE("core-reflection: namespace-level constants are registered")
{
    ...
    const NamedConstant* pi = cr.Find("draconic::core", "kPi");
    REQUIRE(pi != nullptr);
    CHECK(pi->type == &TypeOf<f32>());
    CHECK(pi->value.Get<f32>() == kPi);

    CHECK(cr.Find("draconic::core", "kEpsilon")->value.Get<f32>() == kEpsilon);
    CHECK(cr.Find("draconic::core", "nope") == nullptr);
    CHECK(cr.Count() >= 8u);
...
TEST_CASE("core-reflection: public enums are reflected")
{
    ...
    CHECK(GlobalTypeRegistry().FindByName("draconic::core", "LogLevel") == &TypeOf<LogLevel>());
    CHECK(GlobalTypeRegistry().FindByName("draconic::core", "FileMode") == &TypeOf<FileMode>());
}
...
TEST_CASE("core-reflection: types are in the global registry by qualified name")
{
    ...
    const TypeInfo* vec3 = GlobalTypeRegistry().FindByName("draconic::core", "Float3");
