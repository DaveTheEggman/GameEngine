...
export namespace draconic::gui
{
    ...
    public:
        ...
        void Register(core::StringView name, Factory factory) { m_entries.PushBack(Entry{ core::String(name), core::Move(factory) }); }
        [[nodiscard]] bool Has(core::StringView name) const
        {
            for (const Entry& e : m_entries) if (e.Name.AsView() == name) return true;
            return false;
        }
        [[nodiscard]] RefPtr<Node> Create(core::StringView name) const
        {
            for (const Entry& e : m_entries) if (e.Name.AsView() == name) return e.Make ? e.Make() : RefPtr<Node>{};
            return RefPtr<Node>{};
        }

    private:
        struct Entry { core::String Name; Factory Make; };
    ...
    {
        WidgetFactory f;
        auto reg = [&f](core::StringView name, auto maker) { f.Register(name, [maker]() -> RefPtr<Node> { return maker(); }); };
        reg(core::StringView(u8"Label"),          [] { return core::MakeRef<Label>(core::DefaultAllocator()); });
        reg(core::StringView(u8"Button"),         [] { return core::MakeRef<Button>(core::DefaultAllocator()); });
        reg(core::StringView(u8"CheckBox"),       [] { return core::MakeRef<CheckBox>(core::DefaultAllocator()); });
        reg(core::StringView(u8"RadioButton"),    [] { return core::MakeRef<RadioButton>(core::DefaultAllocator()); });
        reg(core::StringView(u8"Slider"),         [] { return core::MakeRef<Slider>(core::DefaultAllocator()); });
        reg(core::StringView(u8"ProgressBar"),    [] { return core::MakeRef<ProgressBar>(core::DefaultAllocator()); });
        reg(core::StringView(u8"TextField"),      [] { return core::MakeRef<TextField>(core::DefaultAllocator()); });
        ...
        reg(core::StringView(u8"Window"),         [] { return core::MakeRef<Window>(core::DefaultAllocator()); });
        return f;
    }
    ...
    public:
        explicit MarkupLoader(const WidgetFactory& factory, IResourceProvider* resources = nullptr,
                              fonts::IFontService* fontService = nullptr, LengthContext lengths = {}) noexcept
            : m_factory(&factory), m_resources(resources), m_fontService(fontService), m_lengths(lengths) {}
        ...
        [[nodiscard]] RefPtr<Node> LoadFromString(core::StringView markup)
        {
            ...
            if (document.Parse(markup) != xml::XmlResult::Ok) return {};
    ...
    private:
        [[nodiscard]] RefPtr<Node> Inflate(xml::XmlElement& element)
        {
            ...
            if (!widget) return {}; // unknown element name
            ...
            for (xml::XmlAttribute* attr : element.Attributes())
            {
                ...
                if (name == core::StringView(u8"id")) { if (uiwidget != nullptr) uiwidget->SetId(value); continue; }
                if (name == core::StringView(u8"class")) { AddClasses(uiwidget, value); continue; }
                if (uinode != nullptr && uinode->SetMarkupAttribute(name, value)) continue;
                inlineStyle.Set(name, value); // an inline CSS property
            }
            if (uinode != nullptr) ApplyStyle(*uinode, inlineStyle, m_resources, m_fontService, m_lengths);

            for (xml::XmlElement* child = element.FirstChildElement(); child != nullptr; child = child->NextSiblingElement())
                if (RefPtr<Node> childWidget = Inflate(*child)) widget->AddChild(childWidget.Get());
        ...
        static void AddClasses(UIWidget* widget, core::StringView classes)
        {
            ...
            for (usize i = 0; i <= classes.Size(); ++i)
            {
                ...
                if (boundary) { if (i > start) widget->AddClass(classes.SubStr(start, i - start)); start = i + 1; }
            }
        }
        ...
        [[nodiscard]] static xml::XmlElement* FirstElementChild(xml::XmlNode& node)
        {
            for (xml::XmlNode* c = node.FirstChild(); c != nullptr; c = c->NextSibling())
                if (c->NodeType() == xml::XmlNodeType::Element) return static_cast<xml::XmlElement*>(c);
            return nullptr;
        }
