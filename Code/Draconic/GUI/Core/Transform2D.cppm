...
export namespace draconic::gui
{
    struct Transform2D
    {
        ...
        constexpr Transform2D& Translate(core::Float2 offset) noexcept { return Translate(offset.x, offset.y); }
        ...
        {
            const f32 cs = core::Cos(radians), sn = core::Sin(radians);
            return Combine(Transform2D{ cs, sn, -sn, cs,
                                        centerX * (1.0f - cs) + centerY * sn,
                                        -centerX * sn + centerY * (1.0f - cs) });
        }
        Transform2D& Rotate(f32 radians, core::Float2 center) noexcept { return Rotate(radians, center.x, center.y); }

        constexpr Transform2D& Scale(f32 sx, f32 sy) noexcept
        {
            return Combine(Transform2D{ sx, 0.0f, 0.0f, sy, 0.0f, 0.0f });
        }
        constexpr Transform2D& Scale(core::Float2 factors) noexcept { return Scale(factors.x, factors.y); }
        constexpr Transform2D& Scale(f32 sx, f32 sy, f32 centerX, f32 centerY) noexcept
        {
            return Combine(Transform2D{ sx, 0.0f, 0.0f, sy, centerX * (1.0f - sx), centerY * (1.0f - sy) });
        }
        ...
        [[nodiscard]] Transform2D GetInverse() const noexcept
        {
            ...
            if (core::NearlyZero(det)) { return Transform2D::Identity(); }
            const f32 invDet = 1.0f / det;
            const f32 ia = d * invDet, ic = -c * invDet;
            const f32 ib = -b * invDet, id = a * invDet;
            return Transform2D{ ia, ib, ic, id,
                                -(ia * tx + ic * ty),
                                -(ib * tx + id * ty) };
        }
        ...
        [[nodiscard]] static constexpr Transform2D Multiply(const Transform2D& A, const Transform2D& B) noexcept
        {
            return Transform2D{
                A.a * B.a + A.c * B.b,
                A.b * B.a + A.d * B.b,
    ...
    [[nodiscard]] constexpr Transform2D operator*(const Transform2D& a, const Transform2D& b) noexcept
    {
        return Transform2D::Multiply(a, b);
    }

    [[nodiscard]] inline bool NearlyEqual(const Transform2D& x, const Transform2D& y, f32 epsilon = kEpsilon) noexcept
    {
