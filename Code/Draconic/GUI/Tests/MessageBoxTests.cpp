...
namespace
{
    ...
    core::Float2 WorldCenter(Node* node)
    {
        ...
        for (Node* c = node; c != nullptr && c->GetParent() != nullptr; c = c->GetParent()) p += c->GetPosition();
...
TEST_CASE("messagebox: Configure shows the right buttons per style")
{
    ...
    ok->Configure(core::StringView(u8"Hi"), core::StringView(u8"All good."), MessageBox::Buttons::Ok);
    CHECK(ok->ButtonCount() == 1);
    CHECK(ok->GetMessage() == core::StringView(u8"All good."));

    auto okCancel = Make<MessageBox>();
    okCancel->Configure(core::StringView(u8"?"), core::StringView(u8"Proceed?"), MessageBox::Buttons::OkCancel);
    CHECK(okCancel->ButtonCount() == 2);

    auto yesNo = Make<MessageBox>();
    yesNo->Configure(core::StringView(u8"?"), core::StringView(u8"Save?"), MessageBox::Buttons::YesNo);
...
TEST_CASE("messagebox: pressing a button reports the result and closes")
{
    ...
    box->Configure(core::StringView(u8"Confirm"), core::StringView(u8"Delete it?"), MessageBox::Buttons::OkCancel);
    box->SetOnResult([&](MessageBox::Result r) { got = r; ++results; });
