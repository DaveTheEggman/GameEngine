...
{
    enum class ScrollBarPolicy { Auto, AlwaysOn, AlwaysOff };
    ...
    protected:
        void OnChildrenChanged() override { if (OnContentChanged) OnContentChanged(); }
    };
    ...
    public:
        ...
        void SetContentSize(core::Float2 size) { m_content->SetSize(size); Relayout(); }
        ...
        void SetAutoMeasureContent(bool enabled) { m_autoMeasure = enabled; if (enabled) MeasureContent(); }
        ...
        void SetVerticalScrollBarPolicy(ScrollBarPolicy policy) { m_vPolicy = policy; Relayout(); }
        void SetHorizontalScrollBarPolicy(ScrollBarPolicy policy) { m_hPolicy = policy; Relayout(); }
        void SetScrollBarThickness(f32 thickness) { m_barThickness = thickness; Relayout(); }
    ...
    protected:
        ...
        void OnKeyDown(const KeyEvent& event) override
        {
            ...
            {
            case KeyCode::Down:     ScrollBy(core::Float2{ 0.0f, m_lineStep }); break;
            case KeyCode::Up:       ScrollBy(core::Float2{ 0.0f, -m_lineStep }); break;
            case KeyCode::Right:    ScrollBy(core::Float2{ m_lineStep, 0.0f }); break;
            case KeyCode::Left:     ScrollBy(core::Float2{ -m_lineStep, 0.0f }); break;
            case KeyCode::PageDown: ScrollBy(core::Float2{ 0.0f, vp.height }); break;
            case KeyCode::PageUp:   ScrollBy(core::Float2{ 0.0f, -vp.height }); break;
            case KeyCode::Home:     SetScrollOffset(core::Float2{ m_offset.x, 0.0f }); break;
            case KeyCode::End:      SetScrollOffset(core::Float2{ m_offset.x, ScrollRange().y }); break;
            default: break;
            }
        }
    ...
    private:
        ...
        {
            return policy == ScrollBarPolicy::AlwaysOn || (policy == ScrollBarPolicy::Auto && range > 0.0f);
        }
        ...
        void Relayout()
        {
            const Rect vp = Viewport();
            const core::Float2 cs = m_content->GetSize();
            const core::Float2 range{ core::Max(0.0f, cs.x - vp.width), core::Max(0.0f, cs.y - vp.height) };
            ...
            if (vVis)
            {
                ...
                m_vBar->SetSize(core::Float2{ m_barThickness, vp.height - (hVis ? m_barThickness : 0.0f) });
            ...
            if (hVis)
            {
                ...
                m_hBar->SetSize(core::Float2{ vp.width - (vVis ? m_barThickness : 0.0f), m_barThickness });
