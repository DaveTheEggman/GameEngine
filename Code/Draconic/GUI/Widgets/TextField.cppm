...
export namespace draconic::gui
{
    ...
    public:
        ...
        [[nodiscard]] usize SelectionStart() const noexcept { return core::Min(m_selAnchor, m_caret); }
        [[nodiscard]] usize SelectionEnd() const noexcept { return core::Max(m_selAnchor, m_caret); }
        [[nodiscard]] core::StringView SelectedText() const
        {
        ...
        void SetPlaceholder(core::StringView text) { m_placeholder = core::String(text); m_hintText.SetString(m_placeholder.AsView()); Invalidate(); }
        [[nodiscard]] core::StringView GetPlaceholder() const { return m_placeholder.AsView(); }
        void SetPlaceholderColor(Color color) { m_placeholderColor = color; m_hintText.SetColor(color); Invalidate(); }
        ...
        // === Appearance ===
        void SetFont(fonts::CachedFont* font) { m_text.SetFont(font); m_hintText.SetFont(font); Invalidate(); }
    ...
    protected:
        ...
        void OnKeyDown(const KeyEvent& event) override
        {
            ...
            if (ctrl)
            {
                ...
                case KeyCode::C: CopySelection();    return;
                case KeyCode::X: CutSelection();     return;
                case KeyCode::V: Paste();            return;
                default:                             return; // other Ctrl combos ignored
                }
            }
            ...
            case KeyCode::Backspace:
                if (HasSelection()) { DeleteSelection(); NotifyChanged(); }
                else if (m_caret > 0)
                {
            ...
            case KeyCode::Delete:
                ...
                break;
            case KeyCode::Left:
                if (shift)                 m_caret = core::Utf8PrevBoundary(m_value.AsView(), m_caret);
                else if (HasSelection())   m_caret = SelectionStart();
                else                       m_caret = core::Utf8PrevBoundary(m_value.AsView(), m_caret);
                if (!shift) m_selAnchor = m_caret;
                break;
            case KeyCode::Right:
                if (shift)                 m_caret = core::Utf8NextBoundary(m_value.AsView(), m_caret);
                else if (HasSelection())   m_caret = SelectionEnd();
                else                       m_caret = core::Utf8NextBoundary(m_value.AsView(), m_caret);
                if (!shift) m_selAnchor = m_caret;
                break;
            case KeyCode::Home:
                m_caret = 0;
        ...
        void OnMouseDown(const MouseEvent& event) override
        {
            ...
            m_caret = pos;
            if (!shift) m_selAnchor = pos;   // plain press collapses; Shift extends from anchor
        ...
        void OnFocusLost() override { m_caretVisible = false; m_selecting = false; Invalidate(); }
        ...
        void OnDraw(DrawContext& ctx, const Rect& localBounds) override
        {
            ...
            else
            {
                ...
                if (IsFocused() && HasSelection())
                {
                    ...
                    ctx.VG().FillRect(core::Rectangle{ x0, top, x1 - x0, h }, m_selectionColor);
                }
    ...
    private:
        ...
        void CopySelection()
        {
            ...
            if (IClipboard* clip = Clipboard()) clip->SetText(SelectedText());
        }
        void CutSelection()
        {
            if (!HasSelection()) return;
        ...
        void Paste()
        {
            ...
            if (clip == nullptr || !clip->HasText()) return;
            const core::String text = clip->GetText();
        ...
        void InsertText(core::StringView text)
        {
            if (HasSelection()) DeleteSelection();

            core::StringView toInsert = text;
            if (m_maxLength > 0)
            {
                const usize current = CountCodepoints(m_value.AsView());
                if (current >= m_maxLength) return;
        ...
        void SelectWordAt(usize offset)
        {
            ...
            if (view.Size() == 0) { m_selAnchor = m_caret = 0; return; }
            offset = core::Min(offset, view.Size());

            // Decide the run kind from the codepoint to the right (or left, at the very end).
            usize probe = offset;
            if (probe >= view.Size()) probe = core::Utf8PrevBoundary(view, view.Size());
            const bool wantSpace = core::IsWhiteSpace(view[probe]);

            usize start = offset;
            while (start > 0)
            {
                const usize prev = core::Utf8PrevBoundary(view, start);
                if (core::IsWhiteSpace(view[prev]) != wantSpace) break;
                start = prev;
            }
            usize end = offset;
            while (end < view.Size())
            {
                if (core::IsWhiteSpace(view[end]) != wantSpace) break;
                end = core::Utf8NextBoundary(view, end);
            }
            m_selAnchor = start;
            m_caret = end;
            Invalidate();
        ...
        static core::String SanitizeSingleLine(core::StringView text)
        {
            core::String out;
            for (usize i = 0; i < text.Size(); ++i)
            {
                ...
                if (c == u8'\n' || c == u8'\r') continue;
                out.Append(core::StringView(&c, 1));
            }
            return out;
        }

        [[nodiscard]] static usize CountCodepoints(core::StringView view)
        {
            usize count = 0;
            usize offset = 0;
            while (offset < view.Size()) { offset = core::Utf8NextBoundary(view, offset); ++count; }
            return count;
        }

        // The prefix of `text` holding at most `maxCodepoints` codepoints.
        [[nodiscard]] static core::StringView ClampCodepoints(core::StringView text, usize maxCodepoints)
        {
            usize offset = 0;
            usize count = 0;
            while (offset < text.Size() && count < maxCodepoints) { offset = core::Utf8NextBoundary(text, offset); ++count; }
