...
export namespace draconic::gui
{
    ...
    public:
        ...
        void SetOnPicked(core::Function<void(i32, u32)> callback) { m_onPicked = core::Move(callback); }

    protected:
        void OnMouseClick(const MouseEvent& event) override { if (m_onPicked) m_onPicked(m_item, event.Modifiers); }
    ...
    public:
        ...
        ~AbstractItemView() override { if (m_model != nullptr) m_model->RemoveClient(this); }
        ...
        void SetSelectionColor(Color color) { m_selectionColor = color; for (const RefPtr<ItemRow>& r : m_pool) r->SetSelectionColor(color); }
        void SetThemePartColor(core::StringView part, Color color) override { if (part == core::StringView(u8"selection")) SetSelectionColor(color); }
        ...
        [[nodiscard]] i32 GetSelectedItem() const noexcept { return m_selection.Size() > 0 ? m_selection[m_selection.Size() - 1] : -1; }
        [[nodiscard]] ModelIndex GetSelectedIndex() const { return GetSelectedItem() >= 0 ? ItemToModelIndex(GetSelectedItem()) : ModelIndex{}; }
        void SetSelectedItem(i32 item) { SelectSingle(item, /*notify*/ true, /*scroll*/ true); }
        void ClearSelection() { if (m_selection.Size() == 0) return; m_selection.Clear(); m_anchor = -1; Relayout(); }
        [[nodiscard]] bool IsItemSelected(i32 item) const
        {
            for (usize i = 0; i < m_selection.Size(); ++i) if (m_selection[i] == item) return true;
            return false;
        }
    ...
    protected:
        ...
        [[nodiscard]] virtual usize ItemCount() const { return m_model != nullptr ? m_model->RowCount() : 0; }
        [[nodiscard]] virtual f32 ContentTopInset() const { return 0.0f; }         // e.g. a header
        [[nodiscard]] virtual ModelIndex ItemToModelIndex(i32 item) const { return MakeModelIndex(item); }
        ...
        virtual void OnRowClicked(i32 item, u32 modifiers) { HandleSelectionClick(item, modifiers); }

        // === Protected helpers for subclasses ===
        [[nodiscard]] f32 ContentWidth() const noexcept { return m_contentWidth; }
        [[nodiscard]] f32 BodyHeight() const { return core::Max(0.0f, GetSize().y - ContentTopInset()); }
        ...
        void OnKeyDown(const KeyEvent& event) override
        {
            ...
            switch (key)
            {
            case KeyCode::Up:       target = (primary <= 0) ? 0 : primary - 1; break;
            case KeyCode::Down:     target = (primary < 0) ? 0 : core::Min(items - 1, primary + 1); break;
            case KeyCode::Home:     target = 0; break;
            case KeyCode::End:      target = items - 1; break;
            case KeyCode::PageUp:   target = core::Max(0, (primary < 0 ? 0 : primary) - page); break;
            case KeyCode::PageDown: target = core::Min(items - 1, (primary < 0 ? 0 : primary) + page); break;
            default: return;
            }
            if (shift && m_selectionMode == SelectionMode::Multi) SelectRange(target, /*scroll*/ true);
            else                                                  SelectSingle(target, /*notify*/ true, /*scroll*/ true);
        }
        ...
        void HandleSelectionClick(i32 item, u32 modifiers)
        {
            ...
            if (m_selectionMode == SelectionMode::Multi && shift && m_anchor >= 0) { SelectRange(item, /*scroll*/ false); return; }
            if (m_selectionMode == SelectionMode::Multi && ctrl) { ToggleItem(item); return; }
            SelectSingle(item, /*notify*/ true, /*scroll*/ false);
        }

    private:
        [[nodiscard]] f32 ContentHeight() const { return static_cast<f32>(ItemCount()) * m_rowHeight; }
        ...
        void ToggleItem(i32 item)
        {
            ...
            for (usize i = 0; i < m_selection.Size(); ++i)
                if (m_selection[i] == item) { m_selection.RemoveAt(i); m_anchor = item; Relayout(); NotifySelectionChanged(); return; }
            m_selection.PushBack(item);
            m_anchor = item;
            Relayout();
        ...
        void SelectRange(i32 item, bool scroll)
        {
            ...
            m_selection.Clear();
            for (i32 r = lo; r <= hi; ++r) m_selection.PushBack(r);
            // anchor stays put; `item` becomes the primary (kept last)
            if (item != hi) { /* selection already includes item; ensure it's primary */ }
            if (scroll) ScrollItemIntoView(item);
            Relayout();
            NotifySelectionChanged();
        }
        void ClampSelection()
        {
            ...
            for (usize i = m_selection.Size(); i-- > 0;)
                if (m_selection[i] >= items) m_selection.RemoveAt(i);
            if (m_anchor >= items) m_anchor = -1;
        }
        ...
        void EnsurePool(usize needed)
        {
            while (m_pool.Size() < needed)
            {
                ...
                AbstractItemView* self = this;
                row->SetOnPicked([self](i32 item, u32 modifiers) { self->OnRowClicked(item, modifiers); });
                AddChild(row.Get());
        ...
        void Relayout()
        {
            ...
            for (i32 r = first; r < last; ++r, ++p)
            {
                ...
                row->SetPosition(core::Float2{ 0.0f, inset + static_cast<f32>(r) * m_rowHeight - m_offset });
