...
export namespace draconic::gui
{
    ...
    public:
        ...
        void SetFont(fonts::CachedFont* font)
        {
            m_font = font;
            m_text.SetFont(font);
            if (m_dropdown) m_dropdown->SetFont(font);
        }
        ...
        void AddItem(core::StringView text)
        {
            ...
            if (m_dropdown) m_dropdown->AddItem(text);
        }
        ...
        [[nodiscard]] core::StringView GetSelectedText() const
        {
            return (m_selected >= 0 && static_cast<usize>(m_selected) < m_items.Size())
                ? m_items[static_cast<usize>(m_selected)].AsView() : core::StringView{};
        }
    ...
    protected:
        void OnMouseClick(const MouseEvent&) override { if (m_open) CloseDropdown(); else OpenDropdown(); }
    ...
    private:
        void BuildDropdown()
        {
            ...
            for (const core::String& item : m_items) m_dropdown->AddItem(item.AsView());
            ComboBox* self = this;
        ...
        void OpenDropdown()
        {
            ...
            if (root == nullptr || dispatcher == nullptr || m_items.IsEmpty()) return;
            BuildDropdown();

            const core::Float2 below = ConvertToWorldSpace(core::Float2{ 0.0f, GetSize().y });
            const f32 h = core::Min(static_cast<f32>(m_items.Size()) * m_itemHeight, m_dropdownMaxHeight);
        ...
        void CloseDropdown()
        {
            if (!m_open) return;
            m_open = false;
            if (m_dropdown) m_dropdown->RemoveFromParent();
        }

        void OnPicked(i32 index)
        {
            SetSelectedIndex(index);
            if (EventDispatcher* dispatcher = GetEventDispatcher()) dispatcher->ClosePopup();
            else CloseDropdown();
        }
