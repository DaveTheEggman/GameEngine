...
export namespace draconic::gui
{
    ...
    public:
        ...
        i32 AddNode(i32 parentId, core::StringView text)
        {
            ...
            if (parentId == kRoot) m_roots.PushBack(id);
            else                   m_nodes[static_cast<usize>(parentId)].Children.PushBack(id);
            return id;
        }
        void Clear() { m_nodes.Clear(); m_roots.Clear(); DidUpdate(); }
        void Refresh() { DidUpdate(); } // call after a batch of AddNode

        [[nodiscard]] core::StringView TextOf(i32 nodeId) const
        {
            return (nodeId >= 0 && static_cast<usize>(nodeId) < m_nodes.Size()) ? m_nodes[static_cast<usize>(nodeId)].Text.AsView() : core::StringView{};
        }
        ...
        {
            if (!parent.IsValid()) return m_roots.Size();
            const i32 id = static_cast<i32>(parent.InternalId);
            return (id >= 0 && static_cast<usize>(id) < m_nodes.Size()) ? m_nodes[static_cast<usize>(id)].Children.Size() : 0;
        }

        [[nodiscard]] ModelIndex Index(i32 row, i32 column = 0, const ModelIndex& parent = {}) const override
        {
            const i32 childId = ChildIdAt(parent, row);
            if (childId < 0) return ModelIndex{};
        ...
        [[nodiscard]] ModelIndex ParentIndex(const ModelIndex& child) const override
        {
            ...
            if (id < 0 || static_cast<usize>(id) >= m_nodes.Size()) return ModelIndex{};
            const i32 parentId = m_nodes[static_cast<usize>(id)].Parent;
            if (parentId == kRoot) return ModelIndex{}; // top-level: no parent
            return MakeModelIndex(RowOf(parentId), 0, parentId);
        }

        [[nodiscard]] bool HasChildren(const ModelIndex& parent = {}) const override { return RowCount(parent) > 0; }
        ...
        [[nodiscard]] Variant Data(const ModelIndex& index, ModelRole role = ModelRole::Display) const override
        {
            ...
            if (id < 0 || static_cast<usize>(id) >= m_nodes.Size()) return Variant{};
            if (role == ModelRole::Display || role == ModelRole::Sort) return Variant(m_nodes[static_cast<usize>(id)].Text.AsView());
            return Variant{};
        }

    private:
        struct Node { core::String Text; i32 Parent = kRoot; Array<i32> Children; };

        [[nodiscard]] i32 ChildIdAt(const ModelIndex& parent, i32 row) const
        {
            if (!parent.IsValid())
                return (row >= 0 && static_cast<usize>(row) < m_roots.Size()) ? m_roots[static_cast<usize>(row)] : -1;
            const i32 id = static_cast<i32>(parent.InternalId);
            if (id < 0 || static_cast<usize>(id) >= m_nodes.Size()) return -1;
            const Array<i32>& children = m_nodes[static_cast<usize>(id)].Children;
            return (row >= 0 && static_cast<usize>(row) < children.Size()) ? children[static_cast<usize>(row)] : -1;
        }

        // The row index of `nodeId` within its parent's child list (or root list).
        [[nodiscard]] i32 RowOf(i32 nodeId) const
        {
            if (nodeId < 0 || static_cast<usize>(nodeId) >= m_nodes.Size()) return -1;
            const i32 parentId = m_nodes[static_cast<usize>(nodeId)].Parent;
            const Array<i32>& siblings = (parentId == kRoot) ? m_roots : m_nodes[static_cast<usize>(parentId)].Children;
            for (usize i = 0; i < siblings.Size(); ++i) if (siblings[i] == nodeId) return static_cast<i32>(i);
            return -1;
        }
