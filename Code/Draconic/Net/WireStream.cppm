...
public:
    ...
    void WriteU64(u64 v) { WriteBits(static_cast<u32>(v & 0xFFFFFFFFull), 32); WriteBits(static_cast<u32>(v >> 32), 32); }
    void WriteI32(i32 v) { WriteU32(static_cast<u32>(v)); }
    void WriteFloat(f32 v) { u32 bits = 0; MemCopy(&bits, &v, sizeof(bits)); WriteU32(bits); }

    // Variable-length u32 (LEB128, 7 bits/byte): small values cost 1 byte. Ideal for lengths/ids.
    void WriteVarU32(u32 v) {
        while (v >= 0x80u) { WriteBits((v & 0x7Fu) | 0x80u, 8); v >>= 7; }
        WriteBits(v, 8);
    }
    ...
    void WriteBytes(Span<const byte> data) {
        for (byte b : data) { WriteBits(static_cast<u32>(static_cast<u8>(b)), 8); }
    }

    // The finished buffer (flushes a partial trailing byte, zero-padded). Idempotent.
    [[nodiscard]] Span<const byte> Data() { Flush(); return Span<const byte>(m_bytes.Data(), m_bytes.Size()); }
    [[nodiscard]] usize ByteCount() { Flush(); return m_bytes.Size(); }
    // Bits written so far (before flush padding).
    [[nodiscard]] usize BitCount() const noexcept { return m_bytes.Size() * 8u + m_scratchBits; }

    void Reset() { m_bytes.Clear(); m_scratch = 0; m_scratchBits = 0; }
...
public:
    ...
    u32 ReadBits(u32 bits) {
        ...
        while (got < bits) {
            if (m_scratchBits == 0) {
                if (m_bytePos >= m_data.Size()) { m_overflow = true; return result; }
    ...
    u64 ReadU64() { const u64 lo = ReadBits(32); return lo | (static_cast<u64>(ReadBits(32)) << 32); }
    i32 ReadI32() { return static_cast<i32>(ReadU32()); }
    f32 ReadFloat() { const u32 bits = ReadU32(); f32 v = 0.0f; MemCopy(&v, &bits, sizeof(v)); return v; }
    ...
    void ReadBytes(Span<byte> out) {
        for (byte& b : out) { b = static_cast<byte>(ReadU8()); }
    }
    ...
    [[nodiscard]] bool AtEnd() const noexcept { return m_bytePos >= m_data.Size() && m_scratchBits == 0; }
