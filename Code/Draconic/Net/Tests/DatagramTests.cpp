...
{
    net::SimConditions sim; sim.latencyMs = 25.0f;
    ...
    net::DatagramEndpoint from; Array<byte> got;
...
{
    net::SimConditions sim; sim.lossPct = 0.5f; sim.seed = 999;
    net::SimDatagramNetwork network(sim);
    net::IDatagramSocket* a = network.CreateSocket();
    net::IDatagramSocket* b = network.CreateSocket();

    for (int i = 0; i < 100; ++i) { Array<byte> m = Bytes({ static_cast<u8>(i) }); a->Send(b->LocalEndpoint(), m.AsSpan()); }
    network.Advance(1.0f);

    usize received = 0; net::DatagramEndpoint from; Array<byte> got;
