...
namespace {
    ...
    Array<net::NetEvent> Drain(net::INetTransport& t) {
        Array<net::NetEvent> out; net::NetEvent ev;
        while (t.Poll(ev)) { out.PushBack(static_cast<net::NetEvent&&>(ev)); }
        return out;
    }
}
...
{
    net::SimConditions sim; sim.latencyMs = 50.0f;
    net::LoopbackLink link(sim);
    (void)Drain(link.A()); (void)Drain(link.B());   // clear Connected

    const Array<byte> msg = Bytes({ 0xDE, 0xAD, 0xBE, 0xEF });
    link.A().Send(net::LoopbackLink::kRemotePeer, 0, msg.AsSpan(), net::Reliability::ReliableOrdered);
...
{
    net::SimConditions sim; sim.lossPct = 1.0f;   // drop everything droppable
    net::LoopbackLink link(sim);
    (void)Drain(link.A()); (void)Drain(link.B());
...
{
    net::SimConditions sim; sim.lossPct = 0.5f; sim.seed = 42;
    net::LoopbackLink link(sim);
    (void)Drain(link.A()); (void)Drain(link.B());
    ...
    (void)Drain(link2.A()); (void)Drain(link2.B());
    for (int i = 0; i < 100; ++i) {
...
{
    net::SimConditions sim; sim.latencyMs = 20.0f; sim.reorderPct = 1.0f; sim.seed = 7;
    net::LoopbackLink link(sim);
    (void)Drain(link.A()); (void)Drain(link.B());

    for (u8 i = 0; i < 8; ++i) {
        link.A().Send(net::LoopbackLink::kRemotePeer, 0, Bytes({ i }).AsSpan(), net::Reliability::Unreliable);
    }
    ...
    for (usize i = 1; i < got.Size(); ++i) {
        if (static_cast<u8>(got[i].payload[0]) < static_cast<u8>(got[i - 1].payload[0])) { anyOutOfOrder = true; break; }
    }
    CHECK(anyOutOfOrder);
}
...
{
    net::SimConditions sim; sim.latencyMs = 20.0f; sim.reorderPct = 1.0f; sim.dupPct = 1.0f; sim.seed = 7;
    net::LoopbackLink link(sim);
    (void)Drain(link.A()); (void)Drain(link.B());

    for (u8 i = 0; i < 8; ++i) {
        link.A().Send(net::LoopbackLink::kRemotePeer, 0, Bytes({ i }).AsSpan(), net::Reliability::ReliableOrdered);
    }
    link.Advance(1000.0f);
    Array<net::NetEvent> got = Drain(link.B());
    REQUIRE(got.Size() == 8u);            // no dup, no drop
    for (u8 i = 0; i < 8; ++i) { CHECK(static_cast<u8>(got[i].payload[0]) == i); }  // strictly in order
}
...
TEST_CASE("transport: Update on an endpoint advances the shared clock")
{
    ...
    net::LoopbackLink link(sim);
    (void)Drain(link.A()); (void)Drain(link.B());
    link.A().Send(net::LoopbackLink::kRemotePeer, 0, Bytes({ 1 }).AsSpan(), net::Reliability::ReliableOrdered);
