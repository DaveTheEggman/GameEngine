...
namespace {
    ...
    struct Pair {
        ...
        void Step() { network.Advance(10.0f); server.Update(10.0f); client.Update(10.0f); }
        void Pump(int n) { for (int i = 0; i < n; ++i) Step(); }
...
{
    net::SimConditions sim; sim.latencyMs = 20.0f; sim.lossPct = 0.2f; sim.seed = 5;
    Pair fx(sim);
    fx.Pump(20);   // connect

    net::PeerId gotSender = net::kInvalidPeer;
    u32 gotA = 0; f32 gotB = 0.0f;
    net::RpcTable serverRpc;
    serverRpc.On(u8"order", [&](net::PeerId sender, net::BitReader& args) {
        gotSender = sender;
        gotA = args.ReadU32();
        gotB = args.ReadFloat();
    });
    ...
    clientRpc.Call(fx.client, fx.serverPeer, u8"order", [](net::BitWriter& w) { w.WriteU32(1337u); w.WriteFloat(2.5f); });
...
{
    net::SimConditions sim; sim.latencyMs = 15.0f;
    ...
    for (int i = 0; i < 40; ++i) { network.Advance(10.0f); server.Update(10.0f); a.Update(10.0f); b.Update(10.0f); }
    REQUIRE(server.PeerCount() == 2u);

    int aHits = 0, bHits = 0;
    net::RpcTable ra, rb;
    ra.On(u8"tick", [&](net::PeerId, net::BitReader& args) { CHECK(args.ReadU32() == 7u); ++aHits; });
    rb.On(u8"tick", [&](net::PeerId, net::BitReader& args) { CHECK(args.ReadU32() == 7u); ++bHits; });
    ...
    for (int i = 0; i < 120 && (aHits == 0 || bHits == 0); ++i) {
        network.Advance(10.0f); server.Update(10.0f); a.Update(10.0f); b.Update(10.0f);
        ra.Pump(a); rb.Pump(b);
    }
    CHECK(aHits == 1);
    CHECK(bHits == 1);
...
TEST_CASE("rpc: Pump forwards non-RPC events to onOther")
{
    ...
    const u8 raw[] = { 0xAB };
    fx.client.Send(fx.serverPeer, 0, Span<const byte>(reinterpret_cast<const byte*>(raw), 1), net::Reliability::ReliableOrdered);

    net::RpcTable serverRpc;
    bool sawRaw = false, sawConnected = false;
    for (int i = 0; i < 60 && !sawRaw; ++i) {
        fx.Step();
        serverRpc.Pump(fx.server, [&](const net::NetEvent& ev) {
            if (ev.kind == net::NetEventKind::Connected) { sawConnected = true; }
            else if (ev.kind == net::NetEventKind::Received && ev.channel == 0 && ev.payload.Size() == 1
                     && static_cast<u8>(ev.payload[0]) == 0xABu) { sawRaw = true; }
        });
    }
    CHECK(sawConnected);
    CHECK(sawRaw);
