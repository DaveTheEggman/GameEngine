...
TEST_CASE("udp: two sockets bind distinct OS ports and exchange a raw datagram over localhost")
{
    ...
    for (int i = 0; i < 100 && !received; ++i) {
        if (b.Receive(from, got)) { received = true; break; }
        SleepMilliseconds(1);
    }
    REQUIRE(received);
    REQUIRE(got.Size() == 2u);
...
TEST_CASE("udp: reliable transport handshakes + delivers a reliable message over real sockets")
{
    ...
    const auto pump = [&](int n) {
        for (int i = 0; i < n; ++i) { client.Update(10.0f); server.Update(10.0f); SleepMilliseconds(1); }
    };
    pump(30);   // handshake over the OS stack

    bool serverConnected = false;
    { net::NetEvent ev; while (server.Poll(ev)) { if (ev.kind == net::NetEventKind::Connected) serverConnected = true; } }
