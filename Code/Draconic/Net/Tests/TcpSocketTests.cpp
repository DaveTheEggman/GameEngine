...
TEST_CASE("tcp: connect + accept + bidirectional stream over localhost")
{
    ...
    for (int i = 0; i < 300 && !(server.IsOpen() && client.ConnectStatus() == 1); ++i) {
        if (!server.IsOpen()) {
            ...
            if (s.IsOpen()) { server = static_cast<net::TcpSocket&&>(s); }
        }
        SleepMilliseconds(1);
    }
    ...
    byte buf[64] = {};
    i64 got = 0;
    for (int i = 0; i < 300 && got <= 0; ++i) { got = server.Receive(Span<byte>(buf, sizeof(buf))); if (got <= 0) SleepMilliseconds(1); }
    REQUIRE(got == 4);
    ...
    byte rbuf[64] = {};
    i64 back = 0;
    for (int i = 0; i < 300 && back <= 0; ++i) { back = client.Receive(Span<byte>(rbuf, sizeof(rbuf))); if (back <= 0) SleepMilliseconds(1); }
    REQUIRE(back == 4);
    ...
    client.Close();
    i64 afterClose = 0;
    for (int i = 0; i < 300; ++i) { afterClose = server.Receive(Span<byte>(rbuf, sizeof(rbuf))); if (afterClose != 0) break; SleepMilliseconds(1); }
