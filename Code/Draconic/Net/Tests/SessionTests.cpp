...
namespace {
    ...
    Array<u8> DrainReceived(net::NetSession& s) {
        ...
        while (s.PollEvent(ev)) { if (ev.kind == net::NetEventKind::Received && ev.payload.Size() > 0) { out.PushBack(static_cast<u8>(ev.payload[0])); } }
        return out;
    }
}
...
TEST_CASE("session: server tracks connected clients; disconnect updates the registry")
{
    ...
    for (int i = 0; i < 40; ++i) { network.Advance(10.0f); server.Update(10.0f); clientA.Update(10.0f); clientB.Update(10.0f); }
    ...
    for (int i = 0; i < 20; ++i) { network.Advance(10.0f); server.Update(10.0f); clientB.Update(10.0f); }
    CHECK(server.PeerCount() == 1u);
}

TEST_CASE("session: client clock syncs to the server's authoritative time (RTT-adjusted)")
{
    net::SimConditions sim; sim.latencyMs = 40.0f;   // ~80ms RTT
    net::SimDatagramNetwork network(sim);
    net::IDatagramSocket* sv = network.CreateSocket();
    net::NetSession server(*sv);
    net::NetSession client(*network.CreateSocket());
    ...
    for (int i = 0; i < 200; ++i) { network.Advance(10.0f); server.Update(10.0f); client.Update(10.0f); }
    ...
    const long long tickDiff = static_cast<long long>(server.NetworkTick()) - static_cast<long long>(client.NetworkTick());
    CHECK(tickDiff <= 1);
    CHECK(tickDiff >= -1);
}
...
TEST_CASE("session: Broadcast reaches every client; BroadcastExcept skips one")
{
    ...
    (void)DrainReceived(clientA); (void)DrainReceived(clientB);
    ...
    Array<u8> gotA, gotB;
    for (int i = 0; i < 80; ++i) {
        network.Advance(10.0f); server.Update(10.0f); clientA.Update(10.0f); clientB.Update(10.0f);
        for (u8 b : DrainReceived(clientA)) gotA.PushBack(b);
        for (u8 b : DrainReceived(clientB)) gotB.PushBack(b);
    }
    // Both got the broadcast (0x99); exactly one of them (the non-excepted) also got 0x77.
    const auto has = [](const Array<u8>& a, u8 v) { for (u8 x : a) if (x == v) return true; return false; };
