...
struct DatagramEndpoint {
    u64 value = 0;
    [[nodiscard]] bool operator==(const DatagramEndpoint& o) const noexcept { return value == o.value; }
    [[nodiscard]] bool operator!=(const DatagramEndpoint& o) const noexcept { return value != o.value; }
...
public:
    ...
    [[nodiscard]] bool Receive(DatagramEndpoint& from, Array<byte>& out) override {
        if (m_recvHead >= m_recv.Size()) { return false; }
        from = m_recv[m_recvHead].from;
        out = static_cast<Array<byte>&&>(m_recv[m_recvHead].data);
        ++m_recvHead;
        if (m_recvHead >= m_recv.Size()) { m_recv.Clear(); m_recvHead = 0; }
        return true;
    }
...
private:
    ...
    struct Received { DatagramEndpoint from; Array<byte> data; };
...
private:
    ...
    struct InFlight { DatagramEndpoint from; DatagramEndpoint to; Array<byte> data; f64 deliverAt = 0.0; u64 order = 0; };

    [[nodiscard]] SimDatagramSocket* Find(const DatagramEndpoint& ep) {
        for (const UniquePtr<SimDatagramSocket>& s : m_sockets) { if (s->m_endpoint == ep) { return s.Get(); } }
        return nullptr;
    }
    ...
    void Route(const DatagramEndpoint& from, const DatagramEndpoint& to, Span<const byte> data) {
        ...
        if (Chance(m_sim.lossPct)) { return; }
        f64 deliverAt = m_nowMs + static_cast<f64>(Max(0.0f, m_sim.latencyMs + Jitter()));
        if (Chance(m_sim.reorderPct)) { deliverAt += static_cast<f64>(m_rng.NextFloat() * Max(1.0f, m_sim.latencyMs)); }
        Enqueue(from, to, data, deliverAt);
        if (Chance(m_sim.dupPct)) { Enqueue(from, to, data, deliverAt + static_cast<f64>(m_rng.NextFloat())); }
    }

    void Enqueue(const DatagramEndpoint& from, const DatagramEndpoint& to, Span<const byte> data, f64 deliverAt) {
        InFlight pkt;
        pkt.from = from; pkt.to = to;
        pkt.data.Resize(data.Size());
        if (data.Size() > 0) { MemCopy(pkt.data.Data(), data.Data(), data.Size()); }
        pkt.deliverAt = deliverAt;
        pkt.order = m_orderCounter++;
        usize i = m_inflight.Size();
        while (i > 0) {
            const InFlight& prev = m_inflight[i - 1];
            if (prev.deliverAt < deliverAt || (prev.deliverAt == deliverAt && prev.order < pkt.order)) { break; }
            --i;
        }
    ...
    [[nodiscard]] f32 Jitter() { return (m_sim.jitterMs > 0.0f) ? (m_rng.NextFloat() * 2.0f - 1.0f) * m_sim.jitterMs : 0.0f; }
...
inline void SimDatagramSocket::Send(const DatagramEndpoint& to, Span<const byte> data) {
    if (m_net != nullptr) { m_net->Route(m_endpoint, to, data); }
}
