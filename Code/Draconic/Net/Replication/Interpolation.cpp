...
namespace {
    ...
    [[nodiscard]] Quaternion NLerp(const Quaternion& a, const Quaternion& b, f32 t) noexcept {
        ...
        Quaternion r{ Lerp(a.x, s * b.x, t), Lerp(a.y, s * b.y, t),
                      Lerp(a.z, s * b.z, t), Lerp(a.w, s * b.w, t) };
...
bool IsInterpolatableType(const TypeInfo* type) noexcept {
    return type == &TypeOf<f32>()  || type == &TypeOf<f64>()
...
Variant LerpFieldValue(const Variant& a, const Variant& b, f32 t) {
    if (const f32* x = a.TryGet<f32>()) { const f32* y = b.TryGet<f32>(); return Variant::From<f32>(Lerp(*x, y ? *y : *x, t)); }
    if (const f64* x = a.TryGet<f64>()) { const f64* y = b.TryGet<f64>(); const f64 yv = y ? *y : *x; return Variant::From<f64>(*x + (yv - *x) * static_cast<f64>(t)); }
    if (const Float2* x = a.TryGet<Float2>()) { const Float2* y = b.TryGet<Float2>(); const Float2 yy = y ? *y : *x; return Variant::From<Float2>(Float2{ Lerp(x->x, yy.x, t), Lerp(x->y, yy.y, t) }); }
    if (const Float3* x = a.TryGet<Float3>()) { const Float3* y = b.TryGet<Float3>(); const Float3 yy = y ? *y : *x; return Variant::From<Float3>(Float3{ Lerp(x->x, yy.x, t), Lerp(x->y, yy.y, t), Lerp(x->z, yy.z, t) }); }
    if (const Float4* x = a.TryGet<Float4>()) { const Float4* y = b.TryGet<Float4>(); const Float4 yy = y ? *y : *x; return Variant::From<Float4>(Float4{ Lerp(x->x, yy.x, t), Lerp(x->y, yy.y, t), Lerp(x->z, yy.z, t), Lerp(x->w, yy.w, t) }); }
    if (const Quaternion* x = a.TryGet<Quaternion>()) { const Quaternion* y = b.TryGet<Quaternion>(); return Variant::From<Quaternion>(NLerp(*x, y ? *y : *x, t)); }
...
{
    if (component.Type() == nullptr) { return; }
    if (m_entities.Find(id.value) == nullptr) { m_entities.InsertOrAssign(id.value, HashMap<u32, Timeline>{}); }
    HashMap<u32, Timeline>& byType = *m_entities.Find(id.value);
    if (byType.Find(componentTypeHash) == nullptr) { byType.InsertOrAssign(componentTypeHash, Timeline{}); }
