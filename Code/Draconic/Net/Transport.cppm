...
enum class NetEventKind : u8 { Connected, Disconnected, Received };
...
public:
    ...
    virtual void Send(PeerId peer, u8 channel, Span<const byte> data, Reliability reliability) = 0;
...
public:
    explicit LoopbackLink(const SimConditions& sim = {}) : m_sim(sim), m_rng(sim.seed) {
        m_a.m_link = this; m_a.m_side = 0;
        m_b.m_link = this; m_b.m_side = 1;
...
private:
    ...
    static NetEvent MakeEvent(NetEventKind kind, PeerId peer, u8 channel, Array<byte>&& payload) {
        ...
        ev.payload = static_cast<Array<byte>&&>(payload); return ev;
    }
    ...
    void Submit(u32 srcSide, u8 channel, Span<const byte> data, Reliability reliability) {
        ...
        if (unreliable && Chance(m_sim.lossPct)) { return; }
...
inline void LoopbackEndpoint::Send(PeerId peer, u8 channel, Span<const byte> data, Reliability reliability) {
    (void)peer;   // a pair has one remote
    if (m_link != nullptr) { m_link->Submit(m_side, channel, data, reliability); }
}
inline void LoopbackEndpoint::Disconnect(PeerId peer) {
    (void)peer;
    if (m_link != nullptr) {
        const u32 dst = (m_side == 0) ? 1u : 0u;
        m_link->Deliver(dst, LoopbackLink::MakeEvent(NetEventKind::Disconnected, LoopbackLink::kRemotePeer, 0, {}));
    }
}
inline void LoopbackEndpoint::Update(f32 deltaMs) {
    if (m_link != nullptr) { m_link->Advance(deltaMs); }
}
inline TransportStats LoopbackEndpoint::Stats(PeerId peer) const {
    (void)peer;
    TransportStats s = m_stats;
    if (m_link != nullptr) { s.rttMs = 2.0f * m_link->Sim().latencyMs; }
    return s;
}
