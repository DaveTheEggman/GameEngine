...
enum class NetRole : u8 { None, Client, ListenServer, DedicatedServer };
...
public:
    ...
    void Broadcast(u8 channel, Span<const byte> data, Reliability reliability) {
        for (const NetPeer& p : m_peers) { m_transport.Send(p.id, channel, data, reliability); }
    }
    // Server: send to every connected peer EXCEPT `except` (e.g. echo an order to everyone but its author).
    void BroadcastExcept(PeerId except, u8 channel, Span<const byte> data, Reliability reliability) {
        for (const NetPeer& p : m_peers) { if (p.id != except) { m_transport.Send(p.id, channel, data, reliability); } }
    }
    ...
    void Update(f32 deltaMs) {
        ...
        while (m_transport.Poll(ev)) {
            if (ev.kind == NetEventKind::Connected) { AddPeer(ev.peer); }
            else if (ev.kind == NetEventKind::Disconnected) { RemovePeer(ev.peer); }
            else if (ev.kind == NetEventKind::Received && ev.channel == kControlChannel) { HandleControl(ev); continue; }
            m_events.PushBack(static_cast<NetEvent&&>(ev));
        }
        // Server: broadcast the authoritative clock so clients can sync their tick to it.
        if (IsServer() && !m_peers.IsEmpty() && (m_nowMs - m_lastSyncSendMs) >= static_cast<f64>(m_syncIntervalMs)) {
            m_lastSyncSendMs = m_nowMs;
            BitWriter w;
            w.WriteU8(static_cast<u8>(ControlType::TimeSync));
            u64 bits = 0; MemCopy(&bits, &m_nowMs, sizeof(bits)); w.WriteU64(bits);   // server time (ms), bit-exact
            const Span<const byte> payload = w.Data();
            Broadcast(kControlChannel, payload, Reliability::Unreliable);   // frequent, latest-wins
        }
    }

    // --- clock / tick sync (server-authoritative) ---
    void SetFixedStepMs(f32 stepMs) noexcept { m_fixedStepMs = (stepMs > 0.0f) ? stepMs : 1.0f; }
    ...
    [[nodiscard]] f64 NetworkTimeMs() const noexcept { return IsServer() ? m_nowMs : (m_nowMs + m_timeOffsetMs); }
    ...
    [[nodiscard]] bool IsServer() const noexcept { return m_role == NetRole::ListenServer || m_role == NetRole::DedicatedServer; }
...
private:
    enum class ControlType : u8 { TimeSync = 0 };

    void AddPeer(PeerId id) {
        for (const NetPeer& p : m_peers) { if (p.id == id) { return; } }
        m_peers.PushBack(NetPeer{ id, m_nowMs });
    }
    void RemovePeer(PeerId id) {
        for (usize i = 0; i < m_peers.Size(); ++i) { if (m_peers[i].id == id) { m_peers.RemoveAt(i); return; } }
    }
    ...
    void HandleControl(const NetEvent& ev) {
        ...
        if (static_cast<ControlType>(type) == ControlType::TimeSync && !IsServer()) {
            ...
            if (!r.Ok()) { return; }
            f64 serverTime = 0.0; MemCopy(&serverTime, &bits, sizeof(serverTime));
            ...
            const f64 sample = estimatedServerNow - m_nowMs;   // offset: server clock vs our local clock
