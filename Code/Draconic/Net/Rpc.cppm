...
public:
    ...
    void On(StringView name, Handler handler) { m_handlers.InsertOrAssign(Hash(name), static_cast<Handler&&>(handler)); }
    ...
    [[nodiscard]] static bool IsRpcChannel(u8 channel) noexcept { return channel == kRpcChannel; }
    ...
    void Dispatch(PeerId sender, Span<const byte> payload) {
        ...
        if (!r.Ok()) { return; }
        if (Handler* h = m_handlers.Find(id)) { (*h)(sender, r); }
    }
    ...
    void Pump(NetSession& session, const Function<void(const NetEvent&)>& onOther = {}) {
        ...
        while (session.PollEvent(ev)) {
            if (ev.kind == NetEventKind::Received && IsRpcChannel(ev.channel)) { Dispatch(ev.peer, ev.payload.AsSpan()); }
            else if (onOther) { onOther(ev); }
        }
    }
    ...
    [[nodiscard]] static u32 Hash(StringView name) noexcept {
        u32 h = 2166136261u;
        for (usize i = 0; i < name.Size(); ++i) { h ^= static_cast<u8>(name[i]); h *= 16777619u; }
        return h;
    }
