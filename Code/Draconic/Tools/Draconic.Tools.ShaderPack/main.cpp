// RaptorShaderPackCook - a headless CLI that cooks the engine shader corpus into a CookedShaderPack
...
//
//   RaptorShaderPackCook <shaderDir> <output.dpak> [wgsl] [spirv] [dxil]
//
...
namespace
{
    ...
    {
        std::fprintf(stderr,
                     "usage: RaptorShaderPackCook <shaderDir> <output.dpak> [wgsl] [spirv] [dxil]\n");
    }
}
...
int main(int argc, char** argv)
{
    ...
    for (int i = 3; i < argc; ++i)
    {
        ...
        {
            std::fprintf(stderr, "RaptorShaderPackCook: unknown format '%s'\n", argv[i]);
            PrintUsage();
            return 2;
        }
    }
    ...
    {
        std::fprintf(stderr, "RaptorShaderPackCook: DXC runtime unavailable\n");
        return 1;
    }
    ...
    std::fprintf(stderr, "RaptorShaderPackCook: files=%u variants=%u success=%d\n",
    ...
    {
        std::fprintf(stderr, "RaptorShaderPackCook: could not write '%s'\n", argv[2]);
        return 1;
    }
    std::fprintf(stderr, "RaptorShaderPackCook: wrote '%s' (%u entries)\n", argv[2],
