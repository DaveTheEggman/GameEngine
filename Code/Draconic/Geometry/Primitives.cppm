...
public:
    ...
    [[nodiscard]] static RefPtr<StaticMesh> Quad(f32 width = 1.0f, f32 height = 1.0f) {
        ...
        mesh->vertices.PushBack(StaticMeshVertex{ Float3{ -hw, -hh, 0 }, Float3{ 0, 0, 1 }, Float2{ 0, 1 }, white, Float3{ 1, 0, 0 } });
        mesh->vertices.PushBack(StaticMeshVertex{ Float3{  hw, -hh, 0 }, Float3{ 0, 0, 1 }, Float2{ 1, 1 }, white, Float3{ 1, 0, 0 } });
        mesh->vertices.PushBack(StaticMeshVertex{ Float3{  hw,  hh, 0 }, Float3{ 0, 0, 1 }, Float2{ 1, 0 }, white, Float3{ 1, 0, 0 } });
        mesh->vertices.PushBack(StaticMeshVertex{ Float3{ -hw,  hh, 0 }, Float3{ 0, 0, 1 }, Float2{ 0, 0 }, white, Float3{ 1, 0, 0 } });
    ...
    [[nodiscard]] static RefPtr<StaticMesh> Cube(f32 size = 1.0f) {
        ...
        AddFace(*mesh, Float3{ -h, -h,  h }, Float3{ 1, 0, 0 }, Float3{ 0, 1, 0 }, Float3{ 0, 0, 1 }, size);   // +Z
        AddFace(*mesh, Float3{  h, -h, -h }, Float3{ -1, 0, 0 }, Float3{ 0, 1, 0 }, Float3{ 0, 0, -1 }, size);  // -Z
        AddFace(*mesh, Float3{  h, -h,  h }, Float3{ 0, 0, -1 }, Float3{ 0, 1, 0 }, Float3{ 1, 0, 0 }, size);   // +X
        AddFace(*mesh, Float3{ -h, -h, -h }, Float3{ 0, 0, 1 }, Float3{ 0, 1, 0 }, Float3{ -1, 0, 0 }, size);   // -X
        AddFace(*mesh, Float3{ -h,  h,  h }, Float3{ 1, 0, 0 }, Float3{ 0, 0, -1 }, Float3{ 0, 1, 0 }, size);   // +Y
        AddFace(*mesh, Float3{ -h, -h, -h }, Float3{ 1, 0, 0 }, Float3{ 0, 0, 1 }, Float3{ 0, -1, 0 }, size);   // -Y
        Finish(*mesh);
        return mesh;
    }
        ...
        for (u32 z = 0; z <= zs; ++z) {
            for (u32 x = 0; x <= xs; ++x) {
                ...
                mesh->vertices.PushBack(StaticMeshVertex{
                    Float3{ (u - 0.5f) * width, 0.0f, (v - 0.5f) * depth }, Float3{ 0, 1, 0 }, Float2{ u, v }, white, Float3{ 1, 0, 0 } });
            }
        }
    ...
    [[nodiscard]] static RefPtr<StaticMesh> Sphere(f32 radius = 0.5f, u32 segments = 32, u32 rings = 16) {
        ...
        for (u32 r = 0; r <= rng; ++r) {
            ...
            for (u32 s = 0; s <= seg; ++s) {
                ...
                mesh->vertices.PushBack(StaticMeshVertex{ n * radius, n, Float2{ u, v }, white, Float3{ 1, 0, 0 } });
            }
        }
    ...
    [[nodiscard]] static RefPtr<StaticMesh> Cylinder(f32 radius = 0.5f, f32 height = 1.0f, u32 segments = 32) {
        RefPtr<StaticMesh> mesh = MakeRef<StaticMesh>(DefaultAllocator());
        ...
        mesh->vertices.PushBack(StaticMeshVertex{ Float3{ 0, hh, 0 }, Float3{ 0, 1, 0 }, Float2{ 0.5f, 0.5f }, white, Float3{ 1, 0, 0 } });
        ...
        for (u32 i = 0; i < seg; ++i) {
            ...
            mesh->vertices.PushBack(StaticMeshVertex{ Float3{ x, hh, z }, Float3{ 0, 1, 0 },
                Float2{ x / radius * 0.5f + 0.5f, z / radius * 0.5f + 0.5f }, white, Float3{ 1, 0, 0 } });
        }
        ...
        for (u32 i = 0; i <= seg; ++i) {
            ...
            mesh->vertices.PushBack(StaticMeshVertex{ Float3{ x, hh, z }, n, Float2{ u, 0 }, white, Float3{ 1, 0, 0 } });
            mesh->vertices.PushBack(StaticMeshVertex{ Float3{ x, -hh, z }, n, Float2{ u, 1 }, white, Float3{ 1, 0, 0 } });
        }

        mesh->indices.Resize(seg * 3 * 2 + seg * 6);
        for (u32 i = 0; i < seg; ++i) {   // top cap (CCW from above)
            mesh->indices.AddTriangle(topCenter, topRing + (i + 1) % seg, topRing + i);
        }
        for (u32 i = 0; i < seg; ++i) {   // bottom cap (CCW from below)
            mesh->indices.AddTriangle(bottomCenter, bottomRing + i, bottomRing + (i + 1) % seg);
        }
        for (u32 i = 0; i < seg; ++i) {   // sides
    ...
    [[nodiscard]] static RefPtr<StaticMesh> Cone(f32 radius = 0.5f, f32 height = 1.0f, u32 segments = 32) {
        RefPtr<StaticMesh> mesh = MakeRef<StaticMesh>(DefaultAllocator());
        const u32 seg = segments < 3 ? 3 : segments;
        const u32 white = 0xFFFFFFFFu;
        const f32 hh = height * 0.5f;
        ...
        // Tip.
        mesh->vertices.PushBack(StaticMeshVertex{ Float3{ 0, hh, 0 }, Float3{ 0, 1, 0 }, Float2{ 0.5f, 0 }, white, Float3{ 1, 0, 0 } });
        // Base ring for the slanted sides.
        for (u32 i = 0; i < seg; ++i) {
            const f32 a = 2.0f * kPi * static_cast<f32>(i) / static_cast<f32>(seg);
            const f32 x = Cos(a) * radius, z = Sin(a) * radius;
            const Float3 n = Normalized(Float3{ x, radius, z });
            mesh->vertices.PushBack(StaticMeshVertex{ Float3{ x, -hh, z }, n,
                Float2{ static_cast<f32>(i) / static_cast<f32>(seg), 1 }, white, Float3{ 1, 0, 0 } });
        }
    ...
    [[nodiscard]] static RefPtr<StaticMesh> Torus(f32 radius = 1.0f, f32 tubeRadius = 0.3f, u32 segments = 32, u32 tubeSegments = 16) {
        RefPtr<StaticMesh> mesh = MakeRef<StaticMesh>(DefaultAllocator());
        ...
        for (u32 i = 0; i <= seg; ++i) {
            ...
            for (u32 j = 0; j <= tseg; ++j) {
                ...
                const Float3 pos{ (radius + tubeRadius * cp) * ct, tubeRadius * sp, (radius + tubeRadius * cp) * st };
                const Float3 center{ radius * ct, 0, radius * st };
                mesh->vertices.PushBack(StaticMeshVertex{ pos, Normalized(pos - center), Float2{ u, v }, white, Float3{ 1, 0, 0 } });
            }
        }
...
private:
    ...
    static void AddFace(StaticMesh& mesh, Float3 origin, Float3 eu, Float3 ev, Float3 n, f32 size) {
        const u32 base = mesh.VertexCount();
        const u32 white = 0xFFFFFFFFu;
        const Float3 u = eu * size, v = ev * size;
        mesh.vertices.PushBack(StaticMeshVertex{ origin,             n, Float2{ 0, 1 }, white, Float3{ 1, 0, 0 } });
        mesh.vertices.PushBack(StaticMeshVertex{ origin + u,         n, Float2{ 1, 1 }, white, Float3{ 1, 0, 0 } });
        mesh.vertices.PushBack(StaticMeshVertex{ origin + u + v,     n, Float2{ 1, 0 }, white, Float3{ 1, 0, 0 } });
        mesh.vertices.PushBack(StaticMeshVertex{ origin + v,         n, Float2{ 0, 0 }, white, Float3{ 1, 0, 0 } });
