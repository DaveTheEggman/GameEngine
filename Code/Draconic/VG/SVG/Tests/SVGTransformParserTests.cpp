...
{
    const Result<Mat4> r = SVGTransformParser::Parse(u8"translate(10, 20)");
    REQUIRE(r.HasValue());
    const Mat4 m = r.Value();
...
{
    const Result<Mat4> r = SVGTransformParser::Parse(u8"scale(2, 3)");
    REQUIRE(r.HasValue());
    const Mat4 m = r.Value();
...
{
    const Result<Mat4> r = SVGTransformParser::Parse(u8"scale(2)");
    REQUIRE(r.HasValue());
    const Mat4 m = r.Value();
    CHECK(Abs(m.m[0][0] - 2.0f) < 0.01f);
...
{
    const Result<Mat4> r = SVGTransformParser::Parse(u8"rotate(90)");
    REQUIRE(r.HasValue());
    const Mat4 m = r.Value();
...
{
    const Result<Mat4> r = SVGTransformParser::Parse(u8"translate(10, 20) scale(2)");
    CHECK(r.HasValue());
}

TEST_CASE("svg.transform: matrix")
{
    const Result<Mat4> r = SVGTransformParser::Parse(u8"matrix(1 0 0 1 10 20)");
    REQUIRE(r.HasValue());
    const Mat4 m = r.Value();
