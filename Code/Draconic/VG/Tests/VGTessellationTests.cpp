...
TEST_CASE("vg.fill: AA adds an inner+outer fringe ring")
{
    ...
    for (usize i = 0; i < verts.Size(); ++i)
        if (verts[i].coverage == 0.0f && verts[i].color.a == 0) sawTransparent = true;
...
TEST_CASE("vg.fill: gradient fill interpolates per-vertex color (no AA)")
{
    ...
    for (usize i = 1; i < verts.Size(); ++i)
        if (!(verts[i].color == verts[0].color)) varied = true;
    CHECK(varied);
}
...
TEST_CASE("vg.stroke: open polyline (no AA) produces a quad strip")
{
    ...
    Array<VGVertex> verts; Array<u32> idx;
    StrokeTessellator::Tessellate(Span<const Float2>(line, 3), /*closed*/ false, style, Span<const f32>{},
