...
{
    Array<Float2> output;
    CurveUtils::FlattenCubic(Float2{0,0}, Float2{0,10}, Float2{10,10}, Float2{10,0}, 0.25f, output);
    CHECK(output.Size() > 4u);
}
...
TEST_CASE("curveutils: arc to cubics quarter circle")
{
    ...
    CurveUtils::ArcToCubics(Float2{100,0}, 100, 100, 0, false, true, Float2{0,100}, controlPoints);
...
{
    const f32 len = CurveUtils::CubicLength(Float2{0,0}, Float2{3.33f,0}, Float2{6.66f,0}, Float2{10,0});
    CHECK(Abs(len - 10.0f) < 0.1f);
}

TEST_CASE("curveutils: quadratic tangent at endpoints")
{
    const Float2 tangent = CurveUtils::QuadraticTangentAt(Float2{0,0}, Float2{5,0}, Float2{10,0}, 0.5f);
