...
{
    enum class ContainmentType : u8 { Disjoint, Contains, Intersects };
    enum class PlaneIntersectionType : u8 { Front, Back, Intersecting };
    ...
    namespace bounds_detail
    {
        ...
        [[nodiscard]] inline Float3 ClampVec(Float3 v, Float3 lo, Float3 hi) noexcept { return Max(Min(v, hi), lo); }
    }
    ...
    struct BoundingSphere
    {
        ...
        [[nodiscard]] static BoundingSphere FromCenterRadius(Float3 c, f32 r) noexcept { return BoundingSphere{ c, r }; }
        ...
        {
            if (points.IsEmpty()) { return BoundingSphere{ Float3{ 0, 0, 0 }, 0.0f }; }
            Float3 minX = points[0], maxX = points[0], minY = points[0], maxY = points[0], minZ = points[0], maxZ = points[0];
        ...
        [[nodiscard]] PlaneIntersectionType Intersects(const Plane& plane) const noexcept
        {
            ...
            if (dist > radius)  { return PlaneIntersectionType::Front; }
            if (dist < -radius) { return PlaneIntersectionType::Back; }
    ...
    struct BoundingFrustum
    {
        ...
        void SetMatrix(const Float4x4& m) noexcept
        {
            ...
            planes[1] = Plane{ Float3{ m(0,2)-m(0,3), m(1,2)-m(1,3), m(2,2)-m(2,3) }, m(3,2)-m(3,3) };            // Far
            planes[2] = Plane{ Float3{ -m(0,3)-m(0,0), -m(1,3)-m(1,0), -m(2,3)-m(2,0) }, -m(3,3)-m(3,0) };        // Left
            planes[3] = Plane{ Float3{ m(0,0)-m(0,3), m(1,0)-m(1,3), m(2,0)-m(2,3) }, m(3,0)-m(3,3) };            // Right
            planes[4] = Plane{ Float3{ m(0,1)-m(0,3), m(1,1)-m(1,3), m(2,1)-m(2,3) }, m(3,1)-m(3,3) };            // Top
            planes[5] = Plane{ Float3{ -m(0,3)-m(0,1), -m(1,3)-m(1,1), -m(2,3)-m(2,1) }, -m(3,3)-m(3,1) };        // Bottom
        ...
        [[nodiscard]] ContainmentType Contains(Float3 point) const noexcept
        {
            ...
            {
                if (bounds_detail::ApproxGreater(Dot(planes[i].normal, point) + planes[i].d, 0.0f)) { return ContainmentType::Disjoint; }
            }
        ...
        [[nodiscard]] PlaneIntersectionType Intersects(const Plane& plane) const noexcept
        {
            ...
            {
                if (Dot(corners[i], plane.normal) + plane.d > 0.0f) { front = true; } else { back = true; }
    ...
    [[nodiscard]] inline ContainmentType Contains(const BoundingFrustum& f, const BoundingSphere& s) noexcept
    {
        ...
        for (i32 i = 0; i < BoundingFrustum::kPlaneCount; ++i)
        {
            ...
            {
                case PlaneIntersectionType::Front:        return ContainmentType::Disjoint;
                case PlaneIntersectionType::Intersecting: intersects = true; break;
                default: break;
            }
        }
        return intersects ? ContainmentType::Intersects : ContainmentType::Contains;
    }
    [[nodiscard]] inline bool Intersects(const BoundingFrustum& f, const BoundingSphere& s) noexcept { return Contains(f, s) != ContainmentType::Disjoint; }
    ...
    {
        out[0] = Float3{ b.min.x, b.max.y, b.max.z }; out[1] = Float3{ b.max.x, b.max.y, b.max.z };
        out[2] = Float3{ b.max.x, b.min.y, b.max.z }; out[3] = Float3{ b.min.x, b.min.y, b.max.z };
        out[4] = Float3{ b.min.x, b.max.y, b.min.z }; out[5] = Float3{ b.max.x, b.max.y, b.min.z };
        out[6] = Float3{ b.max.x, b.min.y, b.min.z }; out[7] = Float3{ b.min.x, b.min.y, b.min.z };
    }
    ...
    {
        Float3 pos{ plane.normal.x >= 0 ? b.min.x : b.max.x, plane.normal.y >= 0 ? b.min.y : b.max.y, plane.normal.z >= 0 ? b.min.z : b.max.z };
        Float3 neg{ plane.normal.x >= 0 ? b.max.x : b.min.x, plane.normal.y >= 0 ? b.max.y : b.min.y, plane.normal.z >= 0 ? b.max.z : b.min.z };
    ...
    [[nodiscard]] inline AABB TransformAABB(const AABB& b, const Float4x4& m) noexcept
    {
        ...
        const Float3 nc{ c.x*m(0,0)+c.y*m(1,0)+c.z*m(2,0)+m(3,0), c.x*m(0,1)+c.y*m(1,1)+c.z*m(2,1)+m(3,1), c.x*m(0,2)+c.y*m(1,2)+c.z*m(2,2)+m(3,2) };
    ...
    [[nodiscard]] inline bool Intersects(const BoundingFrustum& f, const AABB& b) noexcept { return Contains(f, b) != ContainmentType::Disjoint; }
    ...
    [[nodiscard]] inline ContainmentType ContainsCT(const AABB& b, const BoundingSphere& s) noexcept
    {
        ...
        if (s.center.x > b.max.x - s.radius || s.center.y > b.max.y - s.radius || s.center.z > b.max.z - s.radius ||
            ...
            b.max.x - b.min.x <= s.radius || b.max.y - b.min.y <= s.radius || b.max.z - b.min.z <= s.radius)
        {
    ...
    [[nodiscard]] inline bool Intersects(const BoundingSphere& s, const AABB& b) noexcept { return Intersects(b, s); }
    [[nodiscard]] inline bool Intersects(const BoundingSphere& s, const BoundingFrustum& f) noexcept { return Intersects(f, s); }
    [[nodiscard]] inline ContainmentType Contains(const BoundingSphere& s, const BoundingFrustum& f) noexcept
    {
        if (!Intersects(f, s)) { return ContainmentType::Disjoint; }
        for (i32 i = 0; i < BoundingFrustum::kCornerCount; ++i)
        {
            if (s.Contains(f.corners[i]) == ContainmentType::Disjoint) { return ContainmentType::Intersects; }
        }
        return ContainmentType::Contains;
    }
    [[nodiscard]] inline BoundingSphere BoundingSphereFromFrustum(const BoundingFrustum& f) noexcept
    {
        return BoundingSphere::FromPoints(Span<const Float3>{ f.corners, BoundingFrustum::kCornerCount });
    }
    ...
    [[nodiscard]] inline bool Intersects(const Ray& ray, const Plane& plane, f32& outT) noexcept
    {
        ...
        if (bounds_detail::ApproxZero(nDotDir)) { return false; }
        const f32 dist = -(Dot(plane.normal, ray.position) + plane.d) / nDotDir;
        if (bounds_detail::ApproxZero(dist)) { outT = 0.0f; return true; }
        if (dist < 0.0f) { return false; }
        outT = dist; return true;
    }
    [[nodiscard]] inline bool Intersects(const Ray& ray, const BoundingSphere& sphere, f32& outT) noexcept
    {
        ...
        if (toCenter < 0.0f) { return false; }
        const f32 toSphere = r2 + toCenter * toCenter - offLen2;
        if (toSphere < 0.0f) { return false; }
        outT = toCenter - Sqrt(toSphere); return true;
    }
    [[nodiscard]] inline bool Intersects(const Ray& ray, const AABB& box, f32& outT) noexcept
    {
        bool hasMin = false, hasMax = false; f32 mn = 0.0f, mx = 0.0f;
        // X
        if (bounds_detail::ApproxZero(ray.direction.x)) { if (ray.position.x < box.min.x || ray.position.x > box.max.x) { return false; } }
        else { mn = (box.min.x - ray.position.x) / ray.direction.x; mx = (box.max.x - ray.position.x) / ray.direction.x; if (mn > mx) { const f32 t = mn; mn = mx; mx = t; } hasMin = hasMax = true; }
        // Y
        if (bounds_detail::ApproxZero(ray.direction.y)) { if (ray.position.y < box.min.y || ray.position.y > box.max.y) { return false; } }
        else { f32 y0 = (box.min.y - ray.position.y) / ray.direction.y, y1 = (box.max.y - ray.position.y) / ray.direction.y; if (y0 > y1) { const f32 t = y0; y0 = y1; y1 = t; } if (!hasMin || y0 > mn) { mn = y0; hasMin = true; } if (!hasMax || y1 > mx) { mx = y1; hasMax = true; } }
        // Z
        if (bounds_detail::ApproxZero(ray.direction.z)) { if (ray.position.z < box.min.z || ray.position.z > box.max.z) { return false; } }
        else { f32 z0 = (box.min.z - ray.position.z) / ray.direction.z, z1 = (box.max.z - ray.position.z) / ray.direction.z; if (z0 > z1) { const f32 t = z0; z0 = z1; z1 = t; } if (!hasMin || z0 > mn) { mn = z0; hasMin = true; } if (!hasMax || z1 > mx) { mx = z1; hasMax = true; } }
        if (hasMin && mn < 0.0f && mx > 0.0f) { outT = 0.0f; return true; }
        if (mn < 0.0f) { return false; }
        outT = mn; return true;
    }
    [[nodiscard]] inline bool Intersects(const BoundingFrustum& f, const Ray& ray, f32& outT) noexcept
    {
        if (f.Contains(ray.position) == ContainmentType::Contains) { outT = 0.0f; return true; }
        ...
        for (i32 i = 0; i < BoundingFrustum::kPlaneCount; ++i)
        {
            ...
            if (bounds_detail::ApproxNonZero(dirDotN))
            {
                ...
                if (dirDotN < 0.0f) { if (value > mn) { return false; } if (value > mx) { mx = value; } }
                else                { if (value < mx) { return false; } if (value < mn) { mn = value; } }
            }
            else if (posDotN > 0.0f) { return false; }
        }
        const f32 dist = mx >= 0.0f ? mx : mn;
        if (dist < 0.0f) { return false; }
        outT = dist; return true;
    }
    [[nodiscard]] inline bool Intersects(const Ray& ray, const BoundingFrustum& f, f32& outT) noexcept { return Intersects(f, ray, outT); }

    // ---- remaining Contains/Intersects overloads (complete the suite vs Sedulous) ----
    // sphere contains sphere
    [[nodiscard]] inline ContainmentType Contains(const BoundingSphere& a, const BoundingSphere& b) noexcept
    {
        const f32 d2 = LengthSquared(a.center - b.center);
        const f32 combined = a.radius + b.radius;
        if (d2 > combined * combined) { return ContainmentType::Disjoint; }
    ...
    {
        Float3 c[8]; GetCorners(box, c);
        bool inside = true;
        for (i32 i = 0; i < 8; ++i) { if (s.Contains(c[i]) == ContainmentType::Disjoint) { inside = false; break; } }
        if (inside) { return ContainmentType::Contains; }
        f32 dist = 0.0f;
        if (s.center.x < box.min.x) { dist += (s.center.x - box.min.x) * (s.center.x - box.min.x); } else if (s.center.x > box.max.x) { dist += (s.center.x - box.max.x) * (s.center.x - box.max.x); }
        if (s.center.y < box.min.y) { dist += (s.center.y - box.min.y) * (s.center.y - box.min.y); } else if (s.center.y > box.max.y) { dist += (s.center.y - box.max.y) * (s.center.y - box.max.y); }
        if (s.center.z < box.min.z) { dist += (s.center.z - box.min.z) * (s.center.z - box.min.z); } else if (s.center.z > box.max.z) { dist += (s.center.z - box.max.z) * (s.center.z - box.max.z); }
        return (dist <= s.radius * s.radius) ? ContainmentType::Intersects : ContainmentType::Disjoint;
    }
    // box contains box / box contains frustum
    [[nodiscard]] inline ContainmentType ContainsCT(const AABB& b, const AABB& o) noexcept
    {
        if (o.max.x < b.min.x || o.min.x > b.max.x || o.max.y < b.min.y || o.min.y > b.max.y || o.max.z < b.min.z || o.min.z > b.max.z) { return ContainmentType::Disjoint; }
        if (o.min.x >= b.min.x && o.max.x <= b.max.x && o.min.y >= b.min.y && o.max.y <= b.max.y && o.min.z >= b.min.z && o.max.z <= b.max.z) { return ContainmentType::Contains; }
        return ContainmentType::Intersects;
    }
    [[nodiscard]] inline ContainmentType ContainsCT(const AABB& b, const BoundingFrustum& f) noexcept
    {
        if (!Intersects(f, b)) { return ContainmentType::Disjoint; }
        for (i32 i = 0; i < BoundingFrustum::kCornerCount; ++i) { if (ContainsCT(b, f.corners[i]) == ContainmentType::Disjoint) { return ContainmentType::Intersects; } }
        return ContainmentType::Contains;
    }
    [[nodiscard]] inline bool Intersects(const AABB& b, const BoundingFrustum& f) noexcept { return Intersects(f, b); }
    // frustum contains / intersects frustum
    [[nodiscard]] inline ContainmentType Contains(const BoundingFrustum& f, const BoundingFrustum& g) noexcept
    {
        ...
        for (i32 i = 0; i < BoundingFrustum::kPlaneCount; ++i)
        {
            ...
            {
                case PlaneIntersectionType::Front:        return ContainmentType::Disjoint;
                case PlaneIntersectionType::Intersecting: intersection = true; break;
                default: break;
            }
        }
        return intersection ? ContainmentType::Intersects : ContainmentType::Contains;
    }
    [[nodiscard]] inline bool Intersects(const BoundingFrustum& f, const BoundingFrustum& g) noexcept { return Contains(f, g) != ContainmentType::Disjoint; }
}
