...
export namespace draconic::core
{
    ...
    public:
        ...
        [[nodiscard]] static constexpr Duration FromNanoseconds(i64 ns) noexcept { return Duration(ns); }
        [[nodiscard]] static constexpr Duration FromMicroseconds(f64 us) noexcept { return Duration(static_cast<i64>(us * 1'000.0)); }
        [[nodiscard]] static constexpr Duration FromMilliseconds(f64 ms) noexcept { return Duration(static_cast<i64>(ms * 1'000'000.0)); }
        [[nodiscard]] static constexpr Duration FromSeconds(f64 s) noexcept { return Duration(static_cast<i64>(s * 1'000'000'000.0)); }
        ...
        [[nodiscard]] constexpr f64 AsMicroseconds() const noexcept { return static_cast<f64>(m_nanos) / 1'000.0; }
        [[nodiscard]] constexpr f64 AsMilliseconds() const noexcept { return static_cast<f64>(m_nanos) / 1'000'000.0; }
        [[nodiscard]] constexpr f64 AsSeconds() const noexcept { return static_cast<f64>(m_nanos) / 1'000'000'000.0; }
        // Convenience for the common game-loop f32 delta-time path.
        [[nodiscard]] constexpr f32 AsSecondsF() const noexcept { return static_cast<f32>(AsSeconds()); }

        [[nodiscard]] constexpr bool IsZero() const noexcept { return m_nanos == 0; }

        constexpr Duration operator+(Duration o) const noexcept { return Duration(m_nanos + o.m_nanos); }
        constexpr Duration operator-(Duration o) const noexcept { return Duration(m_nanos - o.m_nanos); }
        constexpr Duration operator-() const noexcept { return Duration(-m_nanos); }
        constexpr Duration operator*(f64 scale) const noexcept { return Duration(static_cast<i64>(static_cast<f64>(m_nanos) * scale)); }
        constexpr Duration operator/(f64 divisor) const noexcept { return Duration(static_cast<i64>(static_cast<f64>(m_nanos) / divisor)); }
        // Ratio of two spans (e.g. how many frames fit in a second).
        constexpr f64 operator/(Duration o) const noexcept { return static_cast<f64>(m_nanos) / static_cast<f64>(o.m_nanos); }

        constexpr Duration& operator+=(Duration o) noexcept { m_nanos += o.m_nanos; return *this; }
        constexpr Duration& operator-=(Duration o) noexcept { m_nanos -= o.m_nanos; return *this; }

        constexpr bool operator==(Duration o) const noexcept { return m_nanos == o.m_nanos; }
        constexpr std::strong_ordering operator<=>(Duration o) const noexcept { return m_nanos <=> o.m_nanos; }
    ...
    public:
        ...
        [[nodiscard]] static constexpr TimePoint FromTicks(u64 ticks) noexcept { return TimePoint(ticks); }
        ...
        constexpr std::strong_ordering operator<=>(TimePoint o) const noexcept { return m_ticks <=> o.m_ticks; }
    ...
    public:
        ...
        void Start() noexcept
        {
            if (!m_running) { m_running = true; m_startedAt = Clock::Now(); }
        }
        void Stop() noexcept
        {
            if (m_running) { m_accumulated += (Clock::Now() - m_startedAt); m_running = false; }
        }
        void Reset() noexcept { m_running = false; m_accumulated = Duration::Zero(); }
        void Restart() noexcept { m_accumulated = Duration::Zero(); m_running = true; m_startedAt = Clock::Now(); }
