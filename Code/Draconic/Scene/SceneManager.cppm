...
class SceneManager {
public:
    explicit SceneManager(SceneAwareRegistry* registry = nullptr) noexcept : m_registry(registry) {}
    ...
    Scene* CreateScene(StringView name = u8"Scene") {
        ...
        if (m_current == nullptr) { m_current = scene; }
        if (m_registry != nullptr) { m_registry->NotifyCreated(*scene); }
        return scene;
    }
    ...
    void ForEachScene(Fn&& fn) { for (auto& scene : m_scenes) { fn(*scene); } }
    ...
    void BeginFrame(f32 rawHostDt, f32 contextScale, f32 contextStep) {
        for (Scene* s : m_active) {
            if (contextStep > 0.0f && s->FixedTimeStep() != contextStep) { s->SetFixedTiming(contextStep, 4); }
    ...
    void Update(f32 contextScaledDt) {
        m_updating = true;
        for (Scene* s : m_active) { s->Update(contextScaledDt * m_timeScale * s->TimeScale()); }
        m_updating = false;
    ...
    void Clear() {
        for (usize i = m_scenes.Size(); i-- > 0;) {
            if (m_registry != nullptr) { m_registry->NotifyDestroyed(*m_scenes[i]); }
        }
        m_active.Clear();
...
private:
    void DestroyImmediate(Scene* scene) {
        if (m_registry != nullptr) { m_registry->NotifyDestroyed(*scene); }
        if (m_current == scene) { m_current = nullptr; }
