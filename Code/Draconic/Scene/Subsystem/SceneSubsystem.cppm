...
public:
    ...
    void ForEachScene(Fn&& fn) {
        for (SceneManager* m : m_managers) { m->ForEachScene([&fn](Scene& s) { fn(s); }); }
    }
    ...
    void BeginFrame(f32 deltaTime) override {
        ...
        for (SceneManager* m : m_managers) { m->BeginFrame(deltaTime, contextScale, contextStep); }
    }
    void Update(f32 deltaTime) override {
        for (SceneManager* m : m_managers) { m->Update(deltaTime); }
    }
...
private:
    ...
    Array<SceneManager*> m_managers;       // all registered managers (borrowed; owned by pages/instances)
};
