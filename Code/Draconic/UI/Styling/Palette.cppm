    ...
    struct Palette
    {
        ...
        [[nodiscard]] static Color Lighten(Color color, f32 amount) noexcept
        {
            ...
            return Color{ Min(1.0f, color.r + (1.0f - color.r) * a),
                          ...
                          Min(1.0f, color.b + (1.0f - color.b) * a),
                          color.a };
        }
        ...
        [[nodiscard]] static Color ComputeHover(Color baseColor) noexcept { return Lighten(baseColor, 0.15f); }
        [[nodiscard]] static Color ComputePressed(Color baseColor) noexcept { return Darken(baseColor, 0.1f); }
        ...
        [[nodiscard]] static Color ComputeDisabled(Color baseColor) noexcept
        {
            ...
            return Color{ (gray + baseColor.r) * 0.5f, (gray + baseColor.g) * 0.5f, (gray + baseColor.b) * 0.5f, baseColor.a * 0.6f };
        }

        /// Tinted toward accent.
        [[nodiscard]] static Color ComputeFocused(Color baseColor,
                                                  Color accentColor = Color{ 60.0f / 255.0f, 130.0f / 255.0f, 220.0f / 255.0f, 1.0f }) noexcept
        {
        ...
        [[nodiscard]] static RefPtr<StateListDrawable> CreateStateColors(Color baseColor)
        {
            ...
            sl->Set(ControlState::Hover, MakeRef<ColorDrawable>(DefaultAllocator(), ComputeHover(baseColor)));
            sl->Set(ControlState::Pressed, MakeRef<ColorDrawable>(DefaultAllocator(), ComputePressed(baseColor)));
            sl->Set(ControlState::Disabled, MakeRef<ColorDrawable>(DefaultAllocator(), ComputeDisabled(baseColor)));
            sl->Set(ControlState::Focused, MakeRef<ColorDrawable>(DefaultAllocator(), ComputeFocused(baseColor)));
            return sl;
        }

        /// StateListDrawable of RoundedRectDrawables with per-corner radii.
        [[nodiscard]] static RefPtr<StateListDrawable> CreateStateRounded(Color baseColor, vg::CornerRadii radii)
        {
            RefPtr<StateListDrawable> sl = MakeRef<StateListDrawable>(DefaultAllocator());
            sl->Set(ControlState::Normal, MakeRef<RoundedRectDrawable>(DefaultAllocator(), baseColor, radii));
            sl->Set(ControlState::Hover, MakeRef<RoundedRectDrawable>(DefaultAllocator(), ComputeHover(baseColor), radii));
            sl->Set(ControlState::Pressed, MakeRef<RoundedRectDrawable>(DefaultAllocator(), ComputePressed(baseColor), radii));
            sl->Set(ControlState::Disabled, MakeRef<RoundedRectDrawable>(DefaultAllocator(), ComputeDisabled(baseColor), radii));
            sl->Set(ControlState::Focused, MakeRef<RoundedRectDrawable>(DefaultAllocator(), ComputeFocused(baseColor), radii));
            return sl;
        }
    };
}
