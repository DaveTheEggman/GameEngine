...
export namespace draconic::ui
{
    ...
    private:
        [[nodiscard]] static RefPtr<StyleSheet> BuildTheme(ThemePalette p)
        {
            ...
            btnBg->Set(ControlState::Normal, MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.SurfaceBright, R));
            btnBg->Set(ControlState::Hover, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputeHover(p.SurfaceBright), R));
            btnBg->Set(ControlState::Pressed, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputePressed(p.SurfaceBright), R));
            btnBg->Set(ControlState::Disabled, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputeDisabled(p.SurfaceBright), R));
            btnBg->Set(ControlState::Focused, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputeFocused(p.SurfaceBright), R));
            sheet.OwnDrawable(btnBg);

            RefPtr<StateListDrawable> btnChecked = MakeRef<StateListDrawable>(DefaultAllocator());
            btnChecked->Set(ControlState::Normal, MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.PrimaryAccent, R));
            btnChecked->Set(ControlState::Hover, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputeHover(p.PrimaryAccent), R));
            btnChecked->Set(ControlState::Pressed, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputePressed(p.PrimaryAccent), R));
            btnChecked->Set(ControlState::Disabled, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputeDisabled(p.PrimaryAccent), R));
            btnChecked->Set(ControlState::Focused, MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::ComputeFocused(p.PrimaryAccent), R));
            ...
            // === Panel ===
            RefPtr<Drawable> panelBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, R, p.Border, 1.0f);
            ...
            sheet.ForTypePseudo(&Slider::StaticType(), u8"fill").Set(StyleProperty::Background, sliderFill);
            ...
            sheet.ForTypePseudo(&ProgressBar::StaticType(), u8"track").Set(StyleProperty::Background, progTrack);
            sheet.ForTypePseudo(&ProgressBar::StaticType(), u8"fill").Set(StyleProperty::Background, progFill);
            ...
            sheet.ForTypePseudo(&ToggleSwitch::StaticType(), u8"track").Set(StyleProperty::Background, switchTrackOff);
            sheet.ForTypePseudoState(&ToggleSwitch::StaticType(), u8"track", ControlState::Checked).Set(StyleProperty::Background, switchTrackOn);
            sheet.ForTypePseudo(&ToggleSwitch::StaticType(), u8"knob").Set(StyleProperty::Background, switchKnob);
            ...
            sheet.ForTypePseudo(&ScrollBar::StaticType(), u8"thumb").Set(StyleProperty::Background, scrollThumb);
            ...
            {
                const f32 tabR = 4.0f;
                RefPtr<Drawable> stripBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::Darken(p.Surface, 0.15f), tabR);
                RefPtr<Drawable> contentBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, tabR);
                RefPtr<Drawable> activeTab = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, tabR);
                RefPtr<Drawable> hoverTab = MakeRef<RoundedRectDrawable>(DefaultAllocator(), Palette::Lighten(p.Surface, 0.05f), tabR);
                sheet.OwnDrawable(stripBg);
                sheet.OwnDrawable(contentBg);
                sheet.OwnDrawable(activeTab);
                sheet.OwnDrawable(hoverTab);
                sheet.ForTypePseudo(&TabView::StaticType(), u8"strip").Set(StyleProperty::Background, stripBg);
                sheet.ForTypePseudo(&TabView::StaticType(), u8"content").Set(StyleProperty::Background, contentBg);
        ...
        static void RegisterIcons(StyleSheet& sheet)
        {
            ...
            if (RefPtr<Drawable> radioMark = SVGDrawable::FromString(ThemeIcons::RadioMarkRound())) { sheet.OwnDrawable(radioMark); sheet.ForTypePseudo(&RadioButton::StaticType(), u8"mark").Set(StyleProperty::Background, radioMark); }
