...
namespace draconic::ui::detail
{
    ...
    [[nodiscard]] inline bool IsLetterC(utf8char c) { return (c >= u8'a' && c <= u8'z') || (c >= u8'A' && c <= u8'Z'); }
    [[nodiscard]] inline bool IsLetterOrDigitC(utf8char c) { return IsLetterC(c) || IsDigitC(c); }
    [[nodiscard]] inline bool IsHexDigitC(utf8char c) { return IsDigitC(c) || (c >= u8'a' && c <= u8'f') || (c >= u8'A' && c <= u8'F'); }
...
export namespace draconic::ui
{
    ...
    public:
        ...
        Token NextToken()
        {
            ...
            switch (ch)
            {
            case u8'{': Advance(); return Token(TokenKind::LBrace, u8"{", startLine, startCol);
            case u8'}': Advance(); return Token(TokenKind::RBrace, u8"}", startLine, startCol);
            case u8'(': Advance(); return Token(TokenKind::LParen, u8"(", startLine, startCol);
            case u8')': Advance(); return Token(TokenKind::RParen, u8")", startLine, startCol);
            case u8';': Advance(); return Token(TokenKind::Semicolon, u8";", startLine, startCol);
            case u8',': Advance(); return Token(TokenKind::Comma, u8",", startLine, startCol);
            case u8'=': Advance(); return Token(TokenKind::Equals, u8"=", startLine, startCol);
            case u8'%': Advance(); return Token(TokenKind::Percent, u8"%", startLine, startCol);
            default: break;
            }
            ...
            if (ch == u8':')
            {
                if (m_pos + 1 < static_cast<i32>(m_source.Size()) && detail::IsIdentStartC(m_source[static_cast<usize>(m_pos + 1)]))
            ...
            if (ch == u8'.')
            {
                ...
                if (m_pos + 1 < static_cast<i32>(m_source.Size()) && detail::IsDigitC(m_source[static_cast<usize>(m_pos + 1)]))
            ...
            if (detail::IsDigitC(ch) || (ch == u8'-' && m_pos + 1 < static_cast<i32>(m_source.Size()) &&
                (detail::IsDigitC(m_source[static_cast<usize>(m_pos + 1)]) || m_source[static_cast<usize>(m_pos + 1)] == u8'.')))
                return ReadNumber(startLine, startCol);
    ...
    private:
        ...
        Token ReadHexColor(i32 line, i32 col)
        {
            ...
            Advance(); // skip #
            while (m_pos < static_cast<i32>(m_source.Size()) && detail::IsHexDigitC(m_source[static_cast<usize>(m_pos)]))
                Advance();
        ...
        Token ReadVariable(i32 line, i32 col)
        {
            const i32 start = m_pos;
            Advance(); // skip $
            while (m_pos < static_cast<i32>(m_source.Size()) && (detail::IsIdentCharC(m_source[static_cast<usize>(m_pos)]) || m_source[static_cast<usize>(m_pos)] == u8'-'))
                Advance();
        ...
        Token ReadDirective(i32 line, i32 col)
        {
            const i32 start = m_pos;
            Advance(); // skip @
            while (m_pos < static_cast<i32>(m_source.Size()) && detail::IsIdentCharC(m_source[static_cast<usize>(m_pos)]))
                Advance();
        ...
        Token ReadString(i32 line, i32 col)
        {
            ...
            const i32 start = m_pos;
            while (m_pos < static_cast<i32>(m_source.Size()) && m_source[static_cast<usize>(m_pos)] != u8'"')
            {
                if (m_source[static_cast<usize>(m_pos)] == u8'\n') { m_line++; m_col = 0; }
                Advance();
            }
            const StringView text = Slice(start, m_pos);
            if (m_pos < static_cast<i32>(m_source.Size())) Advance(); // skip closing "
        ...
        {
            const i32 start = m_pos;
            if (m_pos < static_cast<i32>(m_source.Size()) && m_source[static_cast<usize>(m_pos)] == u8'-') Advance();
            while (m_pos < static_cast<i32>(m_source.Size()) && detail::IsDigitC(m_source[static_cast<usize>(m_pos)])) Advance();
            if (m_pos < static_cast<i32>(m_source.Size()) && m_source[static_cast<usize>(m_pos)] == u8'.')
            {
                Advance();
                while (m_pos < static_cast<i32>(m_source.Size()) && detail::IsDigitC(m_source[static_cast<usize>(m_pos)])) Advance();
            }
            ...
            while (m_pos < static_cast<i32>(m_source.Size()) && detail::IsLetterC(m_source[static_cast<usize>(m_pos)]))
                Advance();
        ...
        Token ReadIdent(i32 line, i32 col)
        {
            ...
            // Keywords
            if (text == StringView(u8"true")) return Token(TokenKind::BoolLit, text, line, col);
            if (text == StringView(u8"false")) return Token(TokenKind::BoolLit, text, line, col);
            if (text == StringView(u8"extends")) return Token(TokenKind::Extends, text, line, col);
        ...
        void SkipWhitespaceAndComments()
        {
            while (m_pos < static_cast<i32>(m_source.Size()))
            {
                ...
                // Block comment /* ... */
                if (ch == u8'/' && m_pos + 1 < static_cast<i32>(m_source.Size()) && m_source[static_cast<usize>(m_pos + 1)] == u8'*')
                {
                    m_pos += 2; m_col += 2;
                    while (m_pos + 1 < static_cast<i32>(m_source.Size()))
                    {
                        if (m_source[static_cast<usize>(m_pos)] == u8'\n') { m_line++; m_col = 0; }
                        if (m_source[static_cast<usize>(m_pos)] == u8'*' && m_source[static_cast<usize>(m_pos + 1)] == u8'/')
                        {
                            m_pos += 2; m_col += 2;
                            break;
                        }
