...
export namespace draconic::ui
{
    ...
    struct TexturedTheme
    {
        ...
        [[nodiscard]] static RefPtr<StyleSheet> Create(const ThemeImageSet& images) { return Create(images, ThemePalette::Dark()); }
        ...
        [[nodiscard]] static RefPtr<StyleSheet> Create(const ThemeImageSet& images, ThemePalette p)
        {
            ...
            sheet.ForType(&ListView::StaticType()).Set(StyleProperty::SelectionColor, C(60, 120, 200, 80));
            sheet.ForType(&GridView::StaticType()).Set(StyleProperty::SelectionColor, C(60, 120, 200, 80));
            ...
            for (const auto& kv : images.GetStateGroups())
            {
                ...
                for (const ThemeStateEntry& e : states)
                {
                    ...
                    if (entry.Value().IsNineSlice)
                        drawable = atlas->CreateNineSliceDrawable(e.Key.AsView(), entry.Value().Slices);
                    else
            ...
            for (const auto& kv : images.GetImages())
            {
                ...
                for (const auto& sg : images.GetStateGroups())
                {
                    ...
                    {
                        if (key == e.Key.AsView()) { isStateImage = true; break; }
                    }
    ...
    private:
        ...
        {
            if (needle.Size() == 0 || needle.Size() > s.Size()) { return needle.Size() == 0 ? 0 : -1; }
            ...
            for (usize i = 0; i <= last; ++i)
            {
                ...
                {
                    if (s[i + j] != needle[j]) { match = false; break; }
                }
                if (match) { return static_cast<i64>(i); }
            }
            return -1;
        }
        ...
        [[nodiscard]] static i64 IndexOf(StringView s, char8_t c)
        {
            ...
            {
                if (s[i] == c) { return static_cast<i64>(i); }
            }
            return -1;
        }
        ...
        static void SetDrawableByKey(StyleSheet& sheet, StringView key, RefPtr<Drawable> drawable)
        {
            ...
            if (pseudoIdx >= 0)
            {
                ...
                StringView remainder = key.SubStr(static_cast<usize>(pseudoIdx) + 2, key.Size() - static_cast<usize>(pseudoIdx) - 2);
                ...
                if (stateIdx >= 0)
                {
                    ...
                    StringView stateName = remainder.SubStr(static_cast<usize>(stateIdx) + 1, remainder.Size() - static_cast<usize>(stateIdx) - 1);
                    Optional<ControlState> state = ParseStateName(stateName);
                    if (state.HasValue())
                        sheet.ForTypePseudoState(type, pseudo, state.Value()).Set(StyleProperty::Background, drawable);
                }
                else
                {
            ...
            StringView propName = key.SubStr(static_cast<usize>(colonIdx) + 1, key.Size() - static_cast<usize>(colonIdx) - 1);

            Optional<StyleProperty> prop = ParsePropertyName(propName);
            if (!prop.HasValue()) { return; }
        ...
        [[nodiscard]] static Optional<StyleProperty> ParsePropertyName(StringView name)
        {
            ...
            if (name == StringView(u8"MenuItemHoverDrawable")) { return StyleProperty::MenuItemHoverDrawable; }
            return {};
        }
        ...
        {
            if (name == StringView(u8"button")) { return &ButtonBase::StaticType(); }
            if (name == StringView(u8"edittext")) { return &EditText::StaticType(); }
            if (name == StringView(u8"checkbox")) { return &CheckBox::StaticType(); }
            if (name == StringView(u8"radiobutton")) { return &RadioButton::StaticType(); }
            if (name == StringView(u8"slider")) { return &Slider::StaticType(); }
            if (name == StringView(u8"progressbar")) { return &ProgressBar::StaticType(); }
            if (name == StringView(u8"toggleswitch")) { return &ToggleSwitch::StaticType(); }
            if (name == StringView(u8"combobox")) { return &ComboBox::StaticType(); }
            if (name == StringView(u8"scrollbar")) { return &ScrollBar::StaticType(); }
            if (name == StringView(u8"separator")) { return &Separator::StaticType(); }
            if (name == StringView(u8"expander")) { return &Expander::StaticType(); }
            if (name == StringView(u8"tabview")) { return &TabView::StaticType(); }
            // "contextmenu" is class-based (shared by ContextMenu and ComboBoxDropdown).
            if (name == StringView(u8"dialog")) { return &Dialog::StaticType(); }
            if (name == StringView(u8"tooltip")) { return &TooltipView::StaticType(); }
            if (name == StringView(u8"listview")) { return &ListView::StaticType(); }
            if (name == StringView(u8"treeview")) { return &TreeView::StaticType(); }
            if (name == StringView(u8"gridview")) { return &GridView::StaticType(); }
            if (name == StringView(u8"numericfield")) { return &NumericField::StaticType(); }
            return nullptr;
        }
        ...
        static void RegisterIcons(StyleSheet& sheet, ThemePalette p)
        {
            ...
            Optional<Color> tint;
            if (isLight) { tint = C(60, 60, 70, 255); }

            auto MakeSVG = [&](StringView svg) -> RefPtr<SVGDrawable>
            {
                if (tint.HasValue()) { return SVGDrawable::FromString(svg, tint.Value()); }
                return SVGDrawable::FromString(svg);
            };

            if (RefPtr<Drawable> checkmark = MakeSVG(ThemeIcons::Checkmark())) { sheet.OwnDrawable(checkmark); sheet.ForTypePseudo(&CheckBox::StaticType(), u8"checkmark").Set(StyleProperty::Background, checkmark); }
            if (RefPtr<Drawable> radioMark = MakeSVG(ThemeIcons::RadioMarkRound())) { sheet.OwnDrawable(radioMark); sheet.ForTypePseudo(&RadioButton::StaticType(), u8"mark").Set(StyleProperty::Background, radioMark); }
            if (RefPtr<Drawable> closeIcon = MakeSVG(ThemeIcons::Close())) { sheet.OwnDrawable(closeIcon); sheet.ForTypePseudo(&TabView::StaticType(), u8"close-button").Set(StyleProperty::Background, closeIcon); }
            if (RefPtr<Drawable> chevExpanded = MakeSVG(ThemeIcons::ChevronDown())) { sheet.OwnDrawable(chevExpanded); sheet.ForTypePseudoState(&Expander::StaticType(), u8"chevron", ControlState::Checked).Set(StyleProperty::Background, chevExpanded); }
            if (RefPtr<Drawable> chevCollapsed = MakeSVG(ThemeIcons::ChevronRight())) { sheet.OwnDrawable(chevCollapsed); sheet.ForTypePseudo(&Expander::StaticType(), u8"chevron").Set(StyleProperty::Background, chevCollapsed); }
            if (RefPtr<Drawable> tvChevExpanded = MakeSVG(ThemeIcons::ChevronDown())) { sheet.OwnDrawable(tvChevExpanded); sheet.ForTypePseudoState(&TreeView::StaticType(), u8"chevron", ControlState::Checked).Set(StyleProperty::Background, tvChevExpanded); }
            if (RefPtr<Drawable> tvChevCollapsed = MakeSVG(ThemeIcons::ChevronRight())) { sheet.OwnDrawable(tvChevCollapsed); sheet.ForTypePseudo(&TreeView::StaticType(), u8"chevron").Set(StyleProperty::Background, tvChevCollapsed); }
            ...
            if (RefPtr<Drawable> arrowDown = MakeSVG(ThemeIcons::ArrowDown())) { sheet.OwnDrawable(arrowDown); sheet.ForTypePseudo(&ComboBox::StaticType(), u8"arrow").Set(StyleProperty::Background, arrowDown); }
            if (RefPtr<Drawable> arrowUp = MakeSVG(ThemeIcons::ArrowUp())) { sheet.OwnDrawable(arrowUp); sheet.ForTypePseudo(&NumericField::StaticType(), u8"arrow-up").Set(StyleProperty::Background, arrowUp); }
            if (RefPtr<Drawable> arrowDn2 = MakeSVG(ThemeIcons::ArrowDown())) { sheet.OwnDrawable(arrowDn2); sheet.ForTypePseudo(&NumericField::StaticType(), u8"arrow-down").Set(StyleProperty::Background, arrowDn2); }
        }
    };
}
