...
export namespace draconic::ui
{
    ...
    struct LightTheme
    {
        [[nodiscard]] static RefPtr<StyleSheet> Create() { return BuildTheme(ThemePalette::Light()); }
    ...
    private:
        ...
        [[nodiscard]] static RefPtr<StyleSheet> BuildTheme(ThemePalette p)
        {
            ...
            // EditText
            RefPtr<Drawable> editBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, 0.0f, p.Border, 1.0f);
            ...
            // CheckBox
            RefPtr<Drawable> cbUnchecked = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, 0.0f, p.Border, 1.0f);
            RefPtr<Drawable> cbChecked = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.PrimaryAccent, 0.0f, p.Border, 1.0f);
            ...
            // RadioButton
            RefPtr<Drawable> rbUnchecked = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, 0.0f, p.Border, 1.0f);
            RefPtr<Drawable> rbChecked = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.PrimaryAccent, 0.0f, p.Border, 1.0f);
            ...
            // ProgressBar
            sheet.ForTypePseudo(&ProgressBar::StaticType(), u8"track").Set(StyleProperty::Background, sheet.OwnColor(C(210, 215, 225, 255)));
            sheet.ForTypePseudo(&ProgressBar::StaticType(), u8"fill").Set(StyleProperty::Background, sheet.OwnColor(p.PrimaryAccent));

            // ToggleSwitch
            {
                RefPtr<Drawable> swOff = MakeRef<RoundedRectDrawable>(DefaultAllocator(), C(200, 205, 215, 255), 0.0f, p.Border, 1.0f);
                ...
                sheet.ForTypePseudo(&ToggleSwitch::StaticType(), u8"knob").Set(StyleProperty::Background, sheet.OwnColor(p.Surface));
            }

            // ComboBox
            RefPtr<Drawable> comboBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, 0.0f, p.Border, 1.0f);
            sheet.OwnDrawable(comboBg);
            sheet.ForType(&ComboBox::StaticType()).Set(StyleProperty::Background, comboBg);
            sheet.ForTypePseudo(&ComboBox::StaticType(), u8"arrow").Set(StyleProperty::TextColor, C(80, 85, 100, 255));

            // ScrollBar
            sheet.ForTypePseudo(&ScrollBar::StaticType(), u8"track").Set(StyleProperty::Background, sheet.OwnColor(C(230, 232, 240, 150)));
            sheet.ForTypePseudo(&ScrollBar::StaticType(), u8"thumb").Set(StyleProperty::Background, sheet.OwnColor(C(160, 165, 180, 200)));

            // Separator
            sheet.ForType(&Separator::StaticType()).Set(StyleProperty::BorderColor, p.Border);

            // Expander
            sheet.ForTypePseudo(&Expander::StaticType(), u8"header").Set(StyleProperty::Background, sheet.OwnColor(C(235, 238, 245, 255)));
            sheet.ForTypePseudoState(&Expander::StaticType(), u8"header", ControlState::Hover).Set(StyleProperty::Background, sheet.OwnColor(Palette::Darken(C(235, 238, 245, 255), 0.05f)));
            sheet.ForTypePseudo(&Expander::StaticType(), u8"chevron").Set(StyleProperty::TextColor, C(80, 85, 100, 255));

            // TabView
            sheet.ForTypePseudo(&TabView::StaticType(), u8"strip").Set(StyleProperty::Background, sheet.OwnColor(Palette::Darken(p.Surface, 0.05f)));
            ...
            sheet.ForTypePseudoState(&TabView::StaticType(), u8"tab", ControlState::Hover).Set(StyleProperty::Background, sheet.OwnColor(Palette::Darken(p.Surface, 0.03f)));
            ...
            sheet.ForTypePseudoState(&TabView::StaticType(), u8"tab", ControlState::Hover).Set(StyleProperty::TextColor, Palette::Darken(p.TextDim, 0.2f));
            ...
            // ContextMenu
            RefPtr<Drawable> menuBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, 0.0f, p.Border, 1.0f);
            sheet.OwnDrawable(menuBg);
            RefPtr<Drawable> menuHover = MakeRef<RoundedRectDrawable>(DefaultAllocator(), C(60, 120, 200, 60), 0.0f);
            ...
            // Dialog
            RefPtr<Drawable> dialogBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), p.Surface, 0.0f, p.Border, 1.0f);
            sheet.OwnDrawable(dialogBg);
            sheet.ForType(&Dialog::StaticType()).Set(StyleProperty::Background, dialogBg);

            // Tooltip
            RefPtr<Drawable> tooltipBg = MakeRef<RoundedRectDrawable>(DefaultAllocator(), C(255, 255, 225, 245), 0.0f, C(180, 175, 140, 255), 1.0f);
        ...
        static void RegisterIcons(StyleSheet& sheet)
        {
            ...
            if (RefPtr<Drawable> checkmark = SVGDrawable::FromString(ThemeIcons::Checkmark(), tint)) { sheet.OwnDrawable(checkmark); sheet.ForTypePseudo(&CheckBox::StaticType(), u8"checkmark").Set(StyleProperty::Background, checkmark); }
            if (RefPtr<Drawable> radioMark = SVGDrawable::FromString(ThemeIcons::RadioMarkSquare(), tint)) { sheet.OwnDrawable(radioMark); sheet.ForTypePseudo(&RadioButton::StaticType(), u8"mark").Set(StyleProperty::Background, radioMark); }
            if (RefPtr<Drawable> closeIcon = SVGDrawable::FromString(ThemeIcons::Close(), tint)) { sheet.OwnDrawable(closeIcon); sheet.ForTypePseudo(&TabView::StaticType(), u8"close-button").Set(StyleProperty::Background, closeIcon); }
            if (RefPtr<Drawable> chevExpanded = SVGDrawable::FromString(ThemeIcons::ChevronDown(), tint)) { sheet.OwnDrawable(chevExpanded); sheet.ForTypePseudoState(&Expander::StaticType(), u8"chevron", ControlState::Checked).Set(StyleProperty::Background, chevExpanded); }
            if (RefPtr<Drawable> chevCollapsed = SVGDrawable::FromString(ThemeIcons::ChevronRight(), tint)) { sheet.OwnDrawable(chevCollapsed); sheet.ForTypePseudo(&Expander::StaticType(), u8"chevron").Set(StyleProperty::Background, chevCollapsed); }
            if (RefPtr<Drawable> tvChevExpanded = SVGDrawable::FromString(ThemeIcons::ChevronDown(), tint)) { sheet.OwnDrawable(tvChevExpanded); sheet.ForTypePseudoState(&TreeView::StaticType(), u8"chevron", ControlState::Checked).Set(StyleProperty::Background, tvChevExpanded); }
            if (RefPtr<Drawable> tvChevCollapsed = SVGDrawable::FromString(ThemeIcons::ChevronRight(), tint)) { sheet.OwnDrawable(tvChevCollapsed); sheet.ForTypePseudo(&TreeView::StaticType(), u8"chevron").Set(StyleProperty::Background, tvChevCollapsed); }
            if (RefPtr<Drawable> subArrow = SVGDrawable::FromString(ThemeIcons::ChevronRight(), tint))
            {
            ...
            if (RefPtr<Drawable> arrowDown = SVGDrawable::FromString(ThemeIcons::ArrowDown(), tint)) { sheet.OwnDrawable(arrowDown); sheet.ForTypePseudo(&ComboBox::StaticType(), u8"arrow").Set(StyleProperty::Background, arrowDown); }
            if (RefPtr<Drawable> arrowUp = SVGDrawable::FromString(ThemeIcons::ArrowUp(), tint)) { sheet.OwnDrawable(arrowUp); sheet.ForTypePseudo(&NumericField::StaticType(), u8"arrow-up").Set(StyleProperty::Background, arrowUp); }
            if (RefPtr<Drawable> arrowDn2 = SVGDrawable::FromString(ThemeIcons::ArrowDown(), tint)) { sheet.OwnDrawable(arrowDn2); sheet.ForTypePseudo(&NumericField::StaticType(), u8"arrow-down").Set(StyleProperty::Background, arrowDn2); }
        }
    };
}
