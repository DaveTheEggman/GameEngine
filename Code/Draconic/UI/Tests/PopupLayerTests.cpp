...
namespace
{
    ...
    core::RefPtr<TestView> MakeView(f32 w = 50.0f, f32 h = 30.0f) { return core::MakeRef<TestView>(core::DefaultAllocator(), w, h); }
    ...
    public:
        bool* Notified;
        View** ClosedPopup;
        TestPopupOwner(bool* notified, View** closedPopup) : Notified(notified), ClosedPopup(closedPopup) {}
        void OnPopupClosed(View* popup) override { *Notified = true; *ClosedPopup = popup; }
...
{
    UIContext ctx; auto root = MakeRoot(); Init(ctx, root.Get());
    root->ViewportSize = Float2{ 800, 600 }; LayoutPass(ctx, root.Get());
...
TEST_CASE("popup-layer: ShowPopup_NotifiesOwner")
{
    ...
    bool notified = false; View* closedPopup = nullptr;
...
{
    UIContext ctx; auto root = MakeRoot(); Init(ctx, root.Get(), 800, 600);
    LayoutPass(ctx, root.Get());
    auto bg = MakeView(800, 600);
    root->AddView(bg.Get()); LayoutPass(ctx, root.Get());
    auto popup = MakeView(100, 50);
    root->GetPopupLayer()->ShowPopup(popup.Get(), nullptr, 10, 10, true, true, true);
    LayoutPass(ctx, root.Get());
