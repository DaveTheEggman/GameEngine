...
export namespace draconic::ui
{
    ...
    public:
        ...
        {
            if (m_selected.Remove(index)) { OnSelectionChanged.Invoke(); }
        }
        ...
        {
            if (Mode == SelectionMode::None) { return; }
            if (Mode == SelectionMode::Single) { Select(to); return; }
            const i32 lo = Min(from, to);
            const i32 hi = Max(from, to);
            m_selected.Clear();
            for (i32 i = lo; i <= hi; ++i) { m_selected.Insert(i); }
            OnSelectionChanged.Invoke();
        }
        ...
        void ClearSelection()
        {
            if (m_selected.Size() > 0) { m_selected.Clear(); OnSelectionChanged.Invoke(); }
        }
        ...
        {
            for (i32 idx : m_selected) { return idx; }
            return -1;
        }
        ...
        {
            Array<i32> old;
            for (i32 idx : m_selected) { old.PushBack(idx); }
            m_selected.Clear();
            for (i32 idx : old)
            {
                if (idx >= startPos) { const i32 newIdx = idx + delta; if (newIdx >= 0) { m_selected.Insert(newIdx); } }
