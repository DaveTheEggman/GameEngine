...
export namespace draconic::ui
{
    ...
    public:
        ...
        [[nodiscard]] i32 ItemCount() const override { return static_cast<i32>(m_visibleNodes.Size()); }
        [[nodiscard]] i32 GetItemViewType(i32 position) const override
        {
            if (position < 0 || position >= static_cast<i32>(m_visibleNodes.Size())) { return 0; }
            return m_source->GetItemViewType(m_visibleNodes[static_cast<usize>(position)]);
        }
        [[nodiscard]] RefPtr<View> CreateView(i32 viewType) override { return m_source->CreateView(viewType); }
        void BindView(View* view, i32 position) override
        {
            if (position < 0 || position >= static_cast<i32>(m_visibleNodes.Size())) { return; }
        ...
        {
            if (m_expanded.Contains(nodeId)) { m_expanded.Remove(nodeId); }
            else if (m_source->HasChildren(nodeId)) { m_expanded.Insert(nodeId); }
            RebuildVisibleList();
        }
        void Expand(i32 nodeId)
        {
            if (!m_expanded.Contains(nodeId) && m_source->HasChildren(nodeId)) { m_expanded.Insert(nodeId); RebuildVisibleList(); }
        }
        void Collapse(i32 nodeId)
        {
            if (m_expanded.Remove(nodeId)) { RebuildVisibleList(); }
        }

        [[nodiscard]] i32 GetNodeId(i32 position) const
        {
            if (position < 0 || position >= static_cast<i32>(m_visibleNodes.Size())) { return -1; }
            return m_visibleNodes[static_cast<usize>(position)];
        }
        [[nodiscard]] i32 GetDepth(i32 position) const
        {
            if (position < 0 || position >= static_cast<i32>(m_depths.Size())) { return 0; }
            return m_depths[static_cast<usize>(position)];
        }

        void GetExpandedNodes(HashSet<i32>& output) const { for (i32 nodeId : m_expanded) { output.Insert(nodeId); } }
        void SetExpandedNodes(const HashSet<i32>& nodes)
        {
            m_expanded.Clear();
            for (i32 nodeId : nodes) { if (m_source->HasChildren(nodeId)) { m_expanded.Insert(nodeId); } }
            RebuildVisibleList();
        }
        ...
        void RebuildVisibleList()
        {
            ...
            for (i32 i = 0; i < rootCount; ++i) { AddNodeRecursive(m_source->GetChildId(-1, i), 0); }
            if (m_observer != nullptr) { m_observer->OnDataSetChanged(); }
        }
    ...
    private:
        void AddNodeRecursive(i32 nodeId, i32 depth)
        {
            ...
            if (m_expanded.Contains(nodeId))
            {
                ...
                for (i32 i = 0; i < childCount; ++i) { AddNodeRecursive(m_source->GetChildId(nodeId, i), depth + 1); }
            }
        }
