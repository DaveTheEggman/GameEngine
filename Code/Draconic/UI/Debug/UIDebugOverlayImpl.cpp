...
namespace draconic::ui
{
    namespace
    {
        [[nodiscard]] constexpr Color C(f32 r, f32 g, f32 b, f32 a) { return Color{ r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f }; }
    ...
    void UIDebugOverlay::DrawOverlays(UIDrawContext& ctx, View& view)
    {
        ...
        if (settings.ShowPadding)
        {
            if (ViewGroup* vg = Cast<ViewGroup>(&view))
            {
                ...
                if (!pad.IsZero())
                {
                    ...
                    ctx.VG().FillRect(Rectangle{ 0, pad.Top, pad.Left, h - pad.Top - pad.Bottom }, sPaddingColor);
                    ctx.VG().FillRect(Rectangle{ w - pad.Right, pad.Top, pad.Right, h - pad.Top - pad.Bottom }, sPaddingColor);
                }
            }
        }
        ...
        if (settings.ShowMargin)
        {
            if (LayoutParams* lp = view.LayoutParams.Get(); lp != nullptr && !lp->Margin.IsZero())
            {
                ...
                ctx.VG().FillRect(Rectangle{ -m.Left, -m.Top, w + m.TotalHorizontal(), m.Top }, sMarginColor);
                ctx.VG().FillRect(Rectangle{ -m.Left, h, w + m.TotalHorizontal(), m.Bottom }, sMarginColor);
        ...
        {
            if (view.IsHovered()) { ctx.VG().FillRect(Rectangle{ 0, 0, w, h }, sHitTargetColor); }
        }
        ...
        if (settings.ShowFocusPath)
        {
            ...
            {
                ctx.VG().StrokeRect(Rectangle{ -1, -1, w + 2, h + 2 }, Color{ sFocusColor.r, sFocusColor.g, sFocusColor.b, 80.0f / 255.0f }, 1.0f);
            }
        }
    }
}
