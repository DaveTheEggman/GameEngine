...
export namespace draconic::ui::toolkit
{
    ...
    public:
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            const Color hoverColor = ResolveStyleColor(StyleProperty::AccentColor, Rgb(60, 65, 80, 255));
                ...
                if (font != nullptr)
                {
                    for (usize i = 0; i < m_menus.Size() && i < m_itemRects.Size(); ++i)
                    {
                        ...
                        ctx.VG().DrawText(m_menus[i].Title, font, rect,
                            fonts::TextAlignment::Center, fonts::VerticalAlignment::Middle, textColor);
                    }
                }
            }
        }
        ...
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            {
            case KeyCode::Left:
                if (m_activeIndex > 0) { OpenMenu(m_activeIndex - 1); }
                e.Handled = true;
                break;
            case KeyCode::Right:
                if (m_activeIndex < static_cast<i32>(m_menus.Size()) - 1) { OpenMenu(m_activeIndex + 1); }
                e.Handled = true;
                break;
            case KeyCode::Escape:
                CloseActiveMenu();
    ...
    protected:
        ...
        {
            MeasuredSize = Float2{ constraints.ConstrainWidth(0.0f), constraints.ConstrainHeight(m_itemHeight) };
        }
    ...
    private:
        ...
        {
            if (index < 0 || index >= static_cast<i32>(m_menus.Size())) { return; }

            // Close current menu if different.
            if (m_activeIndex >= 0 && m_activeIndex != index) { CloseActiveMenu(); }
        ...
        {
            if (m_activeIndex >= 0 && m_activeIndex < static_cast<i32>(m_menus.Size()) && Context != nullptr)
            {
                ...
                {
                    if (RootView* root = Root()) { root->GetPopupLayer()->ClosePopup(menu); }
                }
            }
            m_activeIndex = -1;
            m_menuMode = false;
