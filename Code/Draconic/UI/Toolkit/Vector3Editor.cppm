// Draconic UI Toolkit - :vector3_editor partition
//
// Property editor for Vector3 values - three NumericFields (X, Y, Z) side by side with colored axis labels.
...
// focus overrides are defined out-of-line after Vector3Editor is complete.

module;
#include "Core/Prelude.h"
#include "Core/Reflection/Reflect.h"

export module draconic.ui.toolkit:vector3_editor;
...
{
    /// Property editor for Vector3 values. Three NumericFields (X, Y, Z) side by side with axis labels.
    class Vector3Editor : public PropertyEditor
    {
        DRACONIC_OBJECT(Vector3Editor, PropertyEditor)
    public:
        Function<void(Float3)> Setter;

        Vector3Editor(StringView name, Float3 value, f32 min = -100000, f32 max = 100000,
        ...
        public:
            VectorNumericField(Vector3Editor* editor, i32 axis) : m_editor(editor), m_axis(axis) {}

            void OnFocusGained() override;
            void OnFocusLost() override;

        private:
            Vector3Editor* m_editor;
    ...
    // === Inner-view out-of-line bodies (need the complete Vector3Editor type) ===

    inline void Vector3Editor::VectorNumericField::OnFocusGained()
    {
        NumericField::OnFocusGained();
        if (!m_editor->IsEditing()) { m_editor->BeginEdit(); }
    }

    inline void Vector3Editor::VectorNumericField::OnFocusLost()
    {
        NumericField::OnFocusLost();
        if (m_editor->IsEditing()) { m_editor->EndEdit(); }
    }

    DRACONIC_DEFINE_OBJECT(Vector3Editor, "draconic::ui::toolkit")
    DRACONIC_DEFINE_OBJECT(Vector3Editor::VectorNumericField, "draconic::ui::toolkit")
}
