...
export namespace draconic::ui::toolkit
{
    ...
    public:
        explicit DockSplit(::draconic::ui::Orientation orientation = ::draconic::ui::Orientation::Horizontal)
            : m_orientation(orientation)
        {
        }

        [[nodiscard]] ::draconic::ui::Orientation Orientation() const { return m_orientation; }
        void SetOrientation(::draconic::ui::Orientation value) { m_orientation = value; Invalidate(); }

        [[nodiscard]] f32 SplitRatio() const { return m_splitRatio; }
        void SetSplitRatio(f32 value) { m_splitRatio = Clamp(value, 0.05f, 0.95f); Invalidate(); }

        [[nodiscard]] f32 DividerSize() const { return m_dividerSize; }
        void SetDividerSize(f32 value) { m_dividerSize = Max(2.0f, value); Invalidate(); }
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            const Color dividerColor = (m_isDragging || m_isDividerHovered)
        ...
        [[nodiscard]] View* HitTest(Float2 localPoint) override
        {
            ...
            {
                const Float2 childLocal{ localPoint.x - second->Bounds.x, localPoint.y - second->Bounds.y };
                if (View* hit = second->HitTest(childLocal)) { return hit; }
            }
            if (View* first = First())
            {
                const Float2 childLocal{ localPoint.x - first->Bounds.x, localPoint.y - first->Bounds.y };
                if (View* hit = first->HitTest(childLocal)) { return hit; }
            }
        ...
        void OnMouseMove(MouseEventArgs& e) override
        {
            ...
            {
                const Rectangle dividerRect = GetDividerRect();
                const bool overDivider = e.X >= dividerRect.x && e.X < dividerRect.x + dividerRect.width &&
                    e.Y >= dividerRect.y && e.Y < dividerRect.y + dividerRect.height;
    ...
    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            ...
            if (m_orientation == ::draconic::ui::Orientation::Horizontal)
            {
                ...
                if (View* first = First()) { first->Measure(BoxConstraints::Tight(firstW, h)); }
                if (View* second = Second()) { second->Measure(BoxConstraints::Tight(secondW, h)); }
            }
            else
            {
                ...
                if (View* first = First()) { first->Measure(BoxConstraints::Tight(w, firstH)); }
                if (View* second = Second()) { second->Measure(BoxConstraints::Tight(w, secondH)); }
            }
        ...
        void OnLayout(f32 left, f32 top, f32 width, f32 height) override
        {
            ...
            if (m_orientation == ::draconic::ui::Orientation::Horizontal)
            {
                ...
                const f32 firstW = available * m_splitRatio;
                const f32 secondW = available - firstW;
                if (View* first = First()) { first->Layout(0, 0, firstW, height); }
                if (View* second = Second()) { second->Layout(firstW + m_dividerSize, 0, secondW, height); }
            }
            else
            {
                ...
                const f32 firstH = available * m_splitRatio;
                const f32 secondH = available - firstH;
                if (View* first = First()) { first->Layout(0, 0, width, firstH); }
                if (View* second = Second()) { second->Layout(0, firstH + m_dividerSize, width, secondH); }
            }
        }
