...
namespace
{
    void WalkCheckFields(View* v, i32& fieldCount)
    {
        if (auto* field = Cast<NumericField>(v)) {
            ...
            CHECK(field->ResolveStyleFloat(StyleProperty::FontSize, 14.0f) == doctest::Approx(10.0f));
        }
        if (auto* group = Cast<ViewGroup>(v)) {
            for (usize i = 0; i < group->ChildCount(); ++i) { WalkCheckFields(group->GetChildAt(i), fieldCount); }
        }
    }
}
