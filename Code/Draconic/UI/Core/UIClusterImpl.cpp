...
namespace draconic::ui
{
    ...
    {
        m_needsRedraw = true;
        if (Context != nullptr) { Context->MarkNeedsRedraw(); }
    }
    ...
    RootView* View::Root() const
    {
        ...
        while (view != nullptr)
        {
            if (RootView* root = Cast<RootView>(view)) { return root; }
            view = view->Parent;
        }
        return nullptr;
    }
    ...
    bool View::IsEffectivelyVisible() const
    {
        ...
        while (v != nullptr)
        {
            if (v->Visibility != VisibilityValue::Visible) { return false; }
            if (Cast<RootView>(const_cast<View*>(v)) != nullptr) { return true; }
            v = v->Parent;
        }
        return false;
    }
    ...
    StyleValue View::ResolveStyle(StyleProperty prop)
    {
        if (m_inlineSheet)
        {
            ...
            if (r.GetKind() != StyleValue::Kind::None) { return r; }
        }
        ...
        if (IsInheritableStyle(prop) && Parent != nullptr) { return Parent->ResolveStyle(prop); }
    ...
    {
        if (Context == nullptr || IsPendingDeletion) { return; }
        IsPendingDeletion = true;
        View* self = this;
        Context->MutationQueueRef().QueueAction([self]()
        {
            if (self->Parent != nullptr) { if (ViewGroup* pg = Cast<ViewGroup>(self->Parent)) { pg->RemoveView(self, false); } }
    ...
    void View::QueueDestroy()
    {
        ...
        Context->MutationQueueRef().QueueAction([self]()
        {
            if (self->Parent != nullptr) { if (ViewGroup* pg = Cast<ViewGroup>(self->Parent)) { pg->RemoveView(self, true); } }
        });
    }
    ...
    ViewGroup* ViewGroup::AddView(View* child, LayoutParamsPtr lp)
    {
        ...
        for (const RefPtr<View>& c : m_children) { if (c.Get() == child) { return this; } }

        if (child->Parent != nullptr)
        {
            if (ViewGroup* oldParent = Cast<ViewGroup>(child->Parent)) { oldParent->RemoveView(child, false); }
        }

        if (lp) { child->LayoutParams = Move(lp); }
        else if (!child->LayoutParams) { child->LayoutParams = CreateDefaultLayoutParams(); }

        child->Parent = this;
        if (Context != nullptr) { Context->AttachView(child); }
        else { child->Context = nullptr; }
    ...
    void ViewGroup::RemoveView(View* child, bool deleteChild)
    {
        ...
        {
            if (m_children[i].Get() != child) { continue; }
            RefPtr<View> keepAlive = m_children[i]; // hold across Detach + Parent clear
            if (child->Context != nullptr) { child->Context->DetachView(child); }
    ...
    void ViewGroup::RemoveAllViews(bool deleteChildren)
    {
        ...
        {
            if (child->Context != nullptr) { child->Context->DetachView(child.Get()); }
            child->Parent = nullptr;
        }
        m_children.Clear();
        Invalidate();
    ...
    void ViewGroup::InsertView(View* child, usize index, LayoutParamsPtr lp)
    {
        ...
        for (const RefPtr<View>& c : m_children) { if (c.Get() == child) { return; } }
    ...
    void MutationQueue::QueueDelete(View* view)
    {
        ...
        QueueAction([view]()
        {
            if (view->Parent != nullptr) { if (ViewGroup* pg = Cast<ViewGroup>(view->Parent)) { pg->RemoveView(view, false); } }
        });
    }

    bool StyleSelector::Matches(const View& view, ControlState state, StringView pseudoElement) const
    {
        if (ViewType != nullptr && !IsDerivedFrom(view.GetType(), ViewType)) { return false; }
        for (const String& cls : StyleClasses) { if (!view.HasClass(cls.AsView())) { return false; } }
        if (State.HasValue())
        {
        ...
        else if (pseudoElement.Size() != 0u) { return false; }
        return true;
    }
