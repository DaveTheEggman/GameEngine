...
export namespace draconic::ui
{
    ...
    public:
        ...
        ProgressBar()
        {
            ...
            {
                const f32 clamped = Max(0.0f, Min(val, 1.0f));
    ...
    protected:
        ...
        {
            MeasuredSize = Float2{ constraints.ConstrainWidth(constraints.MaxWidth), constraints.ConstrainHeight(16.0f) };
        }
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            if (Drawable* track = ResolvePartDrawable(u8"track", StyleProperty::Background, state)) { track->Draw(ctx, bounds); }
            else { ctx.VG().FillRect(bounds, Color{ 50.0f / 255.0f, 52.0f / 255.0f, 62.0f / 255.0f, 1.0f }); }

            if (Value.Value() > 0)
            {
                const f32 fillW = Width() * Value.Value();
                ctx.VG().PushClipRect(Rectangle{ 0, 0, fillW, Height() });
                if (Drawable* fill = ResolvePartDrawable(u8"fill", StyleProperty::Background, state)) { fill->Draw(ctx, bounds); }
                else { ctx.VG().FillRect(bounds, Color{ 80.0f / 255.0f, 150.0f / 255.0f, 240.0f / 255.0f, 1.0f }); }
                ctx.VG().PopClip();
            }
        }
    };
