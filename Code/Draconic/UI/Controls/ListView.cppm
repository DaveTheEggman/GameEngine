...
export namespace draconic::ui
{
    ...
    public:
        ...
        ListView()
        {
            ...
            m_scrollBar->OnValueChanged.Add(Event<void(ScrollBar*, f32)>::Handler{ [self](ScrollBar*, f32 val) { self->m_scrollY = val; self->Invalidate(); } });
        }
        ...
        {
            if (m_adapter != nullptr) { m_adapter->SetObserver(nullptr); }
            m_adapter = adapter;
            if (m_adapter != nullptr) { m_adapter->SetObserver(this); }
            RebuildOffsets();
            RecycleAllActive();
            Invalidate();
        }
        ...
        {
            const f32 contentH = m_variableHeight ? m_totalContentHeight : ((m_adapter != nullptr) ? m_adapter->ItemCount() * ItemHeight.Value() : 0.0f);
        ...
        void ScrollBy(f32 dy) { m_scrollY = core::Clamp(m_scrollY + dy, 0.0f, MaxScrollY()); Invalidate(); }

        /// Rebuild visible items after adapter data changed.
        void NotifyDataChanged() { RebuildOffsets(); RecycleAllActive(); Invalidate(); }
        ...
        void OnItemRangeChanged(i32 start, i32 count) override
        {
            ...
            {
                if (RefPtr<View>* v = m_activeViews.Find(pos)) { m_adapter->BindView(v->Get(), pos); }
            }
            if (m_variableHeight) { RebuildOffsets(); Invalidate(); }
        }
        ...
        [[nodiscard]] View* GetVisualChild(usize index) const override
        {
            ...
            {
                usize i = 0;
                for (const auto& kv : m_activeViews) { if (i == index) { return kv.value.Get(); } ++i; }
            }
            if (index == m_activeViews.Size()) { return m_scrollBar.Get(); }
            return nullptr;
        }
        ...
        void OnMouseDown(MouseEventArgs& e) override
        {
            ...
            if (m_adapter != nullptr && e.Button == MouseButton::Right)
            {
                ...
                {
                    if (!Selection.IsSelected(itemIndex)) { Selection.Select(itemIndex); }
                    OnItemRightClicked.Invoke(itemIndex, local.x, local.y);
                }
                else { OnBackgroundRightClicked.Invoke(local.x, local.y); }
                e.Handled = true;
            }
            ...
            if (m_adapter != nullptr && e.Button == MouseButton::Left)
            {
                ...
                if (itemIndex >= 0 && itemIndex < m_adapter->ItemCount())
                {
                    if (HasFlag(e.Modifiers, KeyModifiers::Ctrl)) { Selection.Toggle(itemIndex); }
                    else if (HasFlag(e.Modifiers, KeyModifiers::Shift)) { Selection.SelectRange(Selection.FirstSelected(), itemIndex); }
                    else { Selection.Select(itemIndex); }
        ...
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            switch (e.Key)
            {
            case KeyCode::Down:  { const i32 next = Min(sel + 1, count - 1); if (shift) { Selection.SelectRange(sel, next); } else { Selection.Select(next); } ScrollToPosition(next); e.Handled = true; break; }
            case KeyCode::Up:    { const i32 prev = Max(sel - 1, 0); if (shift) { Selection.SelectRange(sel, prev); } else { Selection.Select(prev); } ScrollToPosition(prev); e.Handled = true; break; }
            case KeyCode::Home:  { if (shift) { Selection.SelectRange(sel, 0); } else { Selection.Select(0); } ScrollToPosition(0); e.Handled = true; break; }
            case KeyCode::End:   { if (shift) { Selection.SelectRange(sel, count - 1); } else { Selection.Select(count - 1); } ScrollToPosition(count - 1); e.Handled = true; break; }
            case KeyCode::PageDown: { const i32 page = static_cast<i32>(Height() / ItemHeight.Value()); const i32 n = Min(sel + page, count - 1); if (shift) { Selection.SelectRange(sel, n); } else { Selection.Select(n); } ScrollToPosition(n); e.Handled = true; break; }
            case KeyCode::PageUp:   { const i32 page = static_cast<i32>(Height() / ItemHeight.Value()); const i32 p = Max(sel - page, 0); if (shift) { Selection.SelectRange(sel, p); } else { Selection.Select(p); } ScrollToPosition(p); e.Handled = true; break; }
            default: break;
            }
        }
        ...
        {
            if (m_adapter == nullptr || position < 0 || position >= m_adapter->ItemCount()) { return; }
            const f32 itemTop = GetItemOffset(position);
            const f32 itemBottom = itemTop + GetItemHeightAt(position);
            const f32 viewportH = Height() - Padding.TotalVertical();
            if (itemTop < m_scrollY) { m_scrollY = itemTop; }
            else if (itemBottom > m_scrollY + viewportH) { m_scrollY = itemBottom - viewportH; }
        ...
        [[nodiscard]] i32 GetItemAtY(f32 localY) const
        {
            ...
            if (m_variableHeight) { return FindFirstVisible(scrolledY); }
    ...
    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            ...
            MeasuredSize = Float2{ constraints.ConstrainWidth(constraints.MaxWidth), constraints.ConstrainHeight(desiredH) };

            m_scrollBarVisible = MaxScrollY() > 0;
            m_scrollBar->Visibility = m_scrollBarVisible ? VisibilityValue::Visible : VisibilityValue::Gone;
        }
        ...
        void OnLayout(f32 left, f32 top, f32 width, f32 height) override
        {
            ...
            const f32 viewportH = height - Padding.TotalVertical();
            const f32 viewportW = width - Padding.TotalHorizontal() - (m_scrollBarVisible ? m_scrollBar->BarThickness : 0.0f);
            ...
            if (Context != nullptr && m_scrollBar->Context == nullptr) { Context->AttachView(m_scrollBar.Get()); }
            ...
            for (i32 pos = firstVis; pos <= lastVis; ++pos)
            {
                if (m_activeViews.Find(pos) == nullptr)
                {
                    ...
                    view->Parent = this;
                    if (Context != nullptr) { Context->AttachView(view.Get()); }
                    m_activeViews.InsertOrAssign(pos, Move(view));
                }
                else { m_adapter->BindView(m_activeViews.Find(pos)->Get(), pos); }
            ...
            if (m_scrollBarVisible)
            {
                ...
                m_scrollBar->Layout(width - m_scrollBar->BarThickness, Padding.Top, m_scrollBar->BarThickness, viewportH);
            }
        }
    ...
    public:
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            if (d.y != 0) { ScrollBy(d.y); }

            if (m_pressedItem >= 0 && !m_longPressFired && !m_dragging)
            {
                m_pressTime += dt;
                if (m_pressTime >= LongPressTime) { m_longPressFired = true; OnItemLongPress.Invoke(m_pressedItem); }
            }
    ...
    private:
        void RebuildOffsets()
        {
            ...
            for (i32 i = 0; i < count; ++i)
            {
                ...
                if (h > 0) { m_variableHeight = true; offset += h; }
                else { offset += ItemHeight.Value(); }
            }
            m_itemOffsets.PushBack(offset);
        ...
        {
            if (m_variableHeight && static_cast<usize>(position) < m_itemOffsets.Size()) { return m_itemOffsets[static_cast<usize>(position)]; }
            return position * ItemHeight.Value();
        }
        [[nodiscard]] f32 GetItemHeightAt(i32 position) const
        {
            if (m_variableHeight && m_adapter != nullptr) { const f32 h = m_adapter->GetItemHeight(position); if (h > 0) { return h; } }
            return ItemHeight.Value();
        }

        [[nodiscard]] i32 FindFirstVisible(f32 scrollY) const
        {
            if (!m_variableHeight || m_itemOffsets.Size() <= 1) { return static_cast<i32>(scrollY / ItemHeight.Value()); }
            i32 lo = 0, hi = static_cast<i32>(m_itemOffsets.Size() - 2);
            while (lo < hi)
            {
                const i32 mid = (lo + hi + 1) / 2;
                if (m_itemOffsets[static_cast<usize>(mid)] <= scrollY) { lo = mid; }
                else { hi = mid - 1; }
            }
            return lo;
        }
        ...
        {
            Array<i32> toRemove;
            for (const auto& kv : m_activeViews) { if (kv.key < first || kv.key > last) { toRemove.PushBack(kv.key); } }
            for (i32 pos : toRemove)
            {
                ...
                View* v = slot->Get();
                if (v->Context != nullptr) { v->Context->DetachView(v); }
                v->Parent = nullptr;
        ...
        void RecycleAllActive()
        {
            ...
            {
                const i32 viewType = (m_adapter != nullptr) ? m_adapter->GetItemViewType(kv.key) : 0;
                View* v = kv.value.Get();
                if (v->Context != nullptr) { v->Context->DetachView(v); }
                v->Parent = nullptr;
