...
export namespace draconic::ui
{
    ...
    public:
        ...
        Expander() { IsFocusable = true; Cursor = CursorType::Hand; HeaderHeight.SetOwner(this); ContentSpacing.SetOwner(this); }
        ...
        void SetIsExpanded(bool value)
        {
            ...
            m_isExpanded = value;
            if (m_content != nullptr) { m_content->Visibility = m_isExpanded ? Visibility::Visible : Visibility::Gone; }
            Invalidate();
        ...
        void SetHeaderText(StringView text) { m_headerText = String(text); Invalidate(); }
        ...
        void OnMouseDown(MouseEventArgs& e) override
        {
            ...
            if (local.y >= 0 && local.y <= HeaderHeight.Value()) { Toggle(); e.Handled = true; }
        }
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            switch (e.Key)
            {
            case KeyCode::Space: case KeyCode::Return: Toggle(); e.Handled = true; break;
            case KeyCode::Right: if (!m_isExpanded) { SetIsExpanded(true); e.Handled = true; } break;
            case KeyCode::Left:  if (m_isExpanded) { SetIsExpanded(false); e.Handled = true; } break;
            default: break;
            }
        }
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            if (Drawable* header = ResolvePartDrawable(u8"header", StyleProperty::Background, headerState)) { header->Draw(ctx, headerRect, headerState); }
            else { ctx.VG().FillRect(headerRect, Color{ 50.0f / 255.0f, 55.0f / 255.0f, 68.0f / 255.0f, 1.0f }); }

            const f32 arrowSize = 8.0f, arrowX = 8.0f, arrowCY = HeaderHeight.Value() * 0.5f;
            ControlState chevronState = headerState;
            if (m_isExpanded) { chevronState |= ControlState::Checked; }
            const Color arrowColor = ResolvePartColor(u8"chevron", StyleProperty::TextColor, chevronState, Color{ 180.0f / 255.0f, 185.0f / 255.0f, 200.0f / 255.0f, 1.0f });
            if (Drawable* chevron = ResolvePartDrawable(u8"chevron", StyleProperty::Background, chevronState))
            {
                chevron->Draw(ctx, Rectangle{ arrowX, arrowCY - arrowSize * 0.5f, arrowSize, arrowSize });
            }
            else
            {
                ctx.VG().BeginPath();
    ...
    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            ...
            if (m_content != nullptr && m_content->Visibility != Visibility::Gone)
            {
                ...
                const Thickness margin = m_content->LayoutParams ? m_content->LayoutParams->Margin : Thickness{};
                m_content->Measure(inner.Deflate(margin));
                contentH = ContentSpacing.Value() + m_content->MeasuredSize.y + margin.TotalVertical();
            }
        ...
        void OnLayout(f32 left, f32 top, f32 width, f32 height) override
        {
            ...
            if (m_content != nullptr && m_content->Visibility != Visibility::Gone)
            {
                ...
                m_content->Layout(margin.Left, contentTop + margin.Top,
                                  Max(0.0f, width - margin.TotalHorizontal()), Max(0.0f, height - contentTop - margin.TotalVertical()));
            }
        }
