...
export namespace draconic::ui
{
    ...
    public:
        ...
        [[nodiscard]] f32 MaxScrollY() const { return core::Max(0.0f, m_totalContentHeight - (Height() - Padding.TotalVertical())); }

        void ScrollBy(f32 dy) { m_scrollY = core::Clamp(m_scrollY + dy, 0.0f, MaxScrollY()); Invalidate(); }

        // === IListAdapterObserver ===
        void OnDataSetChanged() override { RecycleAllActive(); Invalidate(); }
        ...
        void OnMouseDown(MouseEventArgs& e) override
        {
            ...
            else if (e.Button == MouseButton::Left)
            {
                if (pos >= 0 && pos < m_adapter->ItemCount())
                {
                    if (HasFlag(e.Modifiers, KeyModifiers::Ctrl)) { Selection.Toggle(pos); }
                    else if (HasFlag(e.Modifiers, KeyModifiers::Shift)) { Selection.SelectRange(Selection.FirstSelected(), pos); }
                    else { Selection.Select(pos); }
        ...
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            case KeyCode::Left:  if (sel > 0) { Selection.Select(sel - 1); } ScrollToPosition(Selection.FirstSelected()); e.Handled = true; break;
            case KeyCode::Down:  { const i32 next = Min(sel + m_columnsCount, count - 1); Selection.Select(next); ScrollToPosition(next); e.Handled = true; break; }
            case KeyCode::Up:    { const i32 prev = Max(sel - m_columnsCount, 0); Selection.Select(prev); ScrollToPosition(prev); e.Handled = true; break; }
            case KeyCode::Home:  Selection.Select(0); ScrollToPosition(0); e.Handled = true; break;
            case KeyCode::End:   Selection.Select(count - 1); ScrollToPosition(count - 1); e.Handled = true; break;
            case KeyCode::PageDown: { const i32 rows = static_cast<i32>(Height() / (CellHeight.Value() + CellSpacing.Value())); const i32 n = Min(sel + rows * m_columnsCount, count - 1); Selection.Select(n); ScrollToPosition(n); e.Handled = true; break; }
            case KeyCode::PageUp:   { const i32 rows = static_cast<i32>(Height() / (CellHeight.Value() + CellSpacing.Value())); const i32 p = Max(sel - rows * m_columnsCount, 0); Selection.Select(p); ScrollToPosition(p); e.Handled = true; break; }
            default: break;
            }
        }
        ...
        [[nodiscard]] i32 GetItemAtPoint(f32 localX, f32 localY) const
        {
            ...
            const i32 row = static_cast<i32>(scrolledY / (CellHeight.Value() + CellSpacing.Value()));
            if (col < 0 || col >= m_columnsCount) { return -1; }
        ...
        {
            if (m_adapter == nullptr || m_columnsCount <= 0 || position < 0) { return; }
            ...
            if (rowY < m_scrollY) { m_scrollY = rowY; }
            else if (rowY + CellHeight.Value() > m_scrollY + viewportH) { m_scrollY = rowY + CellHeight.Value() - viewportH; }
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            if (m_adapter != nullptr)
            {
                ...
                const f32 cr = ResolveStyleFloat(StyleProperty::CornerRadius, 0.0f);   // round the tile in the rounded theme
    ...
    protected:
        ...
        {
            MeasuredSize = Float2{ constraints.ConstrainWidth(constraints.MaxWidth), constraints.ConstrainHeight(constraints.MaxHeight) };
        }
        ...
        void OnLayout(f32 left, f32 top, f32 width, f32 height) override
        {
            ...
            m_columnsCount = core::Max(1, static_cast<i32>((viewportW + CellSpacing.Value()) / (CellWidth.Value() + CellSpacing.Value())));
            ...
            m_totalContentHeight = (m_rowCount > 0) ? m_rowCount * (CellHeight.Value() + CellSpacing.Value()) - CellSpacing.Value() : 0.0f;

            m_scrollBarVisible = MaxScrollY() > 0;
            m_scrollBar->Visibility = m_scrollBarVisible ? VisibilityValue::Visible : VisibilityValue::Gone;
            m_scrollY = core::Clamp(m_scrollY, 0.0f, MaxScrollY());

            if (m_adapter == nullptr || itemCount == 0) { return; }
            ...
            const i32 lastRow = Min(firstRow + static_cast<i32>(viewportH / rowStride) + 1, m_rowCount - 1);
