    ...
    protected:
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            if (Drawable* boxDrawable = ResolvePartDrawable(u8"box", StyleProperty::Background, state))
            {
                ...
                if (IsChecked.Value())
                {
                    if (Drawable* dotIcon = ResolvePartDrawable(u8"mark", StyleProperty::Background, state)) { dotIcon->Draw(ctx, boxRect); }
                }
            }
            else if (IsChecked.Value()) { DrawFallbackChecked(ctx, boxRect, cy); }
