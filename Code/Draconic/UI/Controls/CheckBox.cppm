...
export namespace draconic::ui
{
    ...
    protected:
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            if (Drawable* boxDrawable = ResolvePartDrawable(u8"box", StyleProperty::Background, state))
            {
                boxDrawable->Draw(ctx, boxRect, state);
                if (IsChecked.Value())
                {
                    if (Drawable* checkIcon = ResolvePartDrawable(u8"checkmark", StyleProperty::Background, state))
                    { checkIcon->Draw(ctx, boxRect); }
                }
            }
            else if (IsChecked.Value()) { DrawFallbackChecked(ctx, boxRect, boxSize); }
            else { DrawFallbackUnchecked(ctx, boxRect); }
            ...
            if (text.Size() > 0 && ctx.FontService() != nullptr)
            {
                if (fonts::CachedFont* font = ctx.FontService()->GetFont(ResolveStyleFontFamily(FontFamily.Value()), fontSize))
                {
                    ...
                    ctx.VG().DrawText(text, font, textRect, fonts::TextAlignment::Left, fonts::VerticalAlignment::Middle, textColor);
                }
            }
        }
    ...
    private:
        static void DrawFallbackUnchecked(UIDrawContext& ctx, const Rectangle& boxRect)
        {
            ...
            ctx.VG().StrokeRect(boxRect, Color{ 100.0f / 255.0f, 105.0f / 255.0f, 120.0f / 255.0f, 1.0f }, 1.0f);
        }
        static void DrawFallbackChecked(UIDrawContext& ctx, const Rectangle& boxRect, f32 boxSize)
        {
            ctx.VG().FillRect(boxRect, Color{ 80.0f / 255.0f, 150.0f / 255.0f, 240.0f / 255.0f, 1.0f });
