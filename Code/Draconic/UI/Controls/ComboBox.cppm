...
export namespace draconic::ui
{
    ...
    public:
        ...
        void SetSelectedIndex(i32 value)
        {
            ...
            if (m_selectedIndex != clamped) { m_selectedIndex = clamped; Invalidate(); OnSelectionChanged.Invoke(this, clamped); }
        }
        [[nodiscard]] StringView SelectedText() const
        {
            return (m_selectedIndex >= 0 && m_selectedIndex < static_cast<i32>(m_items.Size())) ? m_items[static_cast<usize>(m_selectedIndex)].AsView() : StringView{};
        }
        ...
        [[nodiscard]] StringView ItemAt(i32 index) const { return m_items[static_cast<usize>(index)].AsView(); }

        i32 AddItem(StringView text) { const i32 index = static_cast<i32>(m_items.Size()); m_items.PushBack(String(text)); Invalidate(); return index; }
        void RemoveItem(i32 index)
        {
            if (index < 0 || index >= static_cast<i32>(m_items.Size())) { return; }
            m_items.RemoveAt(static_cast<usize>(index));
            if (m_selectedIndex >= static_cast<i32>(m_items.Size())) { m_selectedIndex = static_cast<i32>(m_items.Size()) - 1; }
            Invalidate();
        }
        void ClearItems() { m_items.Clear(); m_selectedIndex = -1; Invalidate(); }

        void OpenDropdown();
        void CloseDropdown() { m_isOpen = false; Invalidate(); }

        // === IPopupOwner ===
        void OnPopupClosed(View*) override { m_isOpen = false; Invalidate(); }
        ...
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            if (e.Key == KeyCode::Space || e.Key == KeyCode::Return) { if (!m_isOpen) { OpenDropdown(); } e.Handled = true; }
            else if (e.Key == KeyCode::Up) { if (m_selectedIndex > 0) { SetSelectedIndex(m_selectedIndex - 1); } e.Handled = true; }
            else if (e.Key == KeyCode::Down) { if (m_selectedIndex < static_cast<i32>(m_items.Size()) - 1) { SetSelectedIndex(m_selectedIndex + 1); } e.Handled = true; }
            else if (e.Key == KeyCode::Escape && m_isOpen) { CloseDropdown(); e.Handled = true; }
        }
        void OnActivate() override { if (IsEffectivelyEnabled()) { OpenDropdown(); } }
    ...
    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            ...
            if (Context != nullptr && Context->FontService() != nullptr)
            {
                if (fonts::CachedFont* font = Context->FontService()->GetFont(ResolveStyleFontFamily(), fontSize))
                {
                    ...
                    for (const String& item : m_items) { maxTextW = Max(maxTextW, font->font->MeasureString(item)); }
                }
            }
            const Thickness padding{ 8, 6 };
            MeasuredSize = Float2{ constraints.ConstrainWidth(padding.TotalHorizontal() + maxTextW + m_arrowAreaWidth),
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            if (bg != nullptr) { bg->Draw(ctx, bounds, state); }
            else
            {
            ...
            if (m_isOpen)
            {
                const Color accent = ResolveStyleColor(StyleProperty::AccentColor, Color{ 80.0f / 255.0f, 160.0f / 255.0f, 1.0f, 1.0f });
                ...
                else
                {
                    ...
                    if (cr > 0) { ctx.VG().StrokeRoundedRect(bounds, cr, accent, 2.0f); } else { ctx.VG().StrokeRect(bounds, accent, 2.0f); }
                }
            }

            const f32 fontSize = ResolveStyleFloat(StyleProperty::FontSize, 14.0f);
            if (m_selectedIndex >= 0 && m_selectedIndex < static_cast<i32>(m_items.Size()) && ctx.FontService() != nullptr)
            {
                if (fonts::CachedFont* font = ctx.FontService()->GetFont(ResolveStyleFontFamily(), fontSize))
                {
                    ...
                    ctx.VG().DrawText(m_items[static_cast<usize>(m_selectedIndex)], font, Rectangle{ 8, 0, Width() - 16 - m_arrowAreaWidth, Height() }, fonts::TextAlignment::Left, fonts::VerticalAlignment::Middle, textColor);
                }
            }

            Drawable* arrowDrawable = ResolvePartDrawable(u8"arrow", StyleProperty::Background, state);
            const f32 arrowX = Width() - m_arrowAreaWidth * 0.5f;
            const f32 arrowY = Height() * 0.5f;
            if (arrowDrawable != nullptr) { arrowDrawable->Draw(ctx, Rectangle{ arrowX - 4.0f, arrowY - 4.0f, 8.0f, 8.0f }); }
            else
            {
                const Color arrowColor = ResolvePartColor(u8"arrow", StyleProperty::TextColor, state, Color{ 180.0f / 255.0f, 185.0f / 255.0f, 200.0f / 255.0f, 1.0f });
                const f32 a = 4.0f;
                ctx.VG().BeginPath();
    ...
    public:
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            {
                ctx.VG().FillRect(bounds, Color{ 45.0f / 255.0f, 48.0f / 255.0f, 58.0f / 255.0f, 1.0f });
                ctx.VG().StrokeRect(bounds, Color{ 70.0f / 255.0f, 75.0f / 255.0f, 90.0f / 255.0f, 1.0f }, 1.0f);
            }
            ...
            for (i32 i = 0; i < m_owner->ItemCount(); ++i)
            {
                ...
                if (i == m_owner->SelectedIndex())
                {
                    ...
                    {
                        if (!rrd->Radii.IsZero()) { ctx.VG().FillRoundedRect(itemRect, rrd->Radii, selectedColor); } else { ctx.VG().FillRect(itemRect, selectedColor); }
                    }
                    else { ctx.VG().FillRect(itemRect, selectedColor); }
                }
                if (i == m_hoveredIndex)
                {
                    if (hoverDrawable != nullptr) { hoverDrawable->Draw(ctx, itemRect); } else { ctx.VG().FillRect(itemRect, hoverColor); }
                }
                if (font != nullptr) { ctx.VG().DrawText(m_owner->ItemAt(i), font, Rectangle{ 12, y, Width() - 24, m_itemHeight }, fonts::TextAlignment::Left, fonts::VerticalAlignment::Middle, textColor); }
                y += m_itemHeight;
            }
        }
        ...
        void OnMouseMove(MouseEventArgs& e) override
        {
            ...
            if (newIndex != m_hoveredIndex) { m_hoveredIndex = newIndex; Invalidate(); }
        }
        ...
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            {
            case KeyCode::Up:
                if (m_hoveredIndex > 0) { m_hoveredIndex--; }
                else if (m_hoveredIndex < 0 && m_owner->ItemCount() > 0) { m_hoveredIndex = m_owner->ItemCount() - 1; }
                Invalidate(); e.Handled = true; break;
            case KeyCode::Down:
                if (m_hoveredIndex < m_owner->ItemCount() - 1) { m_hoveredIndex++; }
                else if (m_hoveredIndex < 0 && m_owner->ItemCount() > 0) { m_hoveredIndex = 0; }
                Invalidate(); e.Handled = true; break;
            case KeyCode::Return:
                if (m_hoveredIndex >= 0 && m_hoveredIndex < m_owner->ItemCount()) { m_owner->SetSelectedIndex(m_hoveredIndex); m_owner->CloseDropdown(); QueueSelfClose(); }
                e.Handled = true; break;
            case KeyCode::Escape:
                m_owner->CloseDropdown(); QueueSelfClose(); e.Handled = true; break;
            default: break;
            }
        }
    ...
    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            ...
            MeasuredSize = Float2{ constraints.ConstrainWidth(w), constraints.ConstrainHeight(totalH) };
        }
    ...
    {
        if (m_isOpen || m_items.Size() == 0 || Context == nullptr) { return; }
