...
export namespace draconic::ui
{
    ...
    public:
        ...
        explicit Separator(::draconic::ui::Orientation orientation) : Separator() { Orientation.SetSilent(orientation); }

    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            if (Orientation.Value() == ::draconic::ui::Orientation::Horizontal)
                MeasuredSize = Float2{ constraints.ConstrainWidth(constraints.MaxWidth), constraints.ConstrainHeight(SeparatorThickness.Value()) };
            else
                MeasuredSize = Float2{ constraints.ConstrainWidth(SeparatorThickness.Value()), constraints.ConstrainHeight(constraints.MaxHeight) };
        }
        void OnDraw(UIDrawContext& ctx) override
        {
            const Color color = ResolveStyleColor(StyleProperty::BorderColor, Color{ 80.0f / 255.0f, 80.0f / 255.0f, 90.0f / 255.0f, 1.0f });
