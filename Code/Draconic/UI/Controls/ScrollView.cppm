...
export namespace draconic::ui
{
    ...
    enum class ScrollBarPolicy { Never, Always, Auto };
    ...
    public:
        ...
        ScrollView()
        {
            ...
            m_vBar->OnValueChanged.Add(Event<void(ScrollBar*, f32)>::Handler{ [self](ScrollBar*, f32 val) { self->SetScrollY(val); } });
            ...
            m_hBar->OnValueChanged.Add(Event<void(ScrollBar*, f32)>::Handler{ [self](ScrollBar*, f32 val) { self->SetScrollX(val); } });
        }

        // === Scroll offsets (Beef properties -> methods) ===
        [[nodiscard]] f32 ScrollX() const noexcept { return m_scrollX; }
        void SetScrollX(f32 value) { const f32 c = core::Clamp(value, 0.0f, MaxScrollX()); if (m_scrollX == c) { return; } m_scrollX = c; Invalidate(); }
        [[nodiscard]] f32 ScrollY() const noexcept { return m_scrollY; }
        void SetScrollY(f32 value) { const f32 c = core::Clamp(value, 0.0f, MaxScrollY()); if (m_scrollY == c) { return; } m_scrollY = c; Invalidate(); }

        [[nodiscard]] f32 MaxScrollX() const { return core::Max(0.0f, m_contentWidth - ViewportWidth()); }
        [[nodiscard]] f32 MaxScrollY() const { return core::Max(0.0f, m_contentHeight - ViewportHeight()); }
        ...
        {
            const f32 barSpace = (ScrollBarMode.Value() == ScrollBarModeValue::Reserved && NeedsVBar()) ? ScrollBarThickness.Value() : 0.0f;
            return core::Max(0.0f, Width() - Padding.TotalHorizontal() - barSpace);
        }
        [[nodiscard]] f32 ViewportHeight() const
        {
            const f32 barSpace = (ScrollBarMode.Value() == ScrollBarModeValue::Reserved && NeedsHBar()) ? ScrollBarThickness.Value() : 0.0f;
            return core::Max(0.0f, Height() - Padding.TotalVertical() - barSpace);
        }

        // === Scroll commands ===
        void ScrollTo(f32 x, f32 y) { SetScrollX(x); SetScrollY(y); m_momentum.Stop(); }
        void ScrollToTop() { SetScrollY(0); m_momentum.Stop(); }
        void ScrollToBottom() { SetScrollY(MaxScrollY()); m_momentum.Stop(); }
        void ScrollToLeft() { SetScrollX(0); m_momentum.Stop(); }
        void ScrollToRight() { SetScrollX(MaxScrollX()); m_momentum.Stop(); }
        void ScrollBy(f32 dx, f32 dy) { SetScrollX(m_scrollX + dx); SetScrollY(m_scrollY + dy); }
        ...
        void ScrollToView(View* child)
        {
            ...
            View* current = child;
            while (current != nullptr && current != this) { offsetX += current->Bounds.x; offsetY += current->Bounds.y; current = current->Parent; }
            if (current == nullptr) { return; } // not a descendant

            offsetX += m_scrollX; offsetY += m_scrollY; // bounds are laid out scroll-adjusted

            const f32 childRight = offsetX + child->Width();
            if (offsetX < m_scrollX) { SetScrollX(offsetX); }
            else if (childRight > m_scrollX + ViewportWidth()) { SetScrollX(childRight - ViewportWidth()); }

            const f32 childBottom = offsetY + child->Height();
            if (offsetY < m_scrollY) { SetScrollY(offsetY); }
            else if (childBottom > m_scrollY + ViewportHeight()) { SetScrollY(childBottom - ViewportHeight()); }
        ...
        [[nodiscard]] View* GetVisualChild(usize index) const override
        {
            ...
            if (index == ChildCount()) { return m_vBar.Get(); }
            if (index == ChildCount() + 1) { return m_hBar.Get(); }
            return nullptr;
        }
        ...
        void OnMouseWheel(MouseWheelEventArgs& e) override
        {
            ...
            f32 hDelta = 0;
            if (e.DeltaX != 0) { hDelta = e.DeltaX; }
            else if (NeedsHBar() && e.DeltaY != 0 && HasFlag(e.Modifiers, KeyModifiers::Shift)) { hDelta = e.DeltaY; }
            else if (NeedsHBar() && !NeedsVBar() && e.DeltaY != 0) { hDelta = e.DeltaY; }

            if (NeedsHBar() && hDelta != 0)
            {
                SetScrollX(m_scrollX - hDelta * scrollAmount);
                if (MomentumEnabled.Value()) { m_momentum.VelocityX = -hDelta * scrollAmount * 3; }
                e.Handled = true;
            }
            else if (NeedsVBar() && e.DeltaY != 0)
            {
                SetScrollY(m_scrollY - e.DeltaY * scrollAmount);
                if (MomentumEnabled.Value()) { m_momentum.VelocityY = -e.DeltaY * scrollAmount * 3; }
                e.Handled = true;
            }
        }
        ...
        {
            (void)e;
            if (m_dragging) { m_dragging = false; if (Context != nullptr) { Context->GetFocusManager()->ReleaseCapture(); } }
        }
        ...
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            switch (e.Key)
            {
            case KeyCode::Up:       SetScrollY(m_scrollY - 40); e.Handled = true; break;
            case KeyCode::Down:     SetScrollY(m_scrollY + 40); e.Handled = true; break;
            case KeyCode::PageUp:   SetScrollY(m_scrollY - pageSize); e.Handled = true; break;
            case KeyCode::PageDown: SetScrollY(m_scrollY + pageSize); e.Handled = true; break;
            case KeyCode::Home:     ScrollToTop(); e.Handled = true; break;
            case KeyCode::End:      ScrollToBottom(); e.Handled = true; break;
            default: break;
            }
        }
    ...
    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            ...
            const f32 childMaxW = (HScrollBarPolicy.Value() == ScrollBarPolicy::Always) ? kFloatMax : fullW;
            const f32 childMaxH = (VScrollBarPolicy.Value() == ScrollBarPolicy::Never) ? fullH : kFloatMax;
            ...
            {
                const bool needsVBar = (VScrollBarPolicy.Value() == ScrollBarPolicy::Always) || (VScrollBarPolicy.Value() == ScrollBarPolicy::Auto && maxH > fullH);
                const bool needsHBar = (HScrollBarPolicy.Value() == ScrollBarPolicy::Always) || (HScrollBarPolicy.Value() == ScrollBarPolicy::Auto && maxW > fullW);
                if (needsVBar || needsHBar)
                {
                    const f32 adjustedW = core::Max(0.0f, fullW - (needsVBar ? ScrollBarThickness.Value() : 0.0f));
                    const f32 adjustedH = core::Max(0.0f, fullH - (needsHBar ? ScrollBarThickness.Value() : 0.0f));
                    const f32 adjChildMaxW = (HScrollBarPolicy.Value() == ScrollBarPolicy::Always) ? kFloatMax : adjustedW;
                    const f32 adjChildMaxH = (VScrollBarPolicy.Value() == ScrollBarPolicy::Never) ? adjustedH : kFloatMax;
                    maxW = 0; maxH = 0;
            ...
            const f32 measuredW = (HScrollBarPolicy.Value() == ScrollBarPolicy::Never) ? maxW + Padding.TotalHorizontal() : constraints.MaxWidth;
            f32 measuredH;
            if (VScrollBarPolicy.Value() == ScrollBarPolicy::Never)
            {
                const f32 hBarH = (HScrollBarPolicy.Value() != ScrollBarPolicy::Never && ScrollBarMode.Value() == ScrollBarModeValue::Reserved) ? ScrollBarThickness.Value() : 0.0f;
                measuredH = maxH + Padding.TotalVertical() + hBarH;
            }
            else { measuredH = constraints.MaxHeight; }

            MeasuredSize = Float2{ constraints.ConstrainWidth(measuredW), constraints.ConstrainHeight(measuredH) };
        }
        ...
        void OnLayout(f32 left, f32 top, f32 width, f32 height) override
        {
            ...
            m_vBar->Parent = this;
            m_hBar->Parent = this;
            if (Context != nullptr && m_vBar->Context == nullptr) { Context->AttachView(m_vBar.Get()); }
            if (Context != nullptr && m_hBar->Context == nullptr) { Context->AttachView(m_hBar.Get()); }
            ...
            for (usize i = 0; i < ChildCount(); ++i)
            {
                ...
                const f32 childW = core::Max(child->MeasuredSize.x, ViewportWidth() - margin.TotalHorizontal());
                const f32 childH = child->MeasuredSize.y;
                child->Layout(Padding.Left + margin.Left - m_scrollX, Padding.Top + margin.Top - m_scrollY, childW, childH);
            }
            ...
            if (NeedsVBar())
            {
                ...
                m_vBar->Measure(BoxConstraints::Tight(ScrollBarThickness.Value(), ViewportHeight()));
                m_vBar->Layout(width - ScrollBarThickness.Value(), Padding.Top, ScrollBarThickness.Value(), ViewportHeight());
            }
            if (NeedsHBar())
            {
                ...
                m_hBar->Layout(Padding.Left, height - ScrollBarThickness.Value(), ViewportWidth(), ScrollBarThickness.Value());
            }
        }
    ...
    private:
        ...
        {
            if (VScrollBarPolicy.Value() == ScrollBarPolicy::Never) { return false; }
            if (VScrollBarPolicy.Value() == ScrollBarPolicy::Always) { return true; }
            return m_contentHeight > Height() - Padding.TotalVertical();
        }
        [[nodiscard]] bool NeedsHBar() const
        {
            if (HScrollBarPolicy.Value() == ScrollBarPolicy::Never) { return false; }
            if (HScrollBarPolicy.Value() == ScrollBarPolicy::Always) { return true; }
        ...
        {
            if (Context == nullptr || Context->GetInputManager() == nullptr) { return Float2{ 0, 0 }; }
