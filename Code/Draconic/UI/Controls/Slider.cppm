...
export namespace draconic::ui
{
    ...
    public:
        ...
        {
            if (m_dragging) { UpdateValueFromMouse(e.X, e.Y); e.Handled = true; }
        }
        void OnKeyDown(KeyEventArgs& e) override
        {
            ...
            switch (e.Key)
            {
            case KeyCode::Right: case KeyCode::Up:   Value.SetValue(Value.Value() + smallStep); e.Handled = true; break;
            case KeyCode::Left:  case KeyCode::Down: Value.SetValue(Value.Value() - smallStep); e.Handled = true; break;
            case KeyCode::Home: Value.SetValue(Min.Value()); e.Handled = true; break;
            case KeyCode::End:  Value.SetValue(Max.Value()); e.Handled = true; break;
            default: break;
            }
        }
    ...
    protected:
        void OnMeasure(BoxConstraints constraints) override
        {
            if (Orientation.Value() == ::draconic::ui::Orientation::Horizontal)
                MeasuredSize = Float2{ constraints.ConstrainWidth(constraints.MaxWidth), constraints.ConstrainHeight(20.0f) };
            else
                MeasuredSize = Float2{ constraints.ConstrainWidth(20.0f), constraints.ConstrainHeight(constraints.MaxHeight) };
        }
        ...
        void OnDraw(UIDrawContext& ctx) override
        {
            ...
            Drawable* fillDrawable = ResolvePartDrawable(u8"fill", StyleProperty::Background, state);
            Drawable* thumbDrawable = ResolvePartDrawable(u8"thumb", StyleProperty::Background, state);
            const f32 progress = (Max.Value() > Min.Value()) ? (Value.Value() - Min.Value()) / (Max.Value() - Min.Value()) : 0.0f;
            ...
            if (Orientation.Value() == ::draconic::ui::Orientation::Horizontal)
            {
                ...
                if (trackDrawable) { trackDrawable->Draw(ctx, trackRect); } else { ctx.VG().FillRect(trackRect, trackCol); }
                const f32 fillW = trackW * progress;
                if (fillW > 0) { const Rectangle fr{ trackLeft, trackY, fillW, trackHeight }; if (fillDrawable) { fillDrawable->Draw(ctx, fr); } else { ctx.VG().FillRect(fr, fillCol); } }
                const f32 thumbX = trackLeft + trackW * progress;
                const Rectangle thumbRect{ thumbX - thumbHalf, Height() * 0.5f - thumbHalf, thumbSize, thumbSize };
                if (thumbDrawable) { thumbDrawable->Draw(ctx, thumbRect); } else { ctx.VG().FillCircle(Float2{ thumbX, Height() * 0.5f }, thumbHalf, thumbCol); }
            }
            else
            {
                ...
                if (fillH > 0) { const Rectangle fr{ trackX, (trackTop + trackH) - fillH, trackHeight, fillH }; if (fillDrawable) { fillDrawable->Draw(ctx, fr); } else { ctx.VG().FillRect(fr, fillCol); } }
                const f32 thumbY = (trackTop + trackH) - trackH * progress;
                const Rectangle thumbRect{ Width() * 0.5f - thumbHalf, thumbY - thumbHalf, thumbSize, thumbSize };
                if (thumbDrawable) { thumbDrawable->Draw(ctx, thumbRect); } else { ctx.VG().FillCircle(Float2{ Width() * 0.5f, thumbY }, thumbHalf, thumbCol); }
            }
        }
    ...
    private:
        void Init()
        {
            IsFocusable = true; IsTabStop = true; WantsArrowKeys = true; Cursor = CursorType::Hand;
            ...
            {
                const f32 clamped = self->SnapToStep(core::Max(self->Min.Value(), core::Min(val, self->Max.Value())));
                if (clamped != val) { self->Value.SetSilent(clamped); }
            ...
            Step.Changed.Add(Event<void(f32)>::Handler{ [self](f32 val) { self->Step.SetSilent(core::Max(0.0f, val)); self->ReclampValue(); } });
        }

        void ReclampValue()
        {
            const f32 clamped = SnapToStep(core::Max(Min.Value(), core::Min(Value.Value(), Max.Value())));
            if (clamped != Value.Value()) { Value.SetValue(clamped); }
        }

        void UpdateValueFromMouse(f32 localX, f32 localY)
        {
            const f32 thumbSize = ResolvePartFloat(u8"thumb", StyleProperty::Width, GetControlState(), 16.0f);
            const f32 thumbHalf = thumbSize * 0.5f;
            f32 progress;
            if (Orientation.Value() == ::draconic::ui::Orientation::Horizontal)
            {
            ...
            Value.SetValue(Min.Value() + (Max.Value() - Min.Value()) * core::Max(0.0f, core::Min(progress, 1.0f)));
        }
