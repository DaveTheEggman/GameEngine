...
export namespace draconic::ui
{
    ...
    enum class EditActionType { None, CharInsert, Delete, Paste, Cut };
    ...
    public:
        ...
        void SetCursorPosition(i32 value) { m_cursorPos = Clamp(value, i32{ 0 }, m_host->TextCharCount()); }
        [[nodiscard]] i32 AnchorPosition() const noexcept { return m_anchorPos; }
        void SetAnchorPosition(i32 value) { m_anchorPos = Clamp(value, i32{ 0 }, m_host->TextCharCount()); }
        ...
        [[nodiscard]] InputFilter* Filter() { return m_inputFilter.HasValue() ? &m_inputFilter.Value() : nullptr; }
        ...
        {
            if (m_host->GetIsReadOnly()) { return; }

            // Filter control characters (allow tab).
            if (character < 32 && character != U'\t') { return; }

            // Apply input filter.
            if (m_inputFilter.HasValue() && !m_inputFilter.Value().Accept(character)) { return; }

            // MaxLength check.
            if (m_host->GetMaxLength() > 0)
            {
                const i32 availableChars = m_host->GetMaxLength() - m_host->TextCharCount() + SelectionLength();
        ...
        void HandleKeyDown(KeyCode key, KeyModifiers mods)
        {
            ...
            case KeyCode::Left:
                ...
                else if (!shift && HasSelection()) { const i32 pos = SelectionStart(); m_cursorPos = pos; m_anchorPos = pos; }
                else { MoveCursor(m_cursorPos - 1, shift); }
                break;
            case KeyCode::Right:
                BreakMergeChain();
                if (ctrl) { MoveWordRight(shift); }
                else if (!shift && HasSelection()) { const i32 pos = SelectionEnd(); m_cursorPos = pos; m_anchorPos = pos; }
                else { MoveCursor(m_cursorPos + 1, shift); }
                break;
            case KeyCode::Up:
                if (m_host->IsMultiline()) { BreakMergeChain(); MoveLineUp(shift); }
                break;
            case KeyCode::Down:
                if (m_host->IsMultiline()) { BreakMergeChain(); MoveLineDown(shift); }
                break;
            case KeyCode::Return:
            ...
            case KeyCode::Backspace:
                if (m_host->GetIsReadOnly()) { return; }
                if (HasSelection()) { PushUndoIfNeeded(EditActionType::Delete); DeleteSelectionText(); m_host->OnTextModified(); }
                else if (ctrl) { PushUndoIfNeeded(EditActionType::Delete); DeleteWordBackward(); }
                else { PushUndoIfNeeded(EditActionType::Delete); DeleteBackward(); }
                break;
            case KeyCode::Delete:
                if (m_host->GetIsReadOnly()) { return; }
                if (HasSelection()) { PushUndoIfNeeded(EditActionType::Delete); DeleteSelectionText(); m_host->OnTextModified(); }
                else if (ctrl) { PushUndoIfNeeded(EditActionType::Delete); DeleteWordForward(); }
                else { PushUndoIfNeeded(EditActionType::Delete); DeleteForward(); }
                break;
            case KeyCode::A:
                if (ctrl) { SelectAll(); }
                break;
            case KeyCode::C:
                ...
                break;
            case KeyCode::V:
                if (ctrl && !m_host->GetIsReadOnly()) { PasteFromClipboard(); }
                break;
            case KeyCode::X:
                if (ctrl && !m_host->GetIsReadOnly()) { CutToClipboard(); }
                break;
            case KeyCode::Z:
        ...
        void HandleMouseDown(f32 localX, f32 localY, i32 clickCount, KeyModifiers mods)
        {
            ...
            if (clickCount == 3) { SelectAll(); }
            else if (clickCount == 2) { SelectWord(pos); }
            else if (HasFlag(mods, KeyModifiers::Shift)) { m_cursorPos = pos; } // extend selection
            else { m_cursorPos = pos; m_anchorPos = pos; } // set cursor, clear selection
        }
    ...
    private:
        ...
        void MoveCursor(i32 newPos, bool extendSelection)
        {
            ...
            if (!extendSelection) { m_anchorPos = m_cursorPos; }
        }

        void MoveWordLeft(bool extendSelection) { MoveCursor(FindWordBoundaryLeft(m_cursorPos), extendSelection); }
        void MoveWordRight(bool extendSelection) { MoveCursor(FindWordBoundaryRight(m_cursorPos), extendSelection); }
        ...
        void MoveLineUp(bool extendSelection)
        {
            ...
            if (curY < lineH * 0.5f) { return; } // already on the first line
        ...
        {
            if (m_host->IsMultiline()) { MoveCursor(GetLineStart(m_cursorPos), extendSelection); }
            else { MoveCursor(0, extendSelection); }
        }

        void MoveEnd(bool extendSelection)
        {
            if (m_host->IsMultiline()) { MoveCursor(GetLineEnd(m_cursorPos), extendSelection); }
            else { MoveCursor(m_host->TextCharCount(), extendSelection); }
        }
        ...
        void SelectWord(i32 position)
        {
            ...
            {
                while (start > 0 && IsWordChar(chars[static_cast<usize>(start - 1)])) { start--; }
            }
            else
            {
                start = pos;
            ...
            // Find word end.
            i32 end = start;
            while (end < charCount && IsWordChar(chars[static_cast<usize>(end)])) { end++; }
        ...
        void CutToClipboard()
        {
            if (!HasSelection() || m_host->GetIsReadOnly() || !AllowClipboardCopy) { return; }
            CopyToClipboard();
        ...
        {
            IClipboard* clipboard = m_host->Clipboard();
            if (clipboard == nullptr || !clipboard->HasText()) { return; }

            String pasteText;
            if (!clipboard->GetText(pasteText).IsOk()) { return; }
            ...
            if (!m_host->IsMultiline())
            {
                ...
                {
                    if (ch == static_cast<utf8char>('\n') || ch == static_cast<utf8char>('\r')) { ch = static_cast<utf8char>(' '); }
                }
            }
            ...
            if (m_host->GetMaxLength() > 0)
            {
                const i32 available = m_host->GetMaxLength() - m_host->TextCharCount() + SelectionLength();
        ...
        void PushUndoIfNeeded(EditActionType actionType)
        {
            ...
            if (actionType != m_lastActionType) { shouldPush = true; }
            else if (actionType != EditActionType::CharInsert) { shouldPush = true; }
            else if (time - m_lastEditTime > UndoCoalesceTime) { shouldPush = true; }

            if (shouldPush) { m_undoStack.PushState(m_host->Text(), m_cursorPos, m_anchorPos); }
        ...
        void PerformUndo()
        {
            ...
            if (m_undoStack.Undo(m_host->Text(), m_cursorPos, m_anchorPos, restoredText, restoredCursor, restoredAnchor))
            {
        ...
        void PerformRedo()
        {
            ...
            if (m_undoStack.Redo(m_host->Text(), m_cursorPos, m_anchorPos, restoredText, restoredCursor, restoredAnchor))
            {
                m_host->ReplaceText(0, m_host->TextCharCount(), restoredText);
                m_cursorPos = Clamp(restoredCursor, i32{ 0 }, m_host->TextCharCount());
                m_anchorPos = Clamp(restoredAnchor, i32{ 0 }, m_host->TextCharCount());
        ...
        [[nodiscard]] i32 FindWordBoundaryLeft(i32 pos) const
        {
            ...
            i32 p = pos - 1;
            while (p >= 0 && !IsWordChar(chars[static_cast<usize>(p)]) && chars[static_cast<usize>(p)] != U'\n') { p--; }

            // If we hit a newline, stop after it.
            if (p >= 0 && chars[static_cast<usize>(p)] == U'\n') { return p + 1; }

            // Skip word chars going left.
            while (p >= 0 && IsWordChar(chars[static_cast<usize>(p)])) { p--; }
        ...
        [[nodiscard]] i32 FindWordBoundaryRight(i32 pos) const
        {
            ...
            i32 p = pos;
            while (p < charCount && IsWordChar(chars[static_cast<usize>(p)])) { p++; }

            // Skip non-word chars going right (stop at newlines).
            while (p < charCount && !IsWordChar(chars[static_cast<usize>(p)]) && chars[static_cast<usize>(p)] != U'\n') { p++; }
        ...
        {
            return (c >= U'0' && c <= U'9') || (c >= U'a' && c <= U'z') || (c >= U'A' && c <= U'Z') || c >= 0x80u;
        }
