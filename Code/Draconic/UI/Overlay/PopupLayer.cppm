...
export namespace draconic::ui
{
    ...
    public:
        ...
        {
            for (const PopupEntry& e : m_entries) { if (e.IsModal) { return true; } }
            return false;
        }
        [[nodiscard]] usize PopupCount() const noexcept { return m_entries.Size(); }
        [[nodiscard]] View* TopmostModalPopup() const
        {
            for (usize i = m_entries.Size(); i-- > 0;) { if (m_entries[i].IsModal) { return m_entries[i].Popup.Get(); } }
            return nullptr;
        }
        ...
        void ShowPopup(View* popup, IPopupOwner* owner, Function<Optional<Float2>(i32)> positionFactory,
                       bool closeOnClickOutside = true, bool isModal = false, bool ownsView = true)
        {
            ...
            for (i32 attempt = 0; attempt < 16; ++attempt)
            {
                ...
                bestX = pos.x; bestY = pos.y;
                if (pos.x >= 0 && pos.y >= 0 && pos.x + popupSize.x <= Width() && pos.y + popupSize.y <= Height())
                { placed = true; break; }
            }
            if (!placed)
            {
        ...
        void ClosePopup(View* popup)
        {
            ...
            for (usize i = 0; i < m_entries.Size(); ++i)
            {
                if (m_entries[i].Popup.Get() == popup)
                {
                    ...
                    if (popup->Context != nullptr) { popup->Context->DetachView(popup); }
                    popup->Parent = nullptr;
                    if (entry.Owner != nullptr) { entry.Owner->OnPopupClosed(popup); }
                    // entry (and its RefPtr) drops here -> popup destroyed iff no other ref (ownsView).

                    if (Context != nullptr) { Context->GetFocusManager()->PopFocus(); }
                    if (!HasModalPopup() && m_backdrop && m_backdrop->Parent != nullptr) { RemoveView(m_backdrop.Get(), false); }
                    Invalidate();
                    return;
                }
            }
        ...
        bool HandleClickOutside(View* hitView, i32 button)
        {
            ...
            for (View* v = hitView; v != nullptr; v = v->Parent)
            {
                if (v->Parent == this)
                {
                    for (usize i = 0; i < m_entries.Size(); ++i) { if (m_entries[i].Popup.Get() == v) { hitPopupIndex = static_cast<i32>(i); break; } }
                    break;
                }
            }
        ...
        void UpdatePopupPosition(View* popup, f32 x, f32 y)
        {
            ...
            {
                if (entry.Popup.Get() == popup) { entry.X = x; entry.Y = y; Invalidate(); return; }
            }
        }
        ...
        {
            if (m_entries.Size() == 0 && (!m_backdrop || m_backdrop->Parent == nullptr)) { return nullptr; }
            ...
            for (usize i = m_entries.Size(); i-- > 0;)
            {
                ...
                if (popupLocal.x >= 0 && popupLocal.y >= 0 && popupLocal.x < popup->Width() && popupLocal.y < popup->Height())
                {
                    if (View* hit = popup->HitTest(popupLocal)) { return hit; }
                }
            }
            if (HasModalPopup()) { return this; }
            return nullptr;
        }

        void OnDraw(UIDrawContext& ctx) override
        {
            if (m_backdrop && m_backdrop->Parent != nullptr && m_backdrop->Visibility == VisibilityValue::Visible)
            {
                ctx.VG().PushState();
            ...
            {
                View* popup = entry.Popup.Get();
                if (popup->Visibility != VisibilityValue::Visible) { continue; }
                ctx.VG().PushState();
                ctx.VG().Translate(entry.X, entry.Y);
                if (popup->Opacity < 1.0f) { ctx.VG().PushOpacity(popup->Opacity); }
                popup->OnDraw(ctx);
                if (popup->Opacity < 1.0f) { ctx.VG().PopOpacity(); }
                ctx.VG().PopState();
            }
        }
    ...
    protected:
        ...
        {
            (void)left; (void)top;
            if (m_backdrop && m_backdrop->Parent != nullptr) { m_backdrop->Layout(0, 0, width, height); }
            for (const PopupEntry& entry : m_entries)
            {
                View* popup = entry.Popup.Get();
                popup->Measure(BoxConstraints::Loose(width, height));
    ...
    private:
        void ShowPopupInternal(View* popup, IPopupOwner* owner, f32 x, f32 y, bool closeOnClickOutside, bool isModal, bool ownsView)
        {
            PopupEntry entry;
            ...
            if (isModal && !hadModal)
            {
                if (!m_backdrop) { m_backdrop = MakeRef<ModalBackdrop>(DefaultAllocator()); }
                if (m_backdrop->Parent == nullptr) { AddView(m_backdrop.Get()); }
            }

            if (Context != nullptr) { Context->GetFocusManager()->PushFocus(); }

            popup->Parent = this;
            if (Context != nullptr) { Context->AttachView(popup); }
            Invalidate();
        }
        ...
        void CloseDependentPopups(View* parent)
        {
            while (true)
            {
                ...
                for (const PopupEntry& entry : m_entries)
                {
                    ...
                    if (ownerView != nullptr && IsDescendant(ownerView, parent)) { toClose = entry.Popup.Get(); break; }
                }
                if (toClose == nullptr) { break; }
                ClosePopup(toClose);
            }
        }
        ...
        {
            for (View* p = child; p != nullptr; p = p->Parent) { if (p == ancestor) { return true; } }
            return false;
        }
