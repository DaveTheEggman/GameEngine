...
export namespace draconic::ui
{
    struct BoxConstraints
    {
        ...
        [[nodiscard]] static constexpr BoxConstraints Tight(f32 width, f32 height) noexcept { return { width, width, height, height }; }
        /// Zero minimum, specified maximum.
        [[nodiscard]] static constexpr BoxConstraints Loose(f32 maxWidth, f32 maxHeight) noexcept { return { 0.0f, maxWidth, 0.0f, maxHeight }; }
        /// Unconstrained on both axes.
        [[nodiscard]] static constexpr BoxConstraints Expand() noexcept { return { 0.0f, kFloatMax, 0.0f, kFloatMax }; }
        ...
        [[nodiscard]] f32 ConstrainWidth(f32 width) const noexcept { return Max(MinWidth, Min(width, MaxWidth)); }
        [[nodiscard]] f32 ConstrainHeight(f32 height) const noexcept { return Max(MinHeight, Min(height, MaxHeight)); }

        /// Whether both axes are tight (min == max).
        [[nodiscard]] constexpr bool IsTight() const noexcept { return MinWidth == MaxWidth && MinHeight == MaxHeight; }
        /// Whether both axes have zero minimum.
        [[nodiscard]] constexpr bool IsLoose() const noexcept { return MinWidth == 0.0f && MinHeight == 0.0f; }

        /// A loose version (zero min, same max).
        [[nodiscard]] constexpr BoxConstraints Loosen() const noexcept { return { 0.0f, MaxWidth, 0.0f, MaxHeight }; }
        /// Tight constraints at the maximum size.
        [[nodiscard]] constexpr BoxConstraints TightenToMax() const noexcept { return { MaxWidth, MaxWidth, MaxHeight, MaxHeight }; }
    };
}
