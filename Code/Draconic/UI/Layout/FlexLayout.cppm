...
export namespace draconic::ui
{
    ...
    enum class Align { Start, End, Center, Stretch, Baseline };
    ...
    protected:
        LayoutParamsPtr CreateDefaultLayoutParams() override { return MakeRef<FlexLayoutParams>(DefaultAllocator()); }

        void OnMeasure(BoxConstraints constraints) override
        {
            const BoxConstraints inner = constraints.Deflate(Padding);
            if (Direction == Orientation::Horizontal) { MeasureHorizontal(inner, constraints); }
            else { MeasureVertical(inner, constraints); }
        }

        void OnLayout(f32 left, f32 top, f32 width, f32 height) override
        {
            (void)left; (void)top;
            if (Direction == Orientation::Horizontal) { LayoutHorizontal(width, height); }
            else { LayoutVertical(width, height); }
        }

    private:
        static SizeSpec ChildWidth(View* child) { return child->LayoutParams ? child->LayoutParams->Width : SizeSpec::Wrap(); }
        static SizeSpec ChildHeight(View* child) { return child->LayoutParams ? child->LayoutParams->Height : SizeSpec::Wrap(); }
        static Thickness ChildMargin(View* child) { return child->LayoutParams ? child->LayoutParams->Margin : Thickness{}; }
        static f32 Grow(View* child) { FlexLayoutParams* flp = Cast<FlexLayoutParams>(child->LayoutParams.Get()); return flp != nullptr ? flp->Grow : 0.0f; }
        ...
        static BoxConstraints MakeChildConstraintsLooseCross(BoxConstraints parent, View* child, bool isHorizontal)
        {
            ...
            if (isHorizontal && heightSpec.kind == SizeSpec::Kind::Match) { heightSpec = SizeSpec::Wrap(); }
            else if (!isHorizontal && widthSpec.kind == SizeSpec::Kind::Match) { widthSpec = SizeSpec::Wrap(); }
        ...
        void MeasureHorizontal(BoxConstraints inner, BoxConstraints outer)
        {
            ...
            for (usize i = 0; i < ChildCount(); ++i)
            {
                ...
                if (grow > 0) { totalGrow += grow; continue; }
                ...
                if (ChildHeight(child).kind == SizeSpec::Kind::Match) { hasMatchCross = true; }
            }
            if (visibleCount > 1) { totalFixed += Spacing * static_cast<f32>(visibleCount - 1); }

            if (totalGrow > 0)
            {
                const bool isMainAxisDefinite = inner.MaxWidth < 100000;
                const f32 remaining = isMainAxisDefinite ? Max(0.0f, inner.MaxWidth - totalFixed) : 0.0f;
                for (usize i = 0; i < ChildCount(); ++i)
                {
                    View* child = GetChildAt(i);
                    if (child->Visibility == Visibility::Gone) { continue; }
                    ...
                    if (isMainAxisDefinite)
                    {
                        ...
                        child->Measure(BoxConstraints{ childMain - margin.TotalHorizontal(), Max(0.0f, childMain - margin.TotalHorizontal()),
                                                       0, Max(0.0f, inner.MaxHeight - margin.TotalVertical()) });
            ...
            if (hasMatchCross && maxCross > 0)
            {
                for (usize i = 0; i < ChildCount(); ++i)
                {
                    ...
                    if (ChildHeight(child).kind == SizeSpec::Kind::Match)
                    {
                        ...
                        child->Measure(BoxConstraints{ child->MeasuredSize.x, child->MeasuredSize.x, crossH, crossH });
                    }
                }
            }
        ...
        void MeasureVertical(BoxConstraints inner, BoxConstraints outer)
        {
            ...
            for (usize i = 0; i < ChildCount(); ++i)
            {
                ...
                if (ChildWidth(child).kind == SizeSpec::Kind::Match) { hasMatchCross = true; }
            }
            if (visibleCount > 1) { totalFixed += Spacing * static_cast<f32>(visibleCount - 1); }
            ...
            if (totalGrow > 0)
            {
                ...
                const f32 remaining = isMainAxisDefinite ? Max(0.0f, inner.MaxHeight - totalFixed) : 0.0f;
                for (usize i = 0; i < ChildCount(); ++i)
                {
                    View* child = GetChildAt(i);
                    if (child->Visibility == Visibility::Gone) { continue; }
                    ...
                    {
                        const f32 childMain = remaining * grow / totalGrow;
                        child->Measure(BoxConstraints{ 0, Max(0.0f, inner.MaxWidth - margin.TotalHorizontal()),
                                                       childMain - margin.TotalVertical(), Max(0.0f, childMain - margin.TotalVertical()) });
                        totalFixed += childMain;
                    }
                    else
                    {
            ...
            if (hasMatchCross && maxCross > 0)
            {
                for (usize i = 0; i < ChildCount(); ++i)
                {
                    ...
                    if (ChildWidth(child).kind == SizeSpec::Kind::Match)
                    {
                        ...
                        child->Measure(BoxConstraints{ crossW, crossW, child->MeasuredSize.y, child->MeasuredSize.y });
                    }
                }
            }
        ...
        void LayoutHorizontal(f32 width, f32 height)
        {
            ...
            if (visibleCount > 1) { totalMain += Spacing * static_cast<f32>(visibleCount - 1); }
            ...
            for (usize i = 0; i < ChildCount(); ++i)
            {
                ...
                const Align align = (flp != nullptr && flp->AlignSelf.HasValue()) ? flp->AlignSelf.Value() : AlignItems;
                ...
                switch (align)
                {
                case Align::Start:   yPos = Padding.Top + margin.Top; break;
                case Align::End:     yPos = Padding.Top + contentH - margin.Bottom - childH; break;
                case Align::Center:  yPos = Padding.Top + margin.Top + (availCross - childH) * 0.5f; break;
                case Align::Stretch: yPos = Padding.Top + margin.Top; finalH = availCross; break;
                case Align::Baseline: yPos = Padding.Top + margin.Top; break;
                }
        ...
        void LayoutVertical(f32 width, f32 height)
        {
            ...
            for (usize i = 0; i < ChildCount(); ++i)
            {
                ...
                switch (align)
                {
                case Align::Start:   xPos = Padding.Left + margin.Left; break;
                case Align::End:     xPos = Padding.Left + contentW - margin.Right - childW; break;
                case Align::Center:  xPos = Padding.Left + margin.Left + (availCross - childW) * 0.5f; break;
                case Align::Stretch: xPos = Padding.Left + margin.Left; finalW = availCross; break;
                case Align::Baseline: xPos = Padding.Left + margin.Left; break;
                }
        ...
        static void ComputeJustify(Justify justify, f32 containerSize, f32 totalChildSize, i32 childCount, f32& startOffset, f32& gap)
        {
            ...
            case Justify::Center: startOffset = freeSpace * 0.5f; break;
            case Justify::SpaceBetween: if (childCount > 1) { gap += freeSpace / static_cast<f32>(childCount - 1); } break;
            case Justify::SpaceAround:
                if (childCount > 0) { const f32 around = freeSpace / static_cast<f32>(childCount); startOffset = around * 0.5f; gap += around; }
                break;
            case Justify::SpaceEvenly:
                if (childCount > 0) { const f32 even = freeSpace / static_cast<f32>(childCount + 1); startOffset = even; gap += even; }
                break;
            }
        }
    };
