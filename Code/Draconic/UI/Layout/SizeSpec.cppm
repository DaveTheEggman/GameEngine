...
export namespace draconic::ui
{
    struct SizeSpec
    {
        enum class Kind { Fixed, Match, Wrap };
        ...
        [[nodiscard]] static constexpr SizeSpec Fixed(Unit size) noexcept { SizeSpec s; s.kind = Kind::Fixed; s.fixedSize = size; return s; }
        /// Fill the parent's available space.
        [[nodiscard]] static constexpr SizeSpec Match() noexcept { SizeSpec s; s.kind = Kind::Match; return s; }
        /// Fit to the view's content (intrinsic size).
        [[nodiscard]] static constexpr SizeSpec Wrap() noexcept { SizeSpec s; s.kind = Kind::Wrap; return s; }

        /// For Fixed: resolves the unit. For Match/Wrap: 0 (parent handles these).
        [[nodiscard]] constexpr f32 ResolveFixed(f32 dpiScale) const noexcept { return kind == Kind::Fixed ? fixedSize.Resolve(dpiScale) : 0.0f; }
