    ...
    private:
                                    ...
                                    StringView parentTagName, LayoutParams* lp, UIContext* context)
        {
            ...
            for (xml::XmlAttribute* attr : element->Attributes())
            {
                ...
                if (name == u8"id" || name == u8"name") { view->Name = String(value); continue; }

                if (name == u8"class")
                {
                    ForEachSplit(value, u8' ', [view](StringView cls) { const StringView t = Trimmed(cls); if (t.Size() > 0) { view->AddClass(t); } });
                    continue;
                }

                if (name == u8"style") { SSSParser::ApplyInlineStyle(view, value); continue; }

                // === Common View properties ===

                if (name == u8"visibility")
                {
                    if (value == u8"visible") { view->Visibility = Visibility::Visible; }
                    else if (value == u8"hidden") { view->Visibility = Visibility::Hidden; }
                    else if (value == u8"gone") { view->Visibility = Visibility::Gone; }
                    continue;
                }

                if (name == u8"is-enabled") { view->IsEnabled = (value == u8"true"); continue; }

                if (name == u8"opacity") { if (Optional<f64> f = ParseFloat(value); f.HasValue()) { view->Opacity = static_cast<f32>(f.Value()); } continue; }

                if (name == u8"cursor")
                {
                    if (value == u8"hand") { view->Cursor = CursorType::Hand; }
                    else if (value == u8"ibeam") { view->Cursor = CursorType::IBeam; }
                    else if (value == u8"crosshair") { view->Cursor = CursorType::Crosshair; }
                    else if (value == u8"arrow") { view->Cursor = CursorType::Arrow; }
                    else if (value == u8"move") { view->Cursor = CursorType::Move; }
                    continue;
                }

                if (name == u8"tooltip") { view->TooltipText = String(value); continue; }
                if (name == u8"is-focusable") { view->IsFocusable = (value == u8"true"); continue; }
                if (name == u8"is-tab-stop") { view->IsTabStop = (value == u8"true"); continue; }
                if (name == u8"tab-index") { if (Optional<i64> idx = ParseInt(value); idx.HasValue()) { view->TabIndex = static_cast<i32>(idx.Value()); } continue; }

                if (name == u8"padding")
                {
                    if (ViewGroup* vg = Cast<ViewGroup>(view)) { vg->Padding = MarkupRegistry::ParseThickness(value); }
                    continue;
                }

                if (name == u8"clips-content") { view->ClipsContent = (value == u8"true"); continue; }

                // === Layout params (width/height/margin + container-specific) ===

                if (name == u8"width" && lp != nullptr) { lp->Width = MarkupRegistry::ParseSizeSpec(value); continue; }
                if (name == u8"height" && lp != nullptr) { lp->Height = MarkupRegistry::ParseSizeSpec(value); continue; }
                if (name == u8"margin" && lp != nullptr) { lp->Margin = MarkupRegistry::ParseThickness(value); continue; }

                if (lp != nullptr && parentTagName.Size() > 0)
                {
                    if (MarkupRegistry::SetLayoutParam(parentTagName, lp, name, value)) { continue; }
                }
        ...
        static void GetTextContent(xml::XmlElement* element, String& outText)
        {
            for (xml::XmlNode* child : element->Children())
            {
                if (child->NodeType() == xml::XmlNodeType::Text)
                {
                    ...
                    if (t.Size() > 0)
                    {
                        if (outText.Size() > 0) { outText.Append(u8" "); }
                        outText.Append(t);
                    }
                }
            }
        ...
        static void ForEachSplit(StringView s, char8_t delim, F&& fn)
        {
            ...
            for (usize i = 0; i <= n; ++i)
            {
                if (i == n || data[i] == delim)
                {
                    if (i > start) { fn(StringView{ data + start, i - start }); }
                    start = i + 1;
                }
            }
        }
