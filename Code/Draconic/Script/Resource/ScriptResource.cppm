...
export namespace draconic::script
{
    ...
    {
        outAssetType = String{};
        if (text == u8"float") { outKind = ScriptPropertyType::Float; return true; }
        if (text == u8"int") { outKind = ScriptPropertyType::Int; return true; }
        if (text == u8"bool") { outKind = ScriptPropertyType::Bool; return true; }
        if (text == u8"string") { outKind = ScriptPropertyType::String; return true; }
        if (text == u8"color") { outKind = ScriptPropertyType::Color; return true; }
        if (text == u8"vec3") { outKind = ScriptPropertyType::Vec3; return true; }
        if (text == u8"entity") { outKind = ScriptPropertyType::Entity; return true; }
        const StringView assetPrefix = u8"asset:";
        if (text.Size() > assetPrefix.Size()
            && text.SubStr(0, assetPrefix.Size()) == assetPrefix)
        {
            outKind = ScriptPropertyType::Asset;
            outAssetType = String(text.SubStr(assetPrefix.Size(),
                                              text.Size() - assetPrefix.Size()));
            return true;
        }
        return false;
    }
                                                        ...
                                                        const ScriptPropertyValue& b) noexcept
    {
        ...
        {
            case ScriptPropertyType::Float:
            case ScriptPropertyType::Int: return a.number == b.number;
            case ScriptPropertyType::Bool: return a.boolean == b.boolean;
            case ScriptPropertyType::String: return a.text == b.text;
            case ScriptPropertyType::Color:
            ...
            case ScriptPropertyType::Entity:
            case ScriptPropertyType::Asset: return a.guid == b.guid;
            case ScriptPropertyType::None:
            default: return true;
        }
    }
    ...
    public:
        ...
        [[nodiscard]] bool HasHandler(StringView name) const
        {
            ...
            {
                if (handler.AsView() == name) { return true; }
            }
            return false;
        }
        ...
        [[nodiscard]] const ScriptPropertyDesc* FindProperty(u64 nameHash) const
        {
            ...
            {
                if (property.hash == nameHash) { return &property; }
            }
            return nullptr;
        }
