...
public:
    ...
    [[nodiscard]] i32 GetChannelCount() const { return static_cast<i32>(draconic::image::ChannelCount(m_format)); }
    ...
    static Image CreateWaveNormalMap(u32 width = 256, u32 height = 256,
        ...
        for (u32 y = 0; y < height; ++y) {
            for (u32 x = 0; x < width; ++x) {
                ...
                const f32 heightValue = Sin(fx * kPi * waveFrequencyX) * amplitude + Sin(fy * kPi * waveFrequencyY) * amplitude * 0.7f;
                const f32 heightRight = Sin((fx + 1.0f / static_cast<f32>(width)) * kPi * waveFrequencyX) * amplitude + Sin(fy * kPi * waveFrequencyY) * amplitude * 0.7f;
                ...
                image.SetPixel(x, y, EncodeNormal(Normalized(Float3{ -dx * 20.0f, -dy * 20.0f, 1.0f })));
            }
        }
        return image;
    }
    ...
    static Image CreateNoiseNormalMap(u32 width = 256, u32 height = 256, f32 scale = 0.1f,
        ...
        for (u32 y = 0; y < height; ++y) {
            for (u32 x = 0; x < width; ++x) {
                ...
                image.SetPixel(x, y, EncodeNormal(Normalized(Float3{ -dx * 8.0f, -dy * 8.0f, 1.0f })));
            }
        }
        return image;
    }
    ...
    [[nodiscard]] static Float3 CalculateNormalFromHeight(f32 heightL, f32 heightR, f32 heightU, f32 heightD, f32 scale = 1.0f) {
