...
export namespace draconic::shell
{
    ...
    {
    public:
        SDL3Gamepad(SDL_Gamepad* pad, SDL_JoystickID id, core::i32 index,
    ...
    public:
        ...
        void AddGamepad(SDL_JoystickID id)
        {
            ...
            m_gamepads.PushBack(core::MakeUnique<SDL3Gamepad>(
                core::DefaultAllocator(), pad, id, index, static_cast<core::String&&>(name)));
        }
    ...
    public:
        ...
        {
            Context* ctx =
                MakeContext(static_cast<DialogResultCallback&&>(callback), {}, defaultPath);
    ...
    private:
        ...
        struct Context
        {
            ...
            core::Array<core::String>
                filterStrings; // name,pattern,name,pattern,... keeps Data() alive
        ...
        static void SDLCALL Trampoline(void* userdata, const char* const* filelist, int /*filter*/)
        {
            ...
            if (filelist != nullptr) // null => cancelled or error
            {
                ...
                {
                    paths.PushBack(core::String(
                        core::StringView(reinterpret_cast<const core::utf8char*>(*p))));
                }
            }
            if (ctx->callback)
            {
            ...
            while (SDL_PollEvent(&event))
            {
                ...
                case SDL_EVENT_DROP_FILE:
                {
                    if (event.drop.data != nullptr)
                    {
                        ...
                        drop.x = event.drop.x;
                        drop.y = event.drop.y;
                        drop.path = core::String(core::StringView(
                            reinterpret_cast<const core::utf8char*>(event.drop.data)));
                ...
                case SDL_EVENT_QUIT:
                    ...
                    if (IWindow* main = m_windows.MainWindow())
                    {
                        main->Close();
                        m_windows.PushEvent(
                            WindowEvent{WindowEventType::CloseRequested, main->Id()});
                    }
                    m_running = false;
                    break;
                ...
                case SDL_EVENT_MOUSE_MOTION:
                {
                    ...
                    m_input.EmitEvent(e);
                    m_input.MouseDevice().OnMotion(event.motion.x, event.motion.y,
    ...
    private:
        static KeyCode MapKeyCode(SDL_Scancode sc) noexcept
        {
            if (sc >= SDL_SCANCODE_A && sc <= SDL_SCANCODE_Z)
                return static_cast<KeyCode>(static_cast<core::u32>(KeyCode::A) +
                                            (sc - SDL_SCANCODE_A));
            ...
            if (sc >= SDL_SCANCODE_F13 && sc <= SDL_SCANCODE_F24)
                return static_cast<KeyCode>(static_cast<core::u32>(KeyCode::F13) +
                                            (sc - SDL_SCANCODE_F13));
            if (sc >= SDL_SCANCODE_KP_1 &&
                sc <= SDL_SCANCODE_KP_9) // SDL keypad digits: 1..9 then 0
