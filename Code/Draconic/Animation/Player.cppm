...
public:
    ...
    void Update(f32 deltaTime) {
        ...
        if (m_eventHandler && !m_currentClip->Events().IsEmpty()) {
            m_currentClip->FireEvents(prevTime, m_currentTime,
                                      [this](StringView name, f32 time) { m_eventHandler(name, time); });
        }
    ...
    [[nodiscard]] Span<const Float4x4> GetPrevSkinningMatrices() const {
        return Span<const Float4x4>{ m_prevSkinningMatrices.Data(), m_prevSkinningMatrices.Size() };
    }
    ...
    [[nodiscard]] Span<BoneTransform> GetLocalPoses() noexcept { return { m_localPoses.Data(), m_localPoses.Size() }; }
    ...
    void BlendAnimation(AnimationClip* clip, f32 time, f32 weight) {
        ...
        Array<BoneTransform> blend; blend.Resize(static_cast<usize>(m_skeleton->BoneCount()));
