...
TEST_CASE("graph: layer states/transitions/mask")
{
    ...
    CHECK(layer.AddState(MakeUnique<AnimationGraphState>(DefaultAllocator(), StringView{ u8"Idle" }, kNullNode)) == 0);
    CHECK(layer.AddState(MakeUnique<AnimationGraphState>(DefaultAllocator(), StringView{ u8"Walk" }, kNullNode)) == 1);
...
TEST_CASE("graph: BlendTree2D entries + positions + duration")
{
    ...
    tree.AddEntry(Vec2{ 1, 0 }, nullptr);
...
TEST_CASE("graph: parameters + layers + find (case sensitive)")
{
    ...
    CHECK(graph.AddLayer(MakeUnique<AnimationLayer>(DefaultAllocator(), StringView{ u8"Base" })) == 0);
    CHECK(graph.AddLayer(MakeUnique<AnimationLayer>(DefaultAllocator(), StringView{ u8"Upper" })) == 1);
...
TEST_CASE("graph: player transitions states on a bool parameter")
{
    ...
    idleClip.GetOrCreatePositionTrack(0)->AddKeyframe(0.0f, Vec3{ 0, 0, 0 });
    idleClip.GetOrCreatePositionTrack(0)->AddKeyframe(1.0f, Vec3{ 0, 0, 0 });
    AnimationClip walkClip{ u8"walk", 1.0f, true };
    walkClip.GetOrCreatePositionTrack(0)->AddKeyframe(0.0f, Vec3{ 0, 0, 0 });
    walkClip.GetOrCreatePositionTrack(0)->AddKeyframe(1.0f, Vec3{ 0, 0, 0 });
    ...
    layer->AddState(MakeUnique<AnimationGraphState>(DefaultAllocator(), StringView{ u8"Idle" },
    ...
    Span<const Mat4> skin = player.GetSkinningMatrices();
    CHECK(skin.Size() == 1);
}
