...
struct Bone {
    ...
    Float4x4          rootCorrection  = Float4x4::Identity();// missing-ancestor transform for roots (e.g. FBX axis conv)
    Array<i32>    children;
};
...
public:
    ...
    [[nodiscard]] i32 FindBone(StringView name) const {
        if (const i32* idx = m_nameMap.Find(String{ name })) { return *idx; }
        return -1;
    }
    ...
    void BuildNameMap() {
        m_nameMap.Clear();
        for (const Bone& b : m_bones) { if (!b.name.IsEmpty()) { m_nameMap.InsertOrAssign(b.name, b.index); } }
    }

    // Cache root-bone indices (parentIndex < 0). Call after parents are set.
    void FindRootBones() {
        m_rootBones.Clear();
        for (const Bone& b : m_bones) { if (b.parentIndex < 0) { m_rootBones.PushBack(b.index); } }
    }
    ...
    void BuildChildIndices() {
        ...
        Array<i32> childCounts; childCounts.Resize(n);
        ...
        for (const Bone& b : m_bones) {
            if (b.parentIndex >= 0 && InBounds(b.parentIndex)) { m_bones[static_cast<usize>(b.parentIndex)].children.PushBack(b.index); }
        }
    ...
    void ComputeInverseBindPoses() {
        ...
        ComputeWorldPoses(Span<const BoneTransform>{}, Span<Float4x4>{ m_worldScratch.Data(), n });   // bind pose
    ...
    void ComputeSkinningMatrices(Span<const BoneTransform> localPoses, Span<Float4x4> outSkinningMatrices) {
        const usize n = m_bones.Size();
        m_worldScratch.Resize(n);
...
private:
    ...
    void ComputeBoneWorldPose(i32 boneIndex, Span<const BoneTransform> localPoses, Span<Float4x4> outWorldPoses) {
    ...
    void BuildHierarchicalOrder() {
        ...
        Array<i32> queue;
        for (i32 root : m_rootBones) { queue.PushBack(root); }
        usize head = 0;
        while (head < queue.Size()) {
            const i32 boneIndex = queue[head++];
            m_hierarchicalOrder.PushBack(boneIndex);
            if (InBounds(boneIndex)) {
                for (i32 child : m_bones[static_cast<usize>(boneIndex)].children) { queue.PushBack(child); }
            }
        }
        ...
        if (m_hierarchicalOrder.Size() < n) {
            for (i32 i = 0; i < static_cast<i32>(n); ++i) {
                ...
                if (!found) { m_hierarchicalOrder.PushBack(i); }
            }
        }
    }
