...
private:
    foundation::core::Status doBatchUpload();
    ...
    static constexpr foundation::core::u32 kTexSize = 128;
    ...
    foundation::core::u64 m_frameFenceVal = 0;

    // Upload tracking
    rhi::Fence* m_uploadFence = nullptr;
    foundation::core::u64 m_uploadFenceVal = 0;
    bool m_uploadComplete = false;
    float m_uploadStartTime = 0.0f;
};

foundation::core::Status BatchUploadSample::OnInit()
{
    ...
    if (m_device->CreateBuffer(tbd, m_transformBuf) != foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;
    ...
    if (m_device->CreateCommandPool(rhi::QueueType::Graphics, m_pool) !=
        foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;
    if (m_device->CreateFence(0, m_frameFence) != foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;

    // Upload fence
    if (m_device->CreateFence(0, m_uploadFence) != foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;

    // === Batch upload: VB + IB + texture in one submission ===
    if (doBatchUpload() != foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;

    return foundation::core::ErrorCode::Ok;
}

foundation::core::Status BatchUploadSample::doBatchUpload()
{
    using foundation::core::Status, foundation::core::Span, foundation::core::u8, foundation::core::u32;
    ...
    // Index data
    foundation::core::u16 indices[6] = {0, 1, 2, 0, 2, 3};
    ...
    m_uploadFenceVal = 1;
    if (transfer->SubmitAsync(m_uploadFence, m_uploadFenceVal) != foundation::core::ErrorCode::Ok)
    {
