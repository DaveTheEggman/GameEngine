        ...
        else
        {
            ...
            if (wgpuAdapters.Size() > 0)
            {
                ...
                if (wgpuAdapters[0]->CreateDevice(DeviceDesc{}, wgpuDevice) == ErrorCode::Ok)
                {
                    ...
                    // The host frame sequence, three real frames on the shell window
                    // (the VK section released its surface, the window is free):
                    // acquire -> encode clear pass -> submit(fence) -> present.
                    SurfacePlatform wgpuPlat = SurfacePlatform::Unknown;
                    switch (nw.system)
                    {
                    case shell::WindowSystem::Win32:
                        wgpuPlat = SurfacePlatform::Win32;
                        break;
                    case shell::WindowSystem::X11:
                        wgpuPlat = SurfacePlatform::X11;
                        break;
                    case shell::WindowSystem::Wayland:
                        wgpuPlat = SurfacePlatform::Wayland;
                        break;
                    default:
                        break;
                    }
                    Surface* wgpuSurface = nullptr;
                    if (wgpuBackend->CreateSurface(native, display, wgpuSurface, wgpuPlat) ==
                        ErrorCode::Ok)
                    {
                        SwapChainDesc scd{};
                        scd.width = 1280;
                        scd.height = 720;
                        scd.format = TextureFormat::BGRA8UnormSrgb;
                        SwapChain* wgpuSwap = nullptr;
                        if (wgpuDevice->CreateSwapChain(wgpuSurface, scd, wgpuSwap) ==
                            ErrorCode::Ok)
                        {
                            CommandPool* framePool = nullptr;
                            wgpuDevice->CreateCommandPool(QueueType::Graphics, framePool);
                            Fence* frameFence = nullptr;
                            wgpuDevice->CreateFence(0, frameFence);
                            u32 presented = 0;
                            for (u32 frame = 0; frame < 3; ++frame)
                            {
                                if (wgpuSwap->AcquireNextImage() != ErrorCode::Ok)
                                {
                                    break;
                                }
                                CommandEncoder* frameEncoder = nullptr;
                                framePool->CreateEncoder(frameEncoder);
                                RenderPassDesc pass;
                                ColorAttachment att;
                                att.view = wgpuSwap->CurrentTextureView();
                                att.clearValue = ClearColor{0.1f, 0.2f, 0.4f, 1.0f};
                                pass.colorAttachments.Add(att);
                                frameEncoder->BeginRenderPass(pass)->End();
                                CommandBuffer* frameCb = frameEncoder->Finish();
                                CommandBuffer* frameCbs[1] = {frameCb};
                                wgpuDevice->GetQueue(QueueType::Graphics)
                                    ->Submit(Span<CommandBuffer* const>(frameCbs, 1),
                                             frameFence, frame + 1);
                                frameFence->Wait(frame + 1, ~0ull);
                                if (wgpuSwap->Present(
                                        wgpuDevice->GetQueue(QueueType::Graphics)) ==
                                    ErrorCode::Ok)
                                {
                                ...
                                framePool->DestroyEncoder(frameEncoder);
                            }
                            std::printf("WebGPU presented frames: %u/3\n", presented);
                            wgpuDevice->DestroyFence(frameFence);
                            wgpuDevice->DestroyCommandPool(framePool);
                            wgpuDevice->DestroySwapChain(wgpuSwap);
                        }
                        else
                        {
                            std::printf("WebGPU swap chain: FAILED\n");
                        }
                        wgpuDevice->DestroySurface(wgpuSurface);
                    }
                    else
                    {
                        std::printf("WebGPU surface: FAILED (window in use?)\n");
                    }
