...
private:
    ...
    static constexpr draconic::core::u16 kCubeIdx[] = {
...
draconic::core::Status UniformBufferSample::OnInit() {
    ...
    if (sf::CompileToModule(m_compiler, m_device, kShaderSource, ds::ShaderStage::Vertex,   u8"VSMain", u8"CubeVS", m_vs) != draconic::core::ErrorCode::Ok) return draconic::core::ErrorCode::Unknown;
    if (sf::CompileToModule(m_compiler, m_device, kShaderSource, ds::ShaderStage::Fragment, u8"PSMain", u8"CubePS", m_ps) != draconic::core::ErrorCode::Ok) return draconic::core::ErrorCode::Unknown;
    ...
    batch->WriteBuffer(m_vb, 0, Span<const u8>(reinterpret_cast<const u8*>(kCubeVerts), sizeof(kCubeVerts)));
    batch->WriteBuffer(m_ib, 0, Span<const u8>(reinterpret_cast<const u8*>(kCubeIdx), sizeof(kCubeIdx)));
...
void UniformBufferSample::OnRender() {
    ...
    draconic::core::f32 pulse = (std::sin(m_totalTime * 2.0f) * 0.3f + 0.7f);
    draconic::core::f32 tint[4] = { pulse, pulse, pulse, 1.0f };
