...
draconic::core::Status TextureSample::OnInit() {
    ...
    if (samples::framework::CompileToModule(m_compiler, m_device, kShaderSource, shaders::ShaderStage::Vertex,   u8"VSMain", u8"QuadVS", m_vs) != draconic::core::ErrorCode::Ok) return draconic::core::ErrorCode::Unknown;
    if (samples::framework::CompileToModule(m_compiler, m_device, kShaderSource, shaders::ShaderStage::Fragment, u8"PSMain", u8"QuadPS", m_ps) != draconic::core::ErrorCode::Ok) return draconic::core::ErrorCode::Unknown;
    ...
    batch->WriteTexture(m_tex, Span<const u8>(texPixels, sizeof(texPixels)), layout, rhi::Extent3D{tw, th, 1});
    batch->Submit();
    ...
    rhi::BindGroupLayoutDesc bgld{}; bgld.entries = Span<const rhi::BindGroupLayoutEntry>(bglEntries, 2);
    ...
    rhi::BindGroupDesc bgd{}; bgd.layout = m_bgl; bgd.entries = Span<const rhi::BindGroupEntry>(bgEntries, 2);
    ...
    rhi::PipelineLayoutDesc pld{}; pld.bindGroupLayouts = Span<rhi::BindGroupLayout* const>(sets, 1);
...
void TextureSample::OnRender() {
    ...
    m_graphicsQueue->Submit(draconic::core::Span<rhi::CommandBuffer* const>(cbs, 1), m_fence, m_fenceVal);
