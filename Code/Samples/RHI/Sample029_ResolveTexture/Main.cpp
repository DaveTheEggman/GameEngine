...
draconic::core::Status ResolveTextureSample::OnInit() {
    ...
    if (samples::framework::CompileToModule(m_compiler, m_device, kShader, shaders::ShaderStage::Vertex,   u8"VSMain", u8"ResolveVS", m_vs) != draconic::core::ErrorCode::Ok) return draconic::core::ErrorCode::Unknown;
    if (samples::framework::CompileToModule(m_compiler, m_device, kShader, shaders::ShaderStage::Fragment, u8"PSMain", u8"ResolvePS", m_ps) != draconic::core::ErrorCode::Ok) return draconic::core::ErrorCode::Unknown;

    // Vertex and index buffers.
    rhi::BufferDesc vbd{}; vbd.size = sizeof(kVerts); vbd.usage = rhi::BufferUsage::Vertex | rhi::BufferUsage::CopyDst; vbd.memory = rhi::MemoryLocation::GpuOnly; vbd.label = u8"ResolveVB";
    if (m_device->CreateBuffer(vbd, m_vb) != draconic::core::ErrorCode::Ok) return draconic::core::ErrorCode::Unknown;
    rhi::BufferDesc ibd{}; ibd.size = sizeof(kIdx); ibd.usage = rhi::BufferUsage::Index | rhi::BufferUsage::CopyDst; ibd.memory = rhi::MemoryLocation::GpuOnly; ibd.label = u8"ResolveIB";
    ...
    rhi::PipelineLayoutDesc pld{}; pld.label = u8"ResolvePL";
    ...
    rhi::RenderPipelineDesc rpd{}; rpd.layout = m_pl; rpd.label = u8"ResolvePipeline";
...
void ResolveTextureSample::OnRender() {
    ...
    enc->TransitionTexture(m_msaaTex, rhi::ResourceState::CopySrc, rhi::ResourceState::RenderTarget);
    ...
    enc->TransitionTexture(m_msaaTex, rhi::ResourceState::RenderTarget, rhi::ResourceState::CopySrc);
    // Swapchain: Present -> CopyDst
    enc->TransitionTexture(m_swapChain->CurrentTexture(), rhi::ResourceState::Present, rhi::ResourceState::CopyDst);
