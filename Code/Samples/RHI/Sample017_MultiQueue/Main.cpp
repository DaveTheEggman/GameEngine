...
private:
    ...
    foundation::core::u64 m_compFenceVal = 0, m_gfxFenceVal = 0;
    bool m_hasDedicatedCompute = false;
    float m_lastReportTime = 0.0f;
};

void MultiQueueSample::recreateDepth(foundation::core::u32 w, foundation::core::u32 h)
{
    m_depthBuf.Recreate(m_device, w, h);
}

foundation::core::Status MultiQueueSample::OnInit()
{
    ...
    if (m_device->CreateCommandPool(compPoolType, m_computePool) != foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;

    if (m_device->CreateFence(0, m_compFence) != foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;
    if (m_device->CreateFence(0, m_gfxFence) != foundation::core::ErrorCode::Ok)
        return foundation::core::ErrorCode::Unknown;
...
void MultiQueueSample::OnRender()
{
    ...
    if (m_computePool->CreateEncoder(cEnc) != foundation::core::ErrorCode::Ok || !cEnc)
        return;
    ...
    if (m_gfxPool->CreateEncoder(gEnc) != foundation::core::ErrorCode::Ok || !gEnc)
        return;
    ...
    foundation::core::u64 waitValues[1] = {m_compFenceVal};
    m_graphicsQueue->Submit(
        Span<rhi::CommandBuffer* const>(gCbs, 1), Span<rhi::Fence* const>(waitFences, 1),
        Span<const foundation::core::u64>(waitValues, 1), m_gfxFence, m_gfxFenceVal);
