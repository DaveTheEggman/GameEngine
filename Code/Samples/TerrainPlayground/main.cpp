// TerrainPlayground - the terrain showcase. Builds a heightfield IN CODE, wraps it in an in-memory
// TerrainResource (no cooking - Ref<> sub-resources take a direct product), attaches a TerrainComponent,
// and lets engine.terrain draw the chunked geo-mipmap terrain through its dedicated TerrainRenderer.
// An ImGui HUD tweaks what the renderer exposes live: the sun direction (a real directional light the
// terrain shades against), the per-instance LOD bias (watch the chunks coarsen/refine), and the
// heightfield itself (type / amplitude / frequency -> a regen that BumpVersion()s so the GPU height
// texture + chunk model re-upload). WASD/RMB fly camera. See Documentation/Plans/terrain.md.
...
namespace
{
    ...
    void GenerateHeightfield(hf::Heightfield& grid, TerrainType type, core::f32 amplitude,
                             core::f32 frequency)
    {
    ...
    public:
        ...
        void OnStartup(runtime::IApplicationHost& host) override
        {
            ...
            m_heightfield =
                MakeRef<hf::Heightfield>(DefaultAllocator(), kGridSize,
                                         core::Float2{kWorldSize, kWorldSize}, 0.0f, kMaxHeight);
            GenerateHeightfield(*m_heightfield, m_type, m_amplitude, m_frequency);

            m_terrainResource = MakeRef<terrain::TerrainResource>(DefaultAllocator());
