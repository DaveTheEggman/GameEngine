...
namespace
{
    ...
    public:
        ...
        void OnStartup(runtime::IApplicationHost& host) override
        {
            ...
            core::ConsoleWrite(u8"AnimatedCrowd: [Space] add batch  [Backspace] remove batch  [P] profiler  [Esc] exit\n");
        }
        ...
        bool CookModel(core::StringView prefix, core::StringView path)
        {
            ...
            if (!m_model) { core::ConsoleWrite(u8"AnimatedCrowd: model bind failed\n"); return false; }
            ...
            for (core::usize i = 0; i < m_model->meshes.Size(); ++i) {
                ...
                if (mesh == nullptr || !mesh->IsSkinned()) { continue; }
                const core::i32 matIdx = (i < m_model->meshMaterial.Size()) ? m_model->meshMaterial[i] : -1;
        ...
        void RebuildToCount(core::u32 count)
        {
            ...
            if (imm == nullptr || anims == nullptr || m_skinnedParts.IsEmpty()) { return; }
        ...
        void OnUpdate(runtime::IApplicationHost& host, core::f32 deltaTime) override
        {
            ...
            if (auto* input = host.Shell() != nullptr ? host.Shell()->Input() : nullptr) {
                if (shell::IKeyboard* kb = input->Keyboard()) {
                    ...
                    if (kb->IsKeyPressed(shell::KeyCode::K))         { ToggleShadows(); }
            ...
            if (kAutoProfile) {
                ...
                if (m_profileElapsed >= 5.0f) {
                    ...
                    core::ConsoleWrite(core::Format(u8"=== AnimatedCrowd PROFILE: chars={}  fps={}  frame={} ms ===\n",
            ...
            if (kAutoRamp) {
                ...
                if (m_rampSettle >= 1.3f) {
                    ...
                    const core::u32 n = static_cast<core::u32>(m_crowdCount);
                    if (fps <= 50.0f) {
                        core::ConsoleWrite(core::Format(u8"AnimatedCrowd: THRESHOLD chars={}  fps={}  frame={} ms\n",
                ...
                if (m_scene != nullptr) {
                    if (auto* lights = m_scene->GetSystem<render::LightComponentManager>()) {
                        if (render::LightComponent* kl = m_keyLight.IsAssigned() ? lights->Get(m_keyLight) : nullptr) {
                            ...
                            if (ImGui::Checkbox("Directional shadows (K)", &shadowsOn)) { kl->castsShadows = shadowsOn; }
                        }
                    }
                }
                if (!shadowsOn) { ImGui::TextDisabled("(shadows off - isolates skinning throughput)"); }
