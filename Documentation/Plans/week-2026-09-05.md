...
- **P2 features**: cross-zone stitching/portals, dynamic obstacles
  ...
  per-tile regeneration primitive (Lumix generateTileAt parity - the
  structural gap; tiled/large-terrain bake rides it, and partial rebake for
  obstacles/terrain edits falls out of it), MCP `nav_bake` tool (rides the
  MCP resume list). Terrain-as-bake-source is DONE (0c05ad89) - do not
  re-plan it.
- **Crowd per-agent speed / stop distance (Lumix parity, user 2026-09-02)**:
  per-call control when steering an agent (maxSpeed + stopDistance per move
  request / per agent), surfaced through the NavAgent component + facade.
  Small, script-facing.
- **Agent-state introspection (Lumix parity, user 2026-09-02)**: expose the
  live dtCrowd agent internals (state, target, corridor/path progress,
  velocity) - a facade query + an inspector read-only block on NavAgent.
  Pairs with the speed work for debugging steering.
