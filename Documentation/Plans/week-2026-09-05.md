...
- **P2 features**: cross-zone stitching/portals, dynamic obstacles
  (DetourTileCache + partial tile rebake), NavBlocker volumes, TILED bake +
  per-tile regen DONE 2026-09-02 (e2ec496f: BuildTiled v2 blobs + the
  BuildTileAt primitive, editor bake tiled, v1 loads; REMAINING = wiring
  partial rebake to a consumer when obstacles/terrain edits land), MCP `nav_bake` tool (rides the
  MCP resume list). Terrain-as-bake-source is DONE (0c05ad89) - do not
  re-plan it.
- **Crowd per-agent speed / stop distance - DONE 2026-09-02 (c04702ae)**:
  SetAgentParams updates the live crowd agent; tick value-compares
  maxSpeed/maxAcceleration; stopDistance = arrival radius w/ target release
  on arrival; navigateAt(x,y,z,speed,stopDistance); envelope v2.
- **Agent-state introspection - DONE 2026-09-02 (c04702ae)**: AgentState
  remaps Detour internals to a stable contract; script
  state/targetState/desiredSpeed/corners; debugDrawPaths floats a
  'walking [valid] 3.5u/s' label per agent (failed = red).
- **Bake stage visualization - DONE 2026-09-02 (10b475a4)**: opt-in
  NavigationBakeStages capture (span tops + contours) on BuildTiled;
  editor bake parks it on the scene system; debugDrawBakeStages setting
  (v2) overlays it in the zone's rigid frame. Capture is observer-pure
  (blob byte-identical, pinned). USER VISUAL VERIFY owed on all four nav
  parity items (agent labels, stage overlay, tiled zones in a real
  session).
...
re-verify the rest), I5 allocator plumbing (architectural track, own pace),
