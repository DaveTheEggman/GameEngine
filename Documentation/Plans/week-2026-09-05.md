...
- **P4 tail**: picker-DIALOG thumbnails (AssetPickerDialog has no thumbnail
  wiring - the grid + picker-slot pattern applies), a settings knob for
  thumbnail size, and a cache-maintenance action (clear/regenerate all; the
  cache dir is <project>/.cache/thumbs, safe to delete).
