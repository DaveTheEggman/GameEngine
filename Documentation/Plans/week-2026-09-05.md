...
P4 UI cluster DONE (the MakeRef pending-control handoff makes
MemoryAllocator() valid inside constructors; ~330 first-party UI sites
switched to the inheritance idiom; UIHost/UIContext root the tree; static
factories + the markup factory ABI take the caller's allocator), P3g mop-up
DONE (shell/physics/shaders/primitives/scene-prefabs); remaining = P5 engine
subsystems, P6 pipeline/editor/samples, P7 lockdown tripwire),
