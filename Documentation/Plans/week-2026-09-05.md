...
- **P2 features**: cross-zone stitching/portals, dynamic obstacles
  (DetourTileCache + partial tile rebake), NavBlocker volumes, tiled/large-
  terrain bake (the nav-terrain follow-on), MCP `nav_bake` tool (rides the MCP
  resume list). Terrain-as-bake-source is DONE (0c05ad89) - do not re-plan it.
