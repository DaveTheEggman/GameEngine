...
PROGRESS 2026-09-02 (I5 P0): `:memory_tag` now tracks peak bytes + total
allocations per tag and `MemoryTagReport()` emits the rollup rows - the data
source above exists; the panel/graph work remains.
...
- *Web (task #112)*: (A) WebSocketTransport + native WS accept path BUILT
  2026-09-01 (a5c14e69 native: RFC 6455 handshake via foundation.http,
  frame codec w/ malformed-input doctests, gateway + WebSocketHybridSocket
  so session/reliability run unchanged; c8bef210 browser: emscripten
  client socket w/ pre-open send buffering, JoinServer on web; 9038b969
  convention: fixed-port hosts open ws on port+1, web join redirects - one
  script nets everywhere). OWED: the user browser smoke (native host, the
  net demo recipe; browser WebScene joins, replicated state moves) - node
  cannot proxy WS so the browser proof is manual by design. Then (B)
  Firefox compat, (C) audio autoplay resume-on-gesture + AS browser smoke.
...
re-verify the rest), I5 allocator plumbing (WELL UNDERWAY 2026-09-02, 8
commits: P0 enablers, P1 nav exemplar, P2 Core consumers incl. the Context
ctor-default removal + JobSystem/host threading, P3a data backbone (XML DOM,
Settings, Http, ContentDatabase/ResourceManager/NativeFileSystem), P3b fonts,
P3c audio, P3d net, P3e script backends (the MemoryAllocator() inheritance
idiom at scale), P3f resource factories; remaining per the spec = P3 misc
mop-up (Scene.Resource internals, Physics, Shell.Desktop, Shaders.System),
TRACK COMPLETE 2026-09-02, 13 commits P0-P7: enablers, nav
exemplar, Core consumers, data/fonts/audio/net/script/factory services, the
UI cluster (MakeRef pending-control handoff = MemoryAllocator() valid in
ctors), engine subsystems (Scene ctor default removed; Render/GameUI tags),
editor+samples (~1,950 sites), and the P7 lockdown: Core.Tests scans every
run against the pinned allocator-allowlist.txt (113 root/bound files after the pipeline slice and
the editor-root pass; tagged Editor root + one AppRoot per binary) -
new ambient references fail CI, stale entries fail too. MemoryTagReport()
now feeds the I4 memory report with Navigation/Render/GameUI tags),
...
sim, post 4/5 (pending the 1-3 verify), AS debugger P2/P3, draconic.gui
...
- **P4 tail**: picker-DIALOG thumbnails DONE 2026-09-01 (b808126b - rows
  query the service first, icon until a tile exists; passive re-query like
  the inspector slots, so a freshly generated tile lands on the next
  rebind - live row refresh on OnThumbnailReady deferred until the single-
  handler Function grows multicast/tokens, the UI-core P4 item). Remaining:
  a settings knob for thumbnail size, and a cache-maintenance action
...
- **User visual verify owed**: the prefab/scene/particle tiles and the newer
  types (font/audio/heightfield/splatmap/collision/skeleton/model) - the mesh
  grid passed 2026-08-30; the rest have not had a look.
