...
- **Bake stage visualization (Lumix parity, user 2026-09-02)**: render the
  intermediate Recast stages (heightfield/contours) as debug draw during/
  after a bake. COMPLEMENT to the live-dtNavMesh draw (which stays - it
  catches drift a bake-time outline masks). Diagnostic polish - after the
  tiled build.
