...
re-verify the rest), I5 allocator plumbing (STARTED 2026-09-02: spec at
Specs/allocator-plumbing.md; P0 enablers + P1 navigation exemplar landed -
`RefCounted::MemoryAllocator()`, `:memory_tag` peaks/totals/`MemoryTagReport`,
`Context::Allocator()` + `Scene::Allocator()`, nav mesh/query/crowd take
required `IAllocator&`, scene system owns a tagged "Navigation" allocator;
next = P2 Core consumers per the spec's phase plan),
