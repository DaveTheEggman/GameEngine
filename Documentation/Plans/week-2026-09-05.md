...
- *Web (task #112)*: (A) WebSocketTransport + native WS accept path (the big
  one - browser client joins native host), (B) Firefox compat, (C) audio
  autoplay resume-on-gesture + AS browser smoke. Supersedes the shorter
  roadmap-absorbed web entry above.
