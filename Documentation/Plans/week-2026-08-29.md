...
FOLLOW-UP (small): a Joint gizmo (constraint anchors/limits from JointComponent data) - the same recipe;
the only sub-item of the original I1 scope not yet landed. Pattern note for other domains stands:
runtime state -> DebugScene, authoring aids from component/resource data -> gizmo renderer.
